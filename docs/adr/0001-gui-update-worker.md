...
For npm installs, the worker runs the Node launcher path (`node bin/opr.mjs update --tag <tag>`) so
...
For npm installs specifically, `node opr.mjs update` already stops the proxy and reinstalls /
...
npm installs skip the probe-first path entirely, because the launcher only prints `opr start`
