...
- 장점, 단점 및 영향: Headless parity improves and future operations share consistent errors. Live management commands require a running proxy; offline config inspection/import remains available through a separately validated `opr config` path.

## Command structure

```text
opr setup                  interactive first-run flow (`init` remains an alias)
opr provider ...           provider config, test, quota, selected models
opr account ...            Codex/OAuth/key-pool login and lifecycle
opr models ...             live/custom models, visibility, context, shadow calls
opr route combo ...        failover and round-robin virtual models
opr agent ...              subagents, fallback, injection, effort, sidecars
opr observe ...            logs, usage, storage, memory, debug captures
opr access ...             external API keys, endpoints, model tests
opr integration ...        Claude and Grok client integrations
opr system ...             settings, startup, diagnostics, sync, update jobs
opr config ...             masked inspection and validated offline import/edit
```
...
- Live mutations go through the running management API. `opr config import/set` validates
