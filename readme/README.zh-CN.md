...
opr start        # 代理 + 仪表盘: localhost:10100
```

<p align="center">
  <img src="../assets/claude-code-models.gif" alt="通过 openprovider 运行路由模型的 Claude Code —— 状态栏显示 gpt-5.6-luna-medium 为当前模型" width="820"><br>
  <sub><b>Claude Code 可以用任何模型。</b>选择器是原生 Claude Code，跑起来的模型随你挑。</sub>
</p>

<p align="center">
  <img src="../assets/demo.gif" alt="openprovider 演示 —— 在 Codex 应用中用路由的非 OpenAI 模型执行任务" width="820"><br>
  <sub><b>Codex 可以用任何模型。</b>选好 provider 直接开跑 —— 同样的 Codex 工作流，换个大脑。</sub>
</p>

<p align="center">
  <a href="../README.md">English</a> · <a href="README.ko.md">한국어</a> · <b>简体中文</b> · <a href="README.ru.md">Русский</a> · <a href="README.ja.md">日本語</a> · 📖 <a href="https://openprovider.me/zh-cn/"><b>完整文档 →</b></a>
</p>

<p align="center">
  <img src="../assets/architecture.png" alt="openprovider 架构 — Codex CLI 通过 openprovider 代理路由到任意 LLM 提供商" width="820">
</p>

在 Codex 中 —— 以及在 **Claude Code** 中 —— 使用 Claude、Gemini、Grok、GLM、DeepSeek、Kimi、Qwen、Ollama 或任意其他 LLM，无需等待官方添加支持。

openprovider 是一个轻量级本地代理，把 Codex 的 Responses API 翻译成你的 provider 所讲的协议。streaming、tool 调用、reasoning token、图片 —— 全部双向工作。
...
```mermaid
flowchart LR
  codex[Codex 会话<br/>CLI, App, SSH, 移动端] --> proxy[openprovider]
  proxy --> existing{已有线程?}
...
# 正常使用 Codex —— 请求已经通过 openprovider 路由
...
openprovider 把 Bun 运行时作为依赖打包，并通过 Node 启动器运行，所以你**不需要**自己安装 Bun。如果看到 "bundled Bun runtime is missing" 错误，说明安装时跳过了 lifecycle 脚本（包括 npm 通过 `allowScripts` 拦截 bun postinstall 的情况）或 optional 依赖。请允许 bun 安装脚本后重新安装：
...
- **在 Claude 中也能使用任意 LLM。** `opr claude` 可通过代理启动 Claude Code。Claude 仪表盘还提供独立的 Desktop 配置，可管理 Opus、Fable、Sonnet、Haiku 四个系列，并支持拖放、键盘操作和 JSON 导入/导出。
...
- **干净退出，零残留。** `opr stop`（或仪表盘的 Stop 按钮）会关闭代理、停止已安装的后台服务，并将 Codex 恢复为原始配置。之后 `codex` 就像从未安装过 openprovider 一样工作 —— 无残留配置，无僵尸进程。
...
你也可以通过 `opr init`（交互式 CLI）或直接编辑 `~/.openprovider/config.json` 来添加 provider。
...
省略 `provider/` 前缀时，openprovider 会路由到默认 provider，或根据模型名模式自动匹配（例如 `claude-*`
...
除非 provider config 明确设置 alias，openprovider 会把 `xhigh` 与 `max` 保持为不同档位。`ultra` 与上游
...
preview gate 限制；openprovider 只是准备好你的账户/provider 可访问时所需的路由和目录元数据。

<p align="center">
  <img src="../assets/codex-app-picker.png" alt="Codex App 展示 openprovider 路由模型及 reasoning effort 选择器" width="480">
</p>
...
openprovider 保持两种独立行为：

- **现有会话保持 affinity。** 线程 id 绑定到所选账户并在后续轮次复用，因此长请求或移动/SSH 连接的会话
  会继续使用同一账户。
- **新会话可自动路由。** 启用自动切换后，openprovider 比较 5 小时、每周、30 天使用量中最热的配额窗口，
...
此外还有 DeepSeek、Groq、OpenRouter、Together、Fireworks、Cerebras、Mistral、Hugging Face、NVIDIA NIM、MiniMax、Qwen Cloud、腾讯云 Coding Plan、SiliconFlow 等等。完整列表可通过 `opr init` 查看，或参阅 [provider 文档](https://openprovider.me/zh-cn/reference/configuration/)。

## CLI

```bash
opr init                       # 交互式初始化
opr start [--port 10100]       # 启动代理
opr stop                       # 停止并恢复原生 Codex 配置
opr restore                    # 仅恢复，不停止（别名：opr eject）
opr uninstall                  # 移除 service/shim/config 并恢复原生 Codex
opr ensure                     # 按需启动 + 刷新 Codex config/cache
opr sync                       # 刷新模型列表 + 重新注入 Codex
opr status                     # 查看代理是否在运行
opr login <xai|anthropic|kimi> # OAuth 登录
opr logout <provider>          # 移除已保存的登录
opr account <list|current|use> # 查看/切换账号与 API-key pool（脱敏；含 refresh/auto-switch/remove/add-key）
opr gui                        # 打开 Web 仪表盘
opr claude [args...]           # 启动接入代理的 Claude Code（模型发现已开启）
opr claude desktop             # 保存并应用 Claude Desktop 四系列配置
opr codex-shim install         # 运行 codex 时自动启动代理
opr service [install|start|stop|status|uninstall]   # 安装/更新/启动后台服务
opr update [--tag preview]     # 更新 openprovider；preview 安装保持 @preview
```
...
```bash
opr claude desktop [apply]                         # 保存并应用当前配置
opr claude desktop show [--json]                   # 查看路由、系列和默认值
opr claude desktop move <route> <family> [--default]
opr claude desktop default <family> <route|none>
opr claude desktop export <path|->                 # 使用 - 将 JSON 输出到 stdout
opr claude desktop import <path> [--apply]         # 验证后保存，可选择立即应用
```
...
`opr claude desktop --static`、`--hybrid` 和
`--discovery-only` 仍然受支持。

### 自动启动：service vs shim

openprovider 提供两种自动启动代理的方式：
...
| **Codex 更新** | 不受影响 | 稳定完成的启动器替换会在下一条普通 `opr` 命令中修复 |
| **移除** | `opr service uninstall` | `opr codex-shim uninstall` |

如需常驻代理，使用 **service**（推荐开发环境）。轻量按需启动使用 **shim**。

如果外部 Codex 更新覆盖了已安装的 shim，下一条普通 `opr` 命令会备份已稳定的新启动器并恢复
...
`opr uninstall` 会停止代理、移除已安装的 service、移除 Codex shim、恢复原生 Codex config/catalog/history，并删除 `~/.openprovider`。

## 配置

配置文件路径：`~/.openprovider/config.json`。
...
> openprovider 会在发送请求前剥离末尾的 `[1m]` 后缀，因为 OpenAI 兼容端点会拒绝带方括号的 id（Z.AI 400 code
...
本地 provider 的 `apiKey` 通常留空。只要你的本地服务暴露了 OpenAI 兼容的 Chat Completions 端点，openprovider 就能直接对接。

WebSocket 传输默认关闭。只有当你希望 Codex 使用 Responses WebSocket 而不是 HTTP/SSE 时，才需要设置 `"websockets": true`。

### 远程访问

默认情况下 openprovider 绑定到 `127.0.0.1`（回环）且无需额外认证。
如果你设置 `"hostname": "0.0.0.0"` 把代理暴露到局域网，openprovider 会要求一个 bearer token 来同时保护管理
...
绑定到非回环地址时若缺少该环境变量，代理会拒绝启动。若为局域网访问安装后台服务，请在 `opr service install`
...
openprovider 会自动 remap Codex resume 历史，使旧的 OpenAI 对话和 openprovider 创建的项目线程在代理活动期间仍在
Codex App 中可见。原始 provider/source 元数据记录在 `~/.openprovider/codex-history-backup.json`。`opr stop` /
`opr restore` 会把备份的 OpenAI 行恢复到 OpenAI，并把剩余的 openprovider 用户线程也 eject 到 OpenAI，这样原生
...
每个字段的详细说明参阅 **[配置参考](https://openprovider.me/zh-cn/reference/configuration/)**。

## 文档

完整文档——安装、provider 配置、路由、sidecar、Codex 集成、Codex App 模型选择器、CLI/配置参考——由 [`docs-site/`](../docs-site) 目录下的 Astro 站点构建，发布在 **[openprovider.me](https://openprovider.me/zh-cn/)**。
...
参阅 **[贡献指南](https://openprovider.me/zh-cn/contributing/)**。

## 免责声明

openprovider 是一个独立的社区维护项目，**与 OpenAI、Anthropic 或任何其他提供商无关，也未获得其认可。**

某些提供商——尤其是 Anthropic (Claude)——可能会对通过第三方代理路由 API 流量的账户进行暂停或限制。**使用风险自负 (UAYOR)。** 在连接提供商之前，请查阅其服务条款以确认是否允许基于代理的访问。openprovider 维护者不对上游提供商采取的任何账户操作承担责任。
