...
opr start        # прокси + дашборд: localhost:10100
```

<p align="center">
  <img src="../assets/claude-code-models.gif" alt="Claude Code работает на маршрутизированной модели через openprovider — в строке состояния активна gpt-5.6-luna-medium" width="820"><br>
  <sub><b>Claude Code на любой модели.</b> Селектор — обычный Claude Code, а вот модель за ним — какую захотите.</sub>
</p>

<p align="center">
  <img src="../assets/demo.gif" alt="Демонстрация openprovider — выполнение задачи в приложении Codex на маршрутизируемой модели не от OpenAI" width="820"><br>
  <sub><b>Codex на любой модели.</b> Выберите провайдера — и вперёд: тот же рабочий процесс Codex, другой «мозг».</sub>
</p>

<p align="center">
  <a href="../README.md">English</a> · <a href="README.ko.md">한국어</a> · <a href="README.zh-CN.md">简体中文</a> · <b>Русский</b> · <a href="README.ja.md">日本語</a> · 📖 <a href="https://openprovider.me/ru/"><b>Полная документация →</b></a>
</p>

<p align="center">
  <img src="../assets/architecture.png" alt="Архитектура openprovider — Codex CLI направляет запросы через прокси openprovider к любому LLM-провайдеру" width="820">
</p>
...
openprovider — это лёгкий локальный прокси, который транслирует Responses API Codex в протокол, понятный вашему провайдеру. Потоковая передача, вызовы инструментов, токены рассуждений, изображения — всё работает в обе стороны.

Кроме того, openprovider умеет управлять **пулом аккаунтов ChatGPT** для аутентификации Codex. Добавьте
...
```mermaid
flowchart LR
  codex[Сессия Codex<br/>CLI, App, SSH, мобильный] --> proxy[openprovider]
...
# Используйте Codex как обычно — теперь запросы идут через openprovider
...
openprovider поставляет рантайм Bun как зависимость и запускает его через Node-лончер,
...
Провайдеров также можно добавлять через `opr init` (интерактивный CLI) или напрямую редактируя `~/.openprovider/config.json`.
...
Если префикс `provider/` опущен, openprovider направляет запрос провайдеру по умолчанию — либо автоматически подбирает провайдера по шаблону имени модели (например, `claude-*` уходит в Anthropic, `gpt-*` — в OpenAI).
...
`max` и `ultra`, если модель их объявляет. openprovider сохраняет `xhigh` и `max` как разные уровни,
...
стороне; openprovider лишь подготавливает маршрутизацию и метаданные каталога для аккаунтов и
провайдеров, которые могут их обслуживать.

<p align="center">
  <img src="../assets/codex-app-picker.png" alt="Codex App с маршрутизируемыми моделями openprovider и селектором уровня рассуждений" width="480">
</p>
...
- Устаревший публичный id провайдера `chatgpt` после миграции скрывается. Исходная конфигурация
  однократно сохраняется в `~/.openprovider/config.json.pre-openai-tiers-v2.bak`; восстановить её можно командой
...
обслужит следующую сессию Codex. openprovider гарантирует следующее поведение:
...
- **Новые сессии могут маршрутизироваться автоматически.** При включённом автопереключении openprovider
...
- **Любая LLM и в Claude Code.** Тот же демон обслуживает Anthropic Messages API (`/v1/messages` + `count_tokens`): `opr claude` запускает Claude Code с полностью готовой конфигурацией, а маршрутизируемые модели появляются в его родном селекторе `/model` благодаря обнаружению моделей через шлюз (алиасы `claude-opr-<provider>--<model>`, Claude Code 2.1.129+). Слоты и сопоставления моделей настраиваются на странице Claude панели управления.
...
- **Работает в фоне.** Установите как системную службу (launchd / systemd / Task Scheduler) и забудьте о ней. На macOS/Linux прокси стартует при входе в систему; на Windows бэкенд Task Scheduler по умолчанию запускается при входе (без окон), либо используйте `opr service install --native`, чтобы получить полноценную службу Windows, стартующую при загрузке.
- **Чистый выход без следов.** `opr stop` (или кнопка Stop в панели управления) завершает работу прокси, останавливает фоновую службу, если она установлена, и возвращает Codex к исходной конфигурации. Обычный `codex` работает ровно так же, как раньше, — без остатков конфигурации и осиротевших процессов.
...
А также DeepSeek, Groq, OpenRouter, Together, Fireworks, Cerebras, Mistral, Hugging Face, NVIDIA NIM, MiniMax, Qwen Cloud, Tencent Cloud Coding Plan, SiliconFlow и другие. Полный список — в `opr init` или в [документации по провайдерам](https://openprovider.me/reference/configuration/).

Поддержка Cursor — поэтапный экспериментальный мост: он появляется в `opr init` и в селекторе
...
не настроен, openprovider возвращает типизированные ответы об отсутствии исполнителя вместо
...
```bash
opr init                       # интерактивная настройка
opr start [--port 10100]       # запустить прокси; если порт занят, выбирается свободный
opr stop                       # остановить + восстановить нативный Codex
opr restore                    # восстановить без остановки (алиас: opr eject)
opr uninstall                  # удалить службу/shim/конфигурацию и восстановить нативный Codex
opr ensure                     # запустить при необходимости + обновить конфигурацию/кэш Codex
opr sync                       # обновить модели + заново встроиться в Codex
opr codex-shim install         # выполнять `opr ensure` при каждом запуске `codex`
opr status                     # работает ли прокси?
opr login <provider>          # вход через OAuth (xai, anthropic, kimi, cursor, ...)
opr logout <provider>          # удалить сохранённый вход
opr account <list|current|use> # просмотр/переключение аккаунтов и пулов API-ключей (маскировано; также refresh/auto-switch/remove/add-key)
opr gui                        # открыть веб-панель управления
opr claude [args...]           # запустить Claude Code, подключённый к прокси (обнаружение моделей включено)
opr service [install|start|stop|status|uninstall]   # установить/обновить/запустить фоновую службу
opr update [--tag preview]     # обновить openprovider; preview-установки остаются на @preview
```

### Автозапуск: служба или shim

У openprovider есть два способа автоматически запускать прокси:
...
| **Когда** | Всегда работает после входа в систему | По требованию — выполняет `opr ensure` при запуске `codex` |
| **Перезапуск** | Автоматический перезапуск при сбое | Запускается один раз на каждый вызов `codex` |
| **Обновления Codex** | Не влияют | Стабильно заменённый лончер восстанавливается следующей обычной командой `opr` |
| **Удаление** | `opr service uninstall` | `opr codex-shim uninstall` |

Используйте **службу**, если прокси должен работать постоянно (рекомендуется для машин разработчиков).
Если внешнее обновление Codex перезапишет установленный shim, следующая обычная команда `opr`
...
`opr uninstall` останавливает прокси, удаляет установленную службу, удаляет shim для Codex,
восстанавливает нативные конфигурацию/каталог/историю Codex и удаляет `~/.openprovider`.

## Конфигурация

Конфигурация хранится в `~/.openprovider/config.json`. Если файл не удаётся разобрать (например,
JSON обрезан или испорчен вручную), openprovider сохраняет его резервную копию в
...
> и `glm-5.2[1m]` — openprovider отрезает завершающий суффикс `[1m]` перед отправкой
...
Локальные модели тоже работают. Направьте openprovider на любой OpenAI-совместимый сервер,
...
По умолчанию openprovider привязывается к `127.0.0.1` (loopback) и не требует дополнительной аутентификации.
Если вы задаёте `"hostname": "0.0.0.0"`, открывая прокси в локальной сети, openprovider требует bearer-токен
...
`opr service install`, чтобы менеджер служб её получил.
...
openprovider автоматически перепривязывает историю возобновления Codex, чтобы старые чаты OpenAI и
созданные openprovider проектные треды оставались видимыми в Codex App, пока прокси активен. Исходные
метаданные provider/source openprovider записывает в `~/.openprovider/codex-history-backup.json`.
`opr stop` / `opr restore` возвращает сохранённые в резервной копии строки OpenAI обратно к OpenAI,
а оставшиеся пользовательские треды openprovider также переводит на OpenAI, чтобы нативный Codex
...
Описание всех полей — в **[справочнике по конфигурации](https://openprovider.me/reference/configuration/)**.

## Документация

Публичная документация — установка, провайдеры, маршрутизация, сайдкары, интеграция с Codex, селектор моделей Codex App и справочник по CLI/конфигурации — собирается из [`docs-site/`](../docs-site) и публикуется на **[openprovider.me](https://openprovider.me/)**.
...
openprovider — независимый проект, поддерживаемый сообществом; он **не аффилирован с OpenAI, Anthropic или каким-либо другим провайдером и не одобрен ими**.

Некоторые провайдеры — в частности Anthropic (Claude) — могут приостанавливать или ограничивать аккаунты, которые направляют API-трафик через сторонние прокси. **Используйте на свой страх и риск (UAYOR).** Прежде чем подключать провайдера, изучите его условия использования и убедитесь, что доступ через прокси разрешён. Мейнтейнеры openprovider не несут ответственности за какие-либо действия вышестоящих провайдеров в отношении аккаунтов.
