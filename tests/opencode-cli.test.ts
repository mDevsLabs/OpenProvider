...
describe("opr opencode provider block", () => {
  ...
  test("non-loopback binds add x-OpenProvider-api-key via env reference", () => {
    ...
    expect(block.options.headers).toEqual({ "x-OpenProvider-api-key": OPENCODE_API_KEY_ENV_REF });
...
describe("opr opencode runtime config", () => {
  ...
  test("merges inherited inline settings and overrides only provider.OpenProvider", () => {
...
describe("opr opencode JSONC parsing", () => {
...
describe("opr opencode proxy model catalog", () => {
  const ENV_KEY = "opr_TEST_OPENCODE_PROXY_ONLY_KEY";
      ...
      const fetched = await fetchOpencodeProxyModels(
        ...
        "sk-mgmt",
        {
          fetchImpl: async (url, init) => {
            ...
            expect(new Headers(init?.headers).get("X-OpenProvider-API-Key")).toBe("sk-mgmt");
...
describe("opr opencode native slug selection", () => {
...
describe("opr opencode project-layer detection", () => {
  test("detects a global config that redefines our provider key", () => {
    const home = mkdtempSync(join(tmpdir(), "opr-opencode-global-"));
  ...
  test("detects a project config that redefines our provider key", () => {
    const dir = mkdtempSync(join(tmpdir(), "opr-opencode-proj-"));
  ...
  test("detects opencode.jsonc and parent directories up to the git root", () => {
    const root = mkdtempSync(join(tmpdir(), "opr-opencode-proj-root-"));
  ...
  test("does not walk above the git root", () => {
    const root = mkdtempSync(join(tmpdir(), "opr-opencode-proj-stop-"));
...
describe("opr opencode env assembly", () => {
  test("OPENCODE_CONFIG_CONTENT carries only the runtime provider block", () => {
    const block = buildOpencodeProviderBlock(10100, [], [{ provider: "kiro", id: "glm-5" }]);
    const built = buildOpencodeEnv(block, "sk-opr-123", { OPENCODE_CONFIG: "/user/mine.json", PATH: "/bin" });
  ...
  test("preserves inherited inline settings in OPENCODE_CONFIG_CONTENT", () => {
    ...
    const built = buildOpencodeEnv(block, "sk-opr-123", { [OPENCODE_CONFIG_CONTENT_ENV]: inherited });
    expect(isOpencodeRuntimeConfigError(built)).toBe(false);
    if (isOpencodeRuntimeConfigError(built)) return;
  ...
  test("surfaces invalid inherited OPENCODE_CONFIG_CONTENT as an error", () => {
    const block = buildOpencodeProviderBlock(10100, [], []);
    expect(buildOpencodeEnv(block, "sk-opr-123", { [OPENCODE_CONFIG_CONTENT_ENV]: "[]" }))
  ...
  test("the admission key travels in the child env, matching the config's {env:…} reference", () => {
    const block = buildOpencodeProviderBlock(10100, [], []);
    const built = buildOpencodeEnv(block, "sk-opr-123", {});
    expect(isOpencodeRuntimeConfigError(built)).toBe(false);
    if (isOpencodeRuntimeConfigError(built)) return;
    expect(built[OPENCODE_API_KEY_ENV]).toBe("sk-opr-123");
    expect(built[OPENCODE_CONFIG_CONTENT_ENV]).not.toContain("sk-opr-123");
  });
});

describe("opr opencode admission key", () => {
  test("the environment token wins over a configured API key", () => {
    ...
    expect(opencodeApiKey(config, { OpenProvider_API_AUTH_TOKEN: "sk-env" })).toBe("sk-env");
  });

  test("falls back to the hardened service token file before config.apiKeys", () => {
    const dir = mkdtempSync(join(tmpdir(), "opr-opencode-token-"));
    const tokenFile = join(dir, "service-api-token");
    writeFileSync(tokenFile, "sk-service\n", "utf8");
    const config = cfg({ apiKeys: [{ id: "1", name: "main", key: "sk-cfg", createdAt: "2026-01-01" }] });
    expect(opencodeApiKey(config, { opr_API_TOKEN_FILE: tokenFile })).toBe("sk-service");
  });
  ...
  test("falls back to a placeholder on an open loopback proxy", () => {
    expect(opencodeApiKey(cfg(), {})).toBe("opr");
  });
});

describe("opr opencode proxy auto-start env", () => {
  test("passes opr_API_TOKEN_FILE to opr start when only the hardened service token exists", () => {
    const dir = mkdtempSync(join(tmpdir(), "opr-opencode-start-"));
    const tokenFile = join(dir, "service-api-token");
    writeFileSync(tokenFile, "sk-service-only\n", "utf8");
    const config = cfg({ hostname: "0.0.0.0", apiKeys: [] as oprConfig["apiKeys"] });

    const startEnv = opencodeProxyStartEnv({ opr_API_TOKEN_FILE: tokenFile });
    expect(startEnv.OpenProvider_API_AUTH_TOKEN).toBeUndefined();
    expect(startEnv.opr_API_TOKEN_FILE).toBe(tokenFile);
    expect(startEnv.opr_SERVICE).toBe("1");
  ...
  test("defaults opr_API_TOKEN_FILE when admission env token is absent", () => {
    const startEnv = opencodeProxyStartEnv({ hostname: "0.0.0.0" });
    expect(startEnv.OpenProvider_API_AUTH_TOKEN).toBeUndefined();
    expect(startEnv.opr_API_TOKEN_FILE).toBe(serviceApiTokenFilePath());
    expect(startEnv.opr_SERVICE).toBe("1");
  });

  test("does not inject opr_API_TOKEN_FILE when OpenProvider_API_AUTH_TOKEN is already set", () => {
    const startEnv = opencodeProxyStartEnv({ OpenProvider_API_AUTH_TOKEN: "sk-env", hostname: "0.0.0.0" });
    expect(startEnv.OpenProvider_API_AUTH_TOKEN).toBe("sk-env");
    expect(startEnv.opr_API_TOKEN_FILE).toBeUndefined();
  });
});

describe("opr opencode global config path", () => {
...
describe("opr opencode not-found hint", () => {
