 ...
 */
function configWithKey(provider: string, adapter: string, baseUrl: string): oprConfig {
  return {
    port: 10100,
...
describe("upsertOAuthProvider credential preservation", () => {
  ...
  test("persists key mode for env-backed keys without consulting the login CLI environment", () => {
    const config = configWithKey("xai", "openai-chat", "https://api.x.ai/v1");
    config.providers.xai!.apiKey = "${opr_TEST_XAI_API_KEY}";
    config.providers.xai!.apiKeyPool = [{ id: "env-key", key: "${opr_TEST_XAI_API_KEY}" }];
    const previous = process.env.opr_TEST_XAI_API_KEY;
    delete process.env.opr_TEST_XAI_API_KEY;
    try {
      upsertOAuthProvider(config, "xai");
      const provider = config.providers.xai!;
      expect(provider.apiKey).toBe("${opr_TEST_XAI_API_KEY}");
      expect(provider.apiKeyPool).toEqual([{ id: "env-key", key: "${opr_TEST_XAI_API_KEY}" }]);
      expect(provider.authMode).toBe("key");
    } finally {
      if (previous === undefined) delete process.env.opr_TEST_XAI_API_KEY;
      else process.env.opr_TEST_XAI_API_KEY = previous;
    }
  });

  test("falls back to OAuth at routing time when the proxy cannot resolve the active key", () => {
    const config = configWithKey("xai", "openai-chat", "https://api.x.ai/v1");
    config.providers.xai!.apiKey = "${opr_TEST_XAI_API_KEY_MISSING}";
    config.providers.xai!.apiKeyPool = [{ id: "env-key", key: "${opr_TEST_XAI_API_KEY_MISSING}" }];
    upsertOAuthProvider(config, "xai");
    expect(config.providers.xai!.authMode).toBe("key");

    const previous = process.env.opr_TEST_XAI_API_KEY_MISSING;
    delete process.env.opr_TEST_XAI_API_KEY_MISSING;
    try {
      ...
      if (previous === undefined) delete process.env.opr_TEST_XAI_API_KEY_MISSING;
      else process.env.opr_TEST_XAI_API_KEY_MISSING = previous;
    }
  });
  ...
  test("uses key billing at routing time when the proxy resolves the env-backed active key", () => {
    ...
    const previous = process.env.opr_TEST_XAI_API_KEY;
    process.env.opr_TEST_XAI_API_KEY = "resolved-xai-secret";
    try {
      const routed = routeModel(config, "xai/grok-4.5").provider;
  ...
  test("CLI and proxy env visibility can diverge without rewriting stored authMode", () => {
    const config = configWithKey("xai", "openai-chat", "https://api.x.ai/v1");
    config.providers.xai!.apiKey = "${opr_TEST_XAI_SPLIT_ENV}";
    config.providers.xai!.apiKeyPool = [{ id: "env-key", key: "${opr_TEST_XAI_SPLIT_ENV}" }];
    const previous = process.env.opr_TEST_XAI_SPLIT_ENV;

    // Login CLI sees the secret; upsert still records key intent, not CLI resolution.
    process.env.opr_TEST_XAI_SPLIT_ENV = "cli-only-secret";
    upsertOAuthProvider(config, "xai");
    expect(config.providers.xai!.authMode).toBe("key");

    // Running proxy lacks the env var: route on OAuth without touching stored mode.
    delete process.env.opr_TEST_XAI_SPLIT_ENV;
    ...
    process.env.opr_TEST_XAI_SPLIT_ENV = "proxy-secret";
    ...
    if (previous === undefined) delete process.env.opr_TEST_XAI_SPLIT_ENV;
    else process.env.opr_TEST_XAI_SPLIT_ENV = previous;
  });
  ...
  test("inserts a missing active key into the pool so listing matches routing", () => {
    ...
    const testHome = mkdtempSync(join(tmpdir(), "opr-oauth-upsert-pool-"));
    process.env.OpenProvider_HOME = testHome;
    try {
      upsertOAuthProvider(config, "xai");
      const provider = config.providers.xai!;
  ...
  test("returns to oauth after the last stored API key is removed", () => {
    const config = configWithKey("xai", "openai-chat", "https://api.x.ai/v1");
    const previousHome = process.env.OpenProvider_HOME;
    const testHome = mkdtempSync(join(tmpdir(), "opr-oauth-upsert-"));
    process.env.OpenProvider_HOME = testHome;
    try {
  ...
  test("a fresh login on an unconfigured provider gets the untouched preset", () => {
    const config = { port: 10100, defaultProvider: "openai", providers: {} } as unknown as oprConfig;
    upsertOAuthProvider(config, "xai");
    const provider = config.providers.xai!;
    expect(provider.authMode).toBe("oauth");
    expect(provider.apiKey).toBeUndefined();
