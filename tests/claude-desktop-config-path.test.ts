 ...
 * three branches; OpenProvider only implemented the third and hardcoded the macOS
...
describe("Claude Desktop configLibrary resolution", () => {
  ...
  test("OpenProvider_CLAUDE_DESKTOP_CONFIG_DIR wins over every branch and is used verbatim", () => {
    // Returned without join(): tests point this straight at a temp dir.
    const dir = resolveConfigLibraryDir({
      env: {
        OpenProvider_CLAUDE_DESKTOP_CONFIG_DIR: "/tmp/opr-override",
    ...
    expect(dir).toBe("/tmp/opr-override");
  });
...
/**
 * Desktop reads ONLY the profile named by `_meta.json`'s appliedId, so an OpenProvider
...
describe("Claude Desktop status reports whether our profile is the active one", () => {
  async function statusWithMeta(meta: unknown): Promise<{ activeProfile: boolean | null }> {
    const dir = mkdtempSync(join(tmpdir(), "opr-desktop-active-"));
    const previous = process.env.OpenProvider_CLAUDE_DESKTOP_CONFIG_DIR;
    ...
    try {
      ...
      server.stop(true);
      if (previous === undefined) delete process.env.OpenProvider_CLAUDE_DESKTOP_CONFIG_DIR;
      else process.env.OpenProvider_CLAUDE_DESKTOP_CONFIG_DIR = previous;
  ...
  test("appliedId pointing at another profile reports activeProfile false", async () => {
    const body = await statusWithMeta({
      ...
      entries: [
        ...
        { id: "22222222-2222-4222-8222-222222222222", name: "OpenProvider" },
      ],
    });
  ...
  test("a readable appliedId with no OpenProvider entry is a known false, not unknown", async () => {
  ...
  test("metadata without appliedId leaves activeProfile undeterminable", async () => {
    const body = await statusWithMeta({
      entries: [{ id: "22222222-2222-4222-8222-222222222222", name: "OpenProvider" }],
    });
