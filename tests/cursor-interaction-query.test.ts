...
describe("planInteractionQueryReply", () => {
  ...
  // (which injects results into the model), so openprovider APPROVES instead of rejecting — rejecting
...
describe("isClientToolFrame", () => {
  ...
  test("opr-bridged mcpToolCall counts as client tool activity", () => {
    const toolCall = create(ToolCallSchema, {
      tool: {
        case: "mcpToolCall",
        value: create(McpToolCallSchema, {
          args: create(McpArgsSchema, { name: "exec_command", toolName: "exec_command", toolCallId: "call_1", providerIdentifier: "openprovider-responses" }),
        }),
      },
    });
