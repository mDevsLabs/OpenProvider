...
function tempGrokHome(): { root: string; grokHome: string } {
  const root = mkdtempSync(join(tmpdir(), "opr-grok-sel-"));
...
// THE audit regression: kimi/k3 and kimi/k3[1m] both sanitize toward opr-kimi-k3-….
...
test("excluding a colliding model leaves the survivor's alias unchanged", () => {
  ...
  const survivorAlias = /\[model\.(opr-[^\]]+)\][^[]*model = "kimi-k3"/.exec(full)?.[1];
  expect(survivorAlias).toBe("opr-kimi-k3-2");
...
test("excluding the first model keeps the TOML shape valid", () => {
  ...
  const afterMarker = block.split("do not edit (removed by `opr stop`) >>>\n")[1]!;
...
test("excluding everything writes a marker-only fence that strip removes", () => {
  ...
  try {
    ...
    expect(content).toContain(">>> openprovider managed block");
...
test("a user-reserved alias still pushes generated aliases past it while exclusions are active", () => {
  const { root, grokHome } = tempGrokHome();
  try {
    writeFileSync(join(grokHome, "config.toml"), '[model.opr-kimi-k3]\nmodel = "mine"\n', "utf8");
    ...
    expect(content).toContain('[model.opr-kimi-k3]\nmodel = "mine"');
    // And the survivor's generated alias starts AFTER the reservation, not at -1.
    expect(content).not.toContain("[model.opr-kimi-k3]\nmodel = \"kimi-k3\"");
  } finally {
    rmSync(root, { recursive: true, force: true });
  }
});
...
test("syncGrokConfig with grokExcludedModels omits that model's table end to end", async () => {
  ...
  try {
    ...
    expect(content).toContain("[model.opr-gpt-");
