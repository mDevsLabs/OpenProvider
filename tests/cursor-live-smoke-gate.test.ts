...
describe("Cursor live smoke credential gate", () => {
  ...
  test("present token enables the gate without storing the token in the gate object", () => {
    const gate = readCursorLiveSmokeGate({
      OPENPROVIDER_CURSOR_TEST_TOKEN: fakeSecret,
      OPENPROVIDER_CURSOR_TEST_BASE_URL: "https://cursor.example.test",
    });
  ...
  test("token accessor is separate from the public gate status", () => {
    expect(getCursorLiveSmokeToken({ OPENPROVIDER_CURSOR_TEST_TOKEN: ` ${fakeSecret} ` })).toBe(fakeSecret);
    expect(getCursorLiveSmokeToken({ OPENPROVIDER_CURSOR_TEST_TOKEN: "   " })).toBeUndefined();
  });

  test("skip messages never include token values", () => {
    const gate = readCursorLiveSmokeGate({ OPENPROVIDER_CURSOR_TEST_TOKEN: fakeSecret });
