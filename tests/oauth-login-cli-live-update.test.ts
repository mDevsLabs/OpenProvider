...
function keyModeXaiConfig(port = 0): oprConfig {
  return {
    port,
    hostname: "127.0.0.1",
    defaultProvider: "xai",
...
beforeEach(() => {
  ...
  isolatedCodexHome = installIsolatedCodexHome("opr-oauth-live-codex-");
  testDir = mkdtempSync(join(tmpdir(), "opr-oauth-live-"));
...
describe("CLI OAuth live-update credential preservation", () => {
  test("notify after OAuth login keeps key billing on live and disk configs", async () => {
    ...
    try {
      ...
      const disk = JSON.parse(readFileSync(join(testDir, "config.json"), "utf-8")) as oprConfig;
