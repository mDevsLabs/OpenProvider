...
beforeEach(() => {
  ...
  previousCleanupTestHooks = process.env.OpenProvider_CLEANUP_TEST_HOOKS;
  process.env.OpenProvider_CLEANUP_TEST_HOOKS = "1";
  isolatedCodexHome = installIsolatedCodexHome("opr-restore-job-responsive-codex-");
  testDir = mkdtempSync(join(tmpdir(), "opr-restore-job-responsive-"));
...
afterEach(() => {
  ...
  if (previousCleanupTestHooks === undefined) delete process.env.OpenProvider_CLEANUP_TEST_HOOKS;
  else process.env.OpenProvider_CLEANUP_TEST_HOOKS = previousCleanupTestHooks;
...
describe("storage trash restore job responsiveness", () => {
  async function withTestStreamRoute(
    ...
    const previousHooksEnv = process.env.OpenProvider_CLEANUP_TEST_HOOKS;
    setup();
    ...
    try {
      ...
      try {
        ...
        setRestoreTrashJobTestHooks(null);
        if (previousHooksEnv === undefined) delete process.env.OpenProvider_CLEANUP_TEST_HOOKS;
        else process.env.OpenProvider_CLEANUP_TEST_HOOKS = previousHooksEnv;
      }
    }
  }

  test("test-stream route is absent without OpenProvider_CLEANUP_TEST_HOOKS", async () => {
    await withTestStreamRoute(
      () => {
        delete process.env.OpenProvider_CLEANUP_TEST_HOOKS;
