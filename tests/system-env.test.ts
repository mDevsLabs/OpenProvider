...
describe("system environment injection", () => {
  test("injectSystemEnv sets the Claude launchctl variables on macOS", async () => {
    ...
    // may add owned opr-*.md writes — devlog 070; count is no longer fixed).
  ...
  // re-injecting without proxy mode must unset ONLY the openprovider-owned dummy token.
  ...
  test("re-inject after switching back to subscription unsets the owned dummy token", async () => {
    ...
    mockAuthTokenGetenv("openprovider-proxy");
  ...
  test("re-inject preserves a tracked token whose value is not the openprovider dummy", async () => {
    trackingFile = trackingWithToken();
    launchctlBaseUrl = "http://127.0.0.1:4567";
  ...
  test("re-inject preserves an untracked dummy-valued token it does not own", async () => {
    ...
    // carries "openprovider-proxy" but WE never injected it (not in injectedKeys).
