...
test("the managed fence stamps the grok attribution header on every model", () => {
  ...
  expect(block).toContain('extra_headers = { "x-openprovider-grok" = "1" }');
  ...
  expect(modelSections[0]).toContain("x-openprovider-grok");
});

test("the fence survives a write and keeps the header line parseable", () => {
  const root = mkdtempSync(join(tmpdir(), "opr-grok-attr-"));
  ...
  try {
    ...
    expect(content).toContain('extra_headers = { "x-openprovider-grok" = "1" }');
  } finally {
...
test("a request carrying the header is tagged grok; one without is not", async () => {
  ...
  await handleChatCompletions(chatReq({ "x-openprovider-grok": "1" }), config, tagged).catch(() => {});
