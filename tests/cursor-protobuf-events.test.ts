...
describe("Cursor MCP display-name alias", () => {
  test("mcp_openprovider-responses_ prefixed calls resolve to the advertised tool", () => {
    const state = createCursorProtobufEventState({ clientToolNames: ["exec_command"] });
    const toolCall = mcpToolCall("mcp_openprovider-responses_exec_command", { cmd: "echo ok" });
  ...
  test("unknown tools are still rejected after normalization", () => {
    const state = createCursorProtobufEventState({ clientToolNames: ["exec_command"] });
    const toolCall = mcpToolCall("mcp_openprovider-responses_made_up_tool", { x: "1" });
    const events = mapCursorProtobufServerMessage(interaction({
      case: "toolCallStarted",
