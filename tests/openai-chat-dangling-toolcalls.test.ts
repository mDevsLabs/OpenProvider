...
function wire(messages: oprMessage[]): ChatMsg[] {
...
function user(text: string): oprMessage {
  return { role: "user", content: text, timestamp: 0 };
}

function developer(text: string): oprMessage {
  return { role: "developer", content: text, timestamp: 0 };
}

function assistantWithCalls(calls: { id: string; name: string }[], text = ""): oprMessage {
  return {
    role: "assistant",
    content: [
...
function toolResult(callId: string, name: string, output = "ok"): oprMessage {
...
describe("openai-chat dangling tool_calls hardening", () => {
  ...
  test("T8 empty call id is minted so the call can be answered", () => {
    ...
    expect(minted).toMatch(/^call_opr_minted_\d+$/);
