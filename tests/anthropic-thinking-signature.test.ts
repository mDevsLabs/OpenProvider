...
import { encodeReasoningEnvelope, decodeReasoningEnvelope, opr_REASONING_PREFIX } from "../src/responses/reasoning-envelope";
import type { AdapterEvent, oprProviderConfig, oprThinkingContent } from "../src/types";
...
describe("bridge oprr1 envelope emission", () => {
...
describe("parser oprr1 decode + anthropic replay", () => {
  test("reasoning input with oprr1 envelope restores the real signature", async () => {
    ...
    const thinking = (assistant as unknown as { content: oprThinkingContent[] }).content.find(p => p.type === "thinking");
  ...
  test("native (non-oprr1) encrypted_content keeps the placeholder signature", async () => {
    const parsed = parseRequest({
      model: "anthropic/claude-x",
      input: [
...
describe("passthrough scrub of oprr1 envelopes", () => {
  test("sanitize strips oprr1 encrypted_content even with empty content", async () => {
    ...
    const body = {
      model: "gpt-5.5",
      input: [
        { type: "reasoning", id: "rs_1", summary: [], encrypted_content: opr_REASONING_PREFIX + Buffer.from(JSON.stringify({ sig: "RealSig1234567890==" })).toString("base64") },
      ],
    };
    // Build the outgoing request the adapter would send; the oprr1 envelope must be stripped.
    const req = await adapter.buildRequest({ _rawBody: body, model: "gpt-5.5", messages: [], options: {} } as never) as { body?: string };
    expect(req.body ?? "").not.toContain(opr_REASONING_PREFIX);
