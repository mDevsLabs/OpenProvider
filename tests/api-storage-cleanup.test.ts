...
beforeEach(() => {
  ...
  isolatedCodexHome = installIsolatedCodexHome("opr-api-storage-cleanup-codex-");
  testDir = mkdtempSync(join(tmpdir(), "opr-api-storage-cleanup-"));
...
describe("POST /api/storage/cleanup", () => {
  ...
  test("partial permanent purge returns relative trashDir on the wire", async () => {
    seedArchived(isolatedCodexHome!.path);
    const previous = process.env.OpenProvider_CLEANUP_TEST_HOOKS;
    ...
    try {
      ...
      await server.stop(true);
      if (previous === undefined) delete process.env.OpenProvider_CLEANUP_TEST_HOOKS;
      else process.env.OpenProvider_CLEANUP_TEST_HOOKS = previous;
    }
  });
});
