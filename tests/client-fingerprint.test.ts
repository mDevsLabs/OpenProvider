...
describe("client fingerprint — anthropic OAuth headers", () => {
  const oauthProvider = { adapter: "anthropic", authMode: "oauth", baseUrl: "https://api.anthropic.com", apiKey: "oauth-tok-123" } as unknown as oprProviderConfig;
  const apiKeyProvider = { adapter: "anthropic", baseUrl: "https://api.anthropic.com", apiKey: "sk-ant-123" } as unknown as oprProviderConfig;
  ...
  test("outgoing session-id header never echoes the raw OAuth token", async () => {
    const secretProvider = { adapter: "anthropic", authMode: "oauth", baseUrl: "https://api.anthropic.com", apiKey: "oauth-super-secret-xyz" } as unknown as oprProviderConfig;
  ...
  test("Accept negotiates SSE for a streaming request", async () => {
    const streaming = { ...parsed(), stream: true } as oprParsedRequest;
