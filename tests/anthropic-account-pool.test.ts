...
import type { oprAccountPoolRotationStrategy, oprConfig } from "../src/types";

const originalHome = process.env.OpenProvider_HOME;
let home: string;

beforeEach(() => {
  home = mkdtempSync(join(tmpdir(), "opr-anthropic-pool-"));
...
function cfg(
  enabled: boolean,
  threshold = 80,
  pool: { strategy?: oprAccountPoolRotationStrategy; stickyLimit?: number } = {},
): oprConfig {
  return {
    port: 0,
