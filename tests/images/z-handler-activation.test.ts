...
beforeAll(async () => {
  ...
  mock.module("../../src/server/adapter-resolve", () => ({
    ...actualResolver,
    resolveAdapter(provider: oprProviderConfig) {
      const base = {
        name: "test",
...
afterAll(() => {
  if (PREV_HOME === undefined) delete process.env.OpenProvider_HOME;
  else process.env.OpenProvider_HOME = PREV_HOME;
  mock.restore();
});

/** Routed (non-OpenAI) keyed provider + an xAI provider with an API key so the real planImageBridge returns a plan. */
function makeConfig(): oprConfig {
  return {
    port: 0,
