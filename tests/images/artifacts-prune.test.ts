...
beforeAll(() => { process.env.OpenProvider_HOME = join(tmpdir(), "opr-prune-" + randomUUID()); });
afterAll(() => { if (PREV_HOME === undefined) delete process.env.OpenProvider_HOME; else process.env.OpenProvider_HOME = PREV_HOME; });
