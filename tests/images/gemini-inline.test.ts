...
beforeAll(() => {
  savedHome = process.env.OpenProvider_HOME;
  tempHome = mkdtempSync(join(tmpdir(), "opr-test-"));
  process.env.OpenProvider_HOME = tempHome;
  artifactsDir = join(tempHome, "artifacts");
});

afterAll(() => {
  if (savedHome !== undefined) process.env.OpenProvider_HOME = savedHome;
  else delete process.env.OpenProvider_HOME;
...
async function collectStream(provider: oprProviderConfig, chunks: unknown[]): Promise<AdapterEvent[]> {
...
const aiStudioProvider = { adapter: "google", baseUrl: "https://generativelanguage.googleapis.com", apiKey: "key" } as oprProviderConfig;
...
describe("markdown emits authenticated opaque artifact URLs", () => {
  test("streaming: markdown uses /v1/OpenProvider/artifacts/<id>, not file: or host paths", async () => {
    const events = await collectStream(aiStudioProvider, [
      { candidates: [{ content: { parts: [{ inlineData: { mimeType: "image/png", data: TINY_PNG } }] } }] },
      { candidates: [{ finishReason: "STOP" }], usageMetadata: { promptTokenCount: 1, candidatesTokenCount: 1 } },
    ]);
    ...
    expect(mdPath.startsWith("/v1/OpenProvider/artifacts/")).toBe(true);
    ...
    const id = mdPath.slice("/v1/OpenProvider/artifacts/".length);
    expect(existsSync(join(artifactsDir, id))).toBe(true);
  });

  test("non-streaming: markdown uses /v1/OpenProvider/artifacts/<id>, not file: or host paths", async () => {
    const adapter = createGoogleAdapter(aiStudioProvider);
    const events = await adapter.parseResponse(jsonResponse({
...
describe("artifact markdown never leaks OpenProvider_HOME paths", () => {
  let underHome: string;
  let savedHome: string | undefined;

  beforeAll(() => {
    savedHome = process.env.OpenProvider_HOME;
    underHome = mkdtempSync(join(homedir(), ".opr-test-leak-"));
    process.env.OpenProvider_HOME = underHome;
  });
  ...
  test("streaming: no home/username path segments in markdown", async () => {
    ...
    expect(md).toContain("/v1/OpenProvider/artifacts/");
