...
This replaces the long `claude-opr-{provider}--{model}` form only on the Claude
...
under the OpenProvider config directory, written atomically. The runtime registry
...
- Legacy `claude-opr-...` and `modelMap` precedence regression tests.
