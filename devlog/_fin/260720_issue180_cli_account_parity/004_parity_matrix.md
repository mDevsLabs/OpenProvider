...
| OAuth: add account / re-auth / manual code / cancel / logout | **partial** — `opr login/logout` cover first credential only (no `addAccount:true`, no reauth, no code paste) | OUT this loop (browser-flow; candidate) |
...
| Self-update check/run | full (`opr update`; GUI runs same job API) | — |
| Debug flags + log streams | full (`opr debug`) | — |
| Provider add/remove/config | full (`opr provider …`) | GUI-only extras: enable/disable PATCH, note edit, codexAccountMode switch — candidate |
| Model list | partial (`opr models` static only) | GUI-only: enable/disable, context caps, selected-models — candidate |
| Multi-agent mode / thread limits | full (`opr v2 …`) | — |
| Usage analytics | **gap** | candidate (`GET /api/usage`) |
| Request logs view | **gap** | candidate (`GET /api/logs`; `opr debug … logs` covers debug streams only) |
...
| Claude surface settings | partial (`opr claude` launches; settings form GUI-only) | candidate |
| Sync models trigger | full (`opr sync`) | — |
| OAuth login/add/reauth flows | partial (`opr login`) | see credential matrix |
...
- `25f3ded2 feat(providers): Re-authenticate for OAuth and Codex accounts (#171)`
  ...
  CLI coverage: ❌ — recorded as a future candidate (`opr account reauth`), NOT
...
- Dedupe: `001_gui_cli_parity_matrix.md` and `002_api_contracts.md` (first-draft
  ...
  the stale `opr login chatgpt` 🟡 row died with them (audit R1#2).
