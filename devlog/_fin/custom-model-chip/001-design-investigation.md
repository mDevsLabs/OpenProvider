...
OpenProvider 대시보드 모델 페이지(`gui/src/pages/Models.tsx`)는 두 출처에서 모델을 가져온다.
...
1. OpenProvider가 `NATIVE_OPENAI_MODELS`를 업데이트
...
```yaml
---
name: OpenProvider-custom-model-chip
colors:
...
```ts
// src/types.ts — oprConfig에 추가
interface oprCustomModel {
...
1. `src/types.ts` — `oprCustomModel` 인터페이스 + `oprConfig.customModels` 필드
...
2. **카탈로그 JSON 직접 주입**: 커스텀 모델을 `model_catalog_json`에 쓸지,
   ...
   Codex 바이너리가 읽는 포맷이므로 OpenProvider가 직접 쓰지 않는 것이 안전.
