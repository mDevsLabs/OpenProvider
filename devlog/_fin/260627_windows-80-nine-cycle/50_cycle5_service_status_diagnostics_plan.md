...
- Keep the change CLI-visible through existing `opr status` and `opr service status` flows without changing service lifecycle semantics.

## Planned diff
- `src/service.ts`: add a small diagnostic suffix/helper for service log location and include it in `serviceStatusSummary()` and direct `opr service status` output.
- `tests/service.test.ts`: assert summaries/status command source expose `serviceLogPath()`.
- `tests/cli-help.test.ts`: assert `opr status` includes the service log path.
