...
- 내용:
  - 권장 config: `[model_providers.openprovider]` 블록 + 얇은 `[model.<alias>]` 목록 (Sol 분석의 신규 표면 활용, grok 0.2.101+).
  ...
  - 인증: loopback은 아무 non-empty `XAI_API_KEY`/api_key로 충분(opr가 무시), non-loopback은 opr admission token.
...
- 카탈로그 fetch: `GROK_MODELS_BASE_URL` 설정 후 `grok models`가 opr 모델 목록을 표시하는지 + 각 항목 백엔드 폴백(chat_completions) 동작 기록.
...
## Out of scope
- opr `/v1/models` 응답 확장(zero-config), grok-build 소스 수정, push/release.
