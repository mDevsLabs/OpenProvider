# 000 — Grok Build 최신화 + OpenProvider 브리지 마감 로드맵
...
- Goal: OpenProvider를 로컬 서버로 쓰는 Grok Build 사용자가 **routed 모델 포함** 깨끗한 exit 0을 받는 상태
...
### OpenProvider 런타임
- 사용자가 재시작 완료: `healthz` → `openprovider 2.7.35`, port 10100
- `/v1/chat/completions` 인바운드 **live 확인**: routed(`cursor/grok-4.5`)·native(`gpt-5.4-mini`) 모두 200 + 정답 텍스트

## Live smoke matrix (2026-07-23, grok 0.2.101 → opr 2.7.35)

격리 `GROK_HOME=/tmp/grok-opr-smoke-260723`, `XAI_API_KEY=dummy-loopback` (grok가 비어있지 않은 키를 요구; loopback opr는 무시).
...
3. 남은 단일 블로커는 ③ usage details. 정정(리뷰 반영): grok의 **Chat Completions 디코더 자체는 usage optional로 관대**하다(001 참조). 실패한 opr-chat 런은 *configured* backend가 chat이었는데도 실제로는 **Responses 이벤트를 소비하다** required `input_tokens_details`에서 죽은 것 — stderr raw_data가 `response.completed`인 것으로 확증. 즉 strict한 것은 Responses 디코더이고, grok 하니스가 chat 설정에서도 Responses 경로를 태우는 조건이 잔여 조사 항목이다.
4. native가 통과한 이유: ChatGPT 상류가 details를 항상 포함하고 opr가 이를 보존 (`prompt_tokens_details`/`completion_tokens_details` 응답 확인). routed는 upstream이 details를 안 주면 opr가 필드를 **생략**한다:
