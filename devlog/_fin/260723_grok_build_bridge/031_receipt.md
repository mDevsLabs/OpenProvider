...
030의 A안은 `[model_providers.openprovider]` 상속이었으나, **grok 0.2.101 라이브 검증에서 상속된 base_url이 실제 추론 라우팅에 적용되지 않음**을 확인:

- `model_provider = "openprovider"` 상속 모델 → 요청이 기본 `cli-chat-proxy.grok.com`으로 나가 401 (`/tmp/grok-wp3.err`, dummy 키)
...
R2 FAIL (1 blocker): quoted/whitespace TOML 동등 표기([model."opr-mine"], [ model . opr-mine ]) 미인식. → `848cab37`: 3형태 canonicalize + Bun.TOML.parse 검증 테스트.
