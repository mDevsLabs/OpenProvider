...
Date: 2026-07-23. User-selected plan A: `opr start`가 `~/.grok/config.toml`에 관리 블록을 주입, `opr stop`/`eject`/`uninstall`이 제거.
...
- `api_key`: grok 크레덴셜 체인이 non-empty를 요구 (loopback opr는 admission 무시).
- alias 규칙: `opr-` + model id에서 `[^A-Za-z0-9_-]`→`-` (TOML bare-key 안전: 점이 서브테이블을 만들므로 반드시 치환). 충돌 시 `-2` 서픽스.
...
- 사용자가 펜스 밖에 자체 `[model_providers.openprovider]`를 정의한 경우: 주입 스킵 + 경고 (충돌 방지).
...
2. live: 격리 `GROK_HOME` + 수정 체크아웃 서버(:10190)로 start→config 주입 확인→`grok models`에 opr-* 노출→1턴 스모크 exit 0→stop→블록 제거·사용자 블록 보존 확인
