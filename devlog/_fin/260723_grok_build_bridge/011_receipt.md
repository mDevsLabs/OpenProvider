...
2. `bun run test` — **3702 pass / 1 fail**. 실패 1건은 `tests/anthropic-thinking-signature.test.ts` "sanitize strips oprr1..." — **stash 후 미수정 트리에서도 동일 실패** (full-run 한정, 단독 실행 시 pass — 테스트 격리/캐시 간섭으로 보이는 기존 이슈, 본 변경과 무관 확증). 변경 파일 3종 단독 실행 70 pass 0 fail.
3. 런타임 프로비넌스: 임시 스택 `OpenProvider_HOME=/tmp/opr-wp1-home` + `--port 10190`으로 **수정 체크아웃에서 직접 기동** (healthz pid 40465, version 2.7.34-dev). :10100 프로덕션 프록시(이 세션 서빙 중)는 건드리지 않음 — 리뷰어 blocker의 취지(수정 코드 서빙 증명)를 재시작 대신 격리 기동으로 충족.
...
|---|---|---|---|---|
| opr-chat | chat_completions | cursor/grok-4.5 | **0** | opr_WP1_OK |
| opr-native-chat | chat_completions | gpt-5.4-mini | **0** | opr_WP1_OK |
| opr-resp | responses | cursor/grok-4.5 | **0** | opr_WP1_OK |
...
| model | exit | stdout |
|---|---|---|
| opr-chat | 0 | opr_WP1B_OK |
| opr-native-chat | 0 | opr_WP1B_OK |
| opr-resp | 1회차 1(히트비트, 하단), 재시도 3/3 **exit 0** | opr_WP1B_OK |
