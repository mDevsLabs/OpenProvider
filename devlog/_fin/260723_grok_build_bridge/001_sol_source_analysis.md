...
3. **system 메시지는 chat 백엔드에서 정상 role로 전송** (`conversation.rs:1781-1784`) — opr chat 인바운드가 instructions로 폴딩하므로 문제 없음.
...
- `api_backend = "chat_completions"`로 설정한 `opr-chat`(cursor/grok-4.5)조차 **실제 와이어는 Responses 이벤트를 역직렬화하다 실패**했다. grok 0.2.101 하니스가 이 turn을 Responses 클라이언트로 태운 것 (설정과 무관하게 harness 내부 경로가 Responses vocab을 쓰는 표면 존재 — 정확한 트리거는 잔여 조사 항목).
- native `gpt-5.4-mini`가 exit 0인 이유: ChatGPT 상류가 details를 항상 포함하고 opr가 보존 → 역직렬화 통과.
- routed가 죽는 이유: opr `src/bridge.ts` `responsesUsage()`가 cached/reasoning 미보고 upstream에서 `*_tokens_details`를 **생략**.

### 수렴 결론

Grok 쪽 3개 백엔드/신규 표면 무엇을 쓰든, **opr가 usage details를 상시 방출하면 전 매트릭스가 뚫린다.** Sol 권고(chat 기본 브리지)와 로컬 스모크(사실상 Responses 경로 강제)를 합치면 wp1은 두 인코더 모두 커버해야 한다:
...
- `[model_providers.openprovider]` 블록 하나로 base_url/backend를 공유하고 모델별 alias만 얇게 추가하는 권장 config.
- 카탈로그 연동: `GROK_MODELS_BASE_URL=http://127.0.0.1:10100/v1` + 아무 non-empty `XAI_API_KEY` (loopback opr는 admission key 무시). opr `/v1/models`는 이미 `{data:[{id,…}]}` 반환 — `api_backend`/`context_window` per-model 필드를 opr가 실어주면 zero-config에 근접(선택 과제, out of scope 표기).
