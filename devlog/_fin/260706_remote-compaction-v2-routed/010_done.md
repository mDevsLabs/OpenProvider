...
## What shipped
- `src/responses/compaction.ts` (new): `opr1:`+base64 transparent envelope
...
- `src/responses/parser.ts`: `compaction_trigger` input item -> sets
  ...
  input items -> decoded to a SUMMARY_PREFIX user message (opr1) or an opaque
  note (real OpenAI encryption).
- `src/types.ts`: `_compactionRequest?: boolean` on oprParsedRequest.
...
- `src/bridge.ts`: `compaction` option on bridgeToResponsesSSE +
  ...
  `{type:"compaction", encrypted_content:"opr1:"+base64(text)}` output item
  before response.completed. Error path unchanged (response.failed, no item).
- `src/adapters/openai-responses.ts`: forward path scrubs opr1 compaction items
