...
3. What OpenProvider does today: bounded mitigation only — RSS watchdog warnings,
   `opr doctor` memory section, `/api/system/memory`; the leak itself is NOT
...
4. Options (audit round dispositions): (a) wait for a bundled runtime bump;
   (b) OpenProvider_BUN_PATH override with a runtime you trust (unvalidated,
   own-risk label) — MUST state the service re-bake step (audit B1): the env
   is read at artifact-generation time (durableBunRuntime → baked opr_BUN,
   service.ts:358), so service users set the env THEN re-run
   `opr service install`; setting the env alone does nothing for an installed
