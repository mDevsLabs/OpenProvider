...
- NEW eager bounded single-reader relay (bounded client queue + post-cancel
  ...
  reachable via OpenProvider_BUN_PATH or file-backed opt-in.
- Ops relief that works TODAY on 1.3.14: RSS watchdog (warn-only) + authed
  management memory endpoint + `opr doctor` memory/runtime section (service
...
| WP4 | 040 | opr doctor memory/runtime section querying service endpoint with service token; OpenProvider_BUN_PATH guidance | WP3 |
