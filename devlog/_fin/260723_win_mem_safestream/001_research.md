...
- Windows service artifacts bake ONLY OpenProvider_HOME + auth token
  (service.ts:227/364-374/646; winsw.ts:90-94) — shell env NEVER reaches the
  service ⇒ stream-mode flag MUST persist in config.json (oprConfig, types.ts:422;
...
- On bundled 1.3.14 the leak is NOT fixed by this work; it is bounded by
  visibility (watchdog/doctor) + runtime override (OpenProvider_BUN_PATH) +
