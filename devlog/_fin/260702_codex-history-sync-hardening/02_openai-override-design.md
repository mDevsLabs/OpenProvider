...
## openprovider-side dependency map (first-hand, verified this session)
...
| History provider | Forward re-tag becomes obsolete; restore/eject machinery stays as ONE-TIME migration (existing `openprovider`-tagged rows → `openai`) and for downgrade cleanup. `syncResumeHistory` config becomes migration-gating only. | `injectCodexConfig` runs `syncCodexHistoryProvider("openai")` once (migration), never forward again. Backup manifest cleared afterward. |
| Product-name strings | healthz `service:"openprovider"`, oauth ORIGINATOR, GUI labels, `comp_hash` marker — product identity, NOT the provider id. | No change. |
...
1. `buildProviderTableBlock`/`buildProfileFile` emit `openai` table id; root model_provider
   line no longer injected; profile keeps `--profile openprovider` NAME but sets no
   model_provider (or sets "openai").
2. Strip/detect: support BOTH legacy (`openprovider` table + root line) and new
...
4. `opr restore` keeps meaning "remove proxy override" (config-only now — no history
...
- Downgrade (new → old opr): old strip won't recognize the openai-table form → stale
