...
bin/ocx.mjs·package-main.mjs, scripts/opr-restart.sh, src/config.ts:475 entrypoint 체크
...
- 260707 WP2 B: e322f40 — 61 git mv + 결정적 import 코드모드(/tmp/opr-restructure.py), src 최상위 raw ts 68→7. 런타임 경로 산술 수정(service cliEntry, codex/shim, update HERE, cli/help·server package.json), config isOcxStartCommandLine 신구 엔트리포인트 겸용, 테스트/스크립트/구조문서 문자열 경로 갱신.
