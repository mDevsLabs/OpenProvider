...
7. `feat(responses)`: reasoning envelope 스캐폴드 (oprr1, 아직 미배선 — parked WIP 명시)
...
bin/opr.mjs·package-main.mjs, scripts/opr-restart.sh, src/config.ts:475 entrypoint 체크
(구경로 "src/cli.ts"도 계속 인식 — 설치본 opr 프로세스 감지용), 테스트의 join("src","cli.ts")류
문자열 경로, structure/*.md.
...
- 260707 WP1 C: 각 커밋 detached worktree(/tmp/opr-verify) checkout 검증 — tsc 0 + 해당 테스트 그린. 최종 HEAD 9fb90e2: bun test 1555 pass/0 fail, tsc exit 0 (실측).
- 260707 WP2 B: e322f40 — 61 git mv + 결정적 import 코드모드(/tmp/opr-restructure.py), src 최상위 raw ts 68→7. 런타임 경로 산술 수정(service cliEntry, codex/shim, update HERE, cli/help·server package.json), config isoprStartCommandLine 신구 엔트리포인트 겸용, 테스트/스크립트/구조문서 문자열 경로 갱신.
- 260707 WP2 C: gpt-5.5 C-gate 감사 verdict FAIL 1건 — update/job.ts packageLauncherPath가 src/bin/opr.mjs로 해석(테스트는 명시 인자라 미포착) → 3ce0a83 수정. 재검증: 1555/0, tsc 0, cli --version 정상.
...
NEEDS_HUMAN: 실행 중 opr 프로세스는 재시작하지 않음 — 새 코드 반영은 사용자 승인 후 restart 필요.
