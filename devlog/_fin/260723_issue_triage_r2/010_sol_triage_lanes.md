...
Root-cause hypothesis: WHAM changed/varies the semantic placement of quota windows. OpenProvider infers semantics from field position and a narrow plan-name exception instead of the supplied window duration, so a `primary_window` of 2,628,000 seconds is necessarily classified as weekly.
...
Root-cause hypothesis: source helper identity was embedded as a literal when the feature was introduced. Codex 0.145.0 changed the hidden/helper family, but OpenProvider has no source-model set or compatibility predicate to absorb that change.
...
- OpenProvider config does expose explicit Claude tier slots, including Haiku (`src/types.ts:332`) and the GUI/config implementation around `src/types.ts:429` and `gui/src/pages/ClaudeCode.tsx`.
- The reported label is presented by Claude Code's subagent UI. No OpenProvider source was found that generates a generic subagent display label of “Sonnet”.
...
- `src/types.ts:475-480` defines OpenProvider visibility control through catalog inclusion/`visibility` and `/v1/models`; there is no Desktop remote-allowlist control.
...
- The issue's direct `model/list` evidence crosses the final OpenProvider-controlled boundary successfully. A client-side `available_models` allowlist applied afterward is not alterable by this proxy.
...
- OpenProvider's current provider adapters translate inference protocols. Hosting another agent runtime requires lifecycle, tool, session, cancellation, and result-translation architecture beyond a normal adapter.
...
- Current OpenProvider pool state is global (`src/codex/quota.ts:23` is a process-wide account quota map; `gui/src/components/CodexAccountPool.tsx:18` calls the pool global).
