...
- 근거 (전부 본 세션 Tier 2 실측):
  - effort는 id 모양과 무관 (opus해시/claude-opr/bare 3종 모두 `output_config.effort` 탑재).
  - 픽커 필터 `/^(claude|anthropic)/i` (2.1.207 바이너리) → 진짜 bare(`gpt-5.6-sol`)는
    디스커버리에 못 실림. CLI의 "bare"는 가독형 `claude-opr-*` + 수동 `--model` bare 병행.
  - CLI 디스커버리 fetch UA는 `claude-code/<version>` (n_() 바이너리 확인).
- Goal: /v1/models가 표면별 id를 서빙 — CLI(claude-code UA 또는 ?ids=cli)는
  `claude-opr-<provider>--<model>` / `claude-opr-native--<slug>`, Desktop(기타 UA 또는
...
6. docs en/ko/zh — CLI 픽커 id가 `claude-opr-*`로 보임 + `--model gpt-5.6-sol` bare 지원 명시.
