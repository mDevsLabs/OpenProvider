...
- Starting facts: PR #336 targets `dev`, is a cross-repository PR from
  `MathiasHeinke/OpenProvider`, has `maintainerCanModify=true`, is mergeable, and
...
rebase with `gh pr view 336 --repo lidge-jun/OpenProvider --json headRefOid,files,commits`; abort and
...
`eligible(target: Required<oprComboTarget>): boolean` callback and compose it
...
git fetch https://github.com/MathiasHeinke/OpenProvider.git \
...
git push --force-with-lease=<full-ref>:<PR336_OLD_SHA> \
  https://github.com/MathiasHeinke/OpenProvider.git \
