...
- Success condition: a previously installed OpenProvider Codex shim that a completed external
  `@openai/codex` npm update replaced is restored by the next real `opr` command without requiring
...
- `diagnoseCodexShim()` already reports the exact stale condition as `present but not an OpenProvider
  shim` (`src/codex/shim.ts:546-577`).
- The repair path is only called explicitly from `opr codex-shim install`
  (`src/cli/index.ts:659-665`) or after OpenProvider's own updater succeeds
  (`src/update/index.ts:255-267`). An external Codex npm update reaches neither call site.
- The published `opr` entry always reaches `src/cli/index.ts`; the runtime SOT identifies that file
...
- Auto-restore only a **previously installed, validly tracked** shim after its tracked wrapper path
  has been replaced by a complete, stable, non-OpenProvider launcher.
- Run a bounded, read-only probe on ordinary `opr` CLI startup and synchronously execute the existing
...
- Changing `codexAutoStart`; it controls whether an installed shim runs `opr ensure`
...
| --- | --- | --- | --- |
| Every real `opr` CLI startup | The next user command, even when no proxy starts | One state read plus bounded wrapper-prefix/metadata reads on the healthy path; no writes | **Chosen** |
| `opr start` / serve hook | Only commands that start the foreground proxy | Misses `status`, `login`, `doctor`, service-managed/no-start workflows | Reject |
...
The hook is deliberately not placed in `bin/opr.mjs`: the runtime SOT assigns that file bundled-Bun
...
- Environment: `OpenProvider_CODEX_SHIM_AUTO_RESTORE=0` disables auto-restore for that process.
...
`codexAutoStart` (`src/types.ts:526-531`), while public OpenProvider environment controls use the
`OpenProvider_` prefix (`src/config.ts:308`, `src/cli/index.ts:134-135`). Internal shim recursion remains
the separate `opr_SHIM_BYPASS` concern (`src/codex/shim.ts:254-260`, `src/codex/shim.ts:348-350`).
...
6. Immediately before **any** mutation, `installCodexShimInternal` rechecks every captured wrapper
   ...
   all already-mutated siblings in reverse order. Concurrent `opr` or npm activity therefore
...
This is a stability check, not an npm-brand check: OpenProvider does not need to parse npm launcher
contents or enumerate npm processes. A replacement that is still being written is deferred; a stable
non-shim at an OpenProvider-owned tracked path is the exact existing repair trigger.
...
export function codexAutoStartEnabled(config: Pick<oprConfig, "codexAutoStart">): boolean {
...
export function codexShimAutoRestoreEnabled(
  config: Pick<oprConfig, "codexShimAutoRestore">,
...
export function maybeAutoRestoreCodexShim(
  ...
  try {
    ...
    deps.warn(
      ...
      }. Run 'opr codex-shim install' after the Codex update finishes.`,
    );
  }
}
```
...
4. Env `OpenProvider_CODEX_SHIM_AUTO_RESTORE=0` disables config unset/true.
...
Use temp `PATH`/`OpenProvider_HOME`, install through public `installCodexShim()`, and restore environment
...
6. Spawned activation: install a temp shim, replace it with a stable real launcher, run the next
   `opr codex-shim status`, assert exit 0, repair warning on stderr, healthy shim status on stdout,
...
| --- | --- | --- | --- |
| Shim replaced by real Codex binary | Valid installed state; overwrite tracked wrapper(s) with stable non-shim launcher(s) | Next ordinary `opr` command | Wrapper auto-restored before dispatch; new launcher backed up; one warning; requested command exits normally. |
| Shim intact / zero-mutation overhead path | Valid healthy state | Any ordinary `opr` command | Bounded state/prefix/metadata reads only; no config parse, rename, write, warning, subprocess, timer, or network. Typical probe <5 ms. |
| Restore failure | Stable candidate; injected/real filesystem failure | Ordinary `opr` command | Warning includes manual fallback; command's own output and exit status are unchanged. |
| Opt-out set | Config false and, separately, env `=0` | Ordinary `opr` command | Candidate remains untouched; no restore warning; explicit `opr codex-shim install` still works. |
| npm update in progress | Changed wrapper is too young or fingerprint changes | Ordinary `opr` command | Silent defer; no rename/write; later command after stability restores. |
...
- Assets: the real Codex launcher, OpenProvider-owned launcher backup, shim state, nearby OpenProvider config
...
- Attacker/failure capabilities: interrupted or concurrent npm update; local process modifying tracked
  paths/state; symlink swap; concurrent `opr` commands; malformed state; filesystem permission error.
...
- [ ] `OpenProvider_CODEX_SHIM_AUTO_RESTORE=0` and config false prevent automatic writes but do not weaken
