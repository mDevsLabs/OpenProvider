...
"claude.smallFastModelNativeWarning": "When unset, OpenProvider leaves the helper-model overrides unset. Claude Code may use its native Sonnet model, which may incur charges from your native provider.",
```
...
"claude.smallFastModelNativeWarning": "Wenn kein Modell gewählt ist, setzt OpenProvider keine Hilfsmodell-Overrides. Claude Code kann dann sein natives Sonnet-Modell verwenden, wodurch Kosten bei deinem nativen Anbieter entstehen können.",
```
...
"claude.smallFastModelNativeWarning": "未設定の場合、OpenProvider はヘルパーモデルの上書きを設定しません。Claude Code がネイティブの Sonnet モデルを使用し、ネイティブプロバイダーで料金が発生する可能性があります。",
```
...
"claude.smallFastModelNativeWarning": "비워 두면 OpenProvider가 보조 모델 환경 변수를 설정하지 않습니다. Claude Code가 네이티브 Sonnet 모델을 사용할 수 있으며, 네이티브 프로바이더 요금이 발생할 수 있습니다.",
```
...
"claude.smallFastModelNativeWarning": "Если модель не выбрана, OpenProvider не задаёт переопределения вспомогательной модели. Claude Code может использовать нативную модель Sonnet, что может привести к расходам у вашего нативного провайдера.",
```
...
"claude.smallFastModelNativeWarning": "留空时，OpenProvider 不会设置辅助模型覆盖项。Claude Code 可能使用其原生 Sonnet 模型，并可能产生原生提供方费用。",
```
...
when both `tierModels.haiku` and `smallFastModel` are absent, OpenProvider leaves both helper
...
```bash
gh pr diff 337 --repo lidge-jun/OpenProvider --name-only
gh pr diff 337 --repo lidge-jun/OpenProvider \
  | rg '^diff --git|^@@|^[+-]  "[^"]+"'
```
...
1. Start an isolated mock provider whose `/v1/models` returns 401. Start an isolated
   OpenProvider runtime with both `OpenProvider_HOME` and `CODEX_HOME` pointed at temporary
...
3. Start Vite with `OpenProvider_PROXY_TARGET` set to the isolated runtime. Use the native
