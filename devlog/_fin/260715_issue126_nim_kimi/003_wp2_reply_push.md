...
1. Draft English follow-up comment for issue #126 (owner account):
   ...
   - State what we patched on `dev` (parallel_tool_calls:false for nvidia, reasoning_effort suppression for NIM kimi family, upstream error detail surfacing) and that a **release is still pending** — user must wait for the next version (and restart `opr`), and some causes (NIM account enablement, free-tier capacity) are outside the proxy's control.
