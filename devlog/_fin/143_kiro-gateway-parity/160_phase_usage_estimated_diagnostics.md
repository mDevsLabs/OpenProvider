...
- `src/types.ts` `oprUsage` has token fields but no estimated marker.
...
- `src/debug.ts` has opt-in `opr_DEBUG_FRAMES=1` diagnostics for dropped frames,
  but no structured redacted provider breadcrumb helper.
- `src/usage-debug.ts` already writes redacted JSONL records behind
  `@mdevs/openprovider_USAGE_DEBUG=1`.

## Diff plan

### MODIFY `src/types.ts`

- Add optional `estimated?: boolean` to `oprUsage`.
...
- Add `usageForFinalLog(provider: string, usage: oprUsage | undefined)`:
...
- `src/types.ts` now carries internal `oprUsage.estimated`.
...
- `src/debug.ts` keeps provider diagnostics behind `opr_DEBUG_FRAMES=1`, redacts
