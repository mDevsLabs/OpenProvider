...
- Codex tool-runtime reference confirms the expected contract: openprovider must emit valid Responses `function_call` / `custom_tool_call` / `tool_search_call` output items, then Codex core dispatches the local handlers and sends outputs back as next-turn input.
...
5. Inject openprovider into Codex using `127.0.0.1` to avoid localhost IPv6 connection noise against an IPv4-only listener.
...
- Rationale: openprovider binds IPv4 loopback by default; Codex's HTTP client tries `::1` for `localhost` first, producing avoidable reconnect noise.
...
- Live non-destructive checks:
  - restart openprovider with latest branch
