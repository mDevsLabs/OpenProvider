...
After Design B, existing threads tagged `openprovider` would be invisible (codex lists by
 ...
 * openprovider-tagged threads to openai. Reuses restore machinery; recoverable-lock
...
- rows/ejected > 0 → "N openprovider-tagged thread(s) migrated back to openai (one-time)."
...
**MODIFY `tests/codex-history-provider.test.ts`** — migration: seeded openprovider rows
...
**MODIFY `~/.codex/skills/openprovider/SKILL.md` + references** — OUT OF SCOPE for the
