...
- **Goal:** plain `codex` routes through opr WITHOUT changing `model_provider`, so
...
- **HOTL bounds:** write scope = this repo only (src/, tests/, devlog/, README).
  ...
  to ~/.openprovider or ~/.codex live configs. Budget: this session's token budget;
...
3. Spike (opr 2.6.25 + codex 0.142.5, isolated homes): requests hit proxy 200,
...
4. zstd risk: `enable_request_compression` (Stable, default ON) fires when auth is
   ...
   HTTP fallback sends `content-encoding: zstd` body → opr today returns 400
...
| 3 | 030 | History one-time migration on inject (openprovider→openai eject), message surface, docs/SoT sync, full-suite gate | 2 |
