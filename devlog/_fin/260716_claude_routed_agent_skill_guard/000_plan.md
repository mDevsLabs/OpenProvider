...
- Archetype: repair.
- Trigger: an `opr-gpt-5-6-sol` Claude Code subagent invoked the built-in `claude-api` skill, received its very large document bundle, and terminated with `Prompt is too long`.
- Goal: generated `opr-*` agents do not invoke skills configured in `claudeCode.blockedSkills`; proxy-side bundle elision remains the fallback.
...
- Verifier: `bun test --isolate ./tests/claude-agents-inject.test.ts ./tests/claude-inbound.test.ts` proves preventive generation plus fallback elision; `bun run typecheck` proves the shared contract; `opr claude --version` followed by inspection of `~/.claude/agents/opr-gpt-5-6-sol.md` proves launch-time generation; a direct `opr-gpt-5-6-sol` Claude Code run plus transcript inspection proves the guard reaches the agent and suppresses `Skill("claude-api")`.
- Stop condition: default/custom blocked-skill names appear in every generated agent instruction (roster and `opr-self`), an explicit empty list emits no guard, hostile names remain single-line serialized, fallback elision tests stay green, and a live Sol run completes without a blocked Skill tool-use.
...
- C-phase Sol review found one high blocker: the first implementation guarded every `opr-self`, including unclaimed native Claude passthrough. The repaired boundary applies the instruction only when `resolveInboundModel` changes the marker-stripped agent model; raw native Claude self definitions remain untouched, while aliases and `modelMap` claims remain guarded.
...
- MODIFY `tests/claude-agents-inject.test.ts`: assert default, custom, explicit-empty, `opr-self`, and hostile-name behavior through generated files.
...
1. Default activation: `claudeCode.blockedSkills` is unset. Every generated file, including roster entries and `opr-self`, names `claude-api` and says not to invoke it.
...
7. Runtime generation: `opr claude --version` rewrites the owned Sol agent file with the default guard; no interactive Claude process is launched.
8. Runtime preventive activation: a direct `opr-gpt-5-6-sol` Claude Code prompt that names Claude Code completes, its system prompt contains the generated guard, and its transcript contains no blocked `Skill` tool-use or skill document bundle.
9. Native passthrough isolation: `opr-self` with an unclaimed `claude-sonnet-*` picker model has no guard; the same picker id claimed by `modelMap` has the routed guard.
10. Direct-route classification: `opr-self` using direct `provider/model` syntax has the guard even when inbound resolution is identity; raw `claude-*` also has the guard when `nativePassthrough:false`.
...
- Check `opr-self` and roster definitions receive consistent policy.
...
- Plan audit: Sol reviewer required shared normalization, `opr-self` coverage, delimiter-safe serialization, and live activation; amended plan passed on round 3.
...
- Preventive activation: direct Claude Code session `752388f2-f213-43bc-b133-1344adeac744` ran `opr-gpt-5-6-sol`, completed normally, and its transcript contained zero `Skill` tool calls and zero skill bundles.
- Generated artifact: `opr claude --version` rewrote `~/.claude/agents/opr-gpt-5-6-sol.md`; line 15 contains the default `claude-api` guard.
