...
| `devlog/_chase/_model/006_jawcode_import_matrix.md` | 신규 | 로직 후보별 최종 판정·opr owner·검증 gate |
...
5. 이미 opr가 충족하는 동작은 `NOOP`, 제품 경계 밖은 `REJECT`, live proof가 필요한 것은 `RESEARCH`로 남긴다.
...
3. Show jawcode GPT-5.6 pre-policy and post-policy values separately from each opr transport; classify cost ingestion separately because opr has no cost consumer.
