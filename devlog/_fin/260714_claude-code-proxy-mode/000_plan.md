...
Claude Code를 Anthropic 계정 없이 openprovider 프록시만으로 사용 가능하게 하는 authMode 설정 추가.

## Context

- 서버 측 라우팅은 이미 Anthropic 없이 동작 (claude-messages.ts:300 — Responses 변환)
- 병목: opr claude가 ANTHROPIC_AUTH_TOKEN placeholder를 주입하지 않음 (cli/claude.ts:49)
...
- proxy: ANTHROPIC_AUTH_TOKEN=openprovider-proxy placeholder 주입

## Accept Criteria

1. authMode: proxy시 opr claude가 Anthropic 로그인 없이 실행
