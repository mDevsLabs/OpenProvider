...
- `src/server.ts` `/healthz` (≈:2010): add `service: "openprovider"`, `pid: process.pid`,
...
- `src/cli.ts`:
  - New `proxyIdentityAt(port, expectedPid?): Promise<{ pid?: number } | null>` — GET
    `/healthz` (existing hostname logic), require `res.ok` AND `body.service === "openprovider"`
  ...
  - `opr sync` path (`syncModelsToCodex()` no-arg call ≈:430): pass
...
- `src/service.ts` `stopTrackedProxyIfRunning` (≈:504): also `removeRuntimePort(pid)`
  (import from `./config`) on both the stale and stopped paths, matching `opr stop`
  (`cli.ts:256`).
...
- `serviceStatusSummary()`: when state records baked paths and either no longer exists,
  prefix the summary with `installed (STALE baked paths — run 'opr service install')`.
...
- `src/update.ts` `runUpdate()`: before invoking the package manager, if `readPid()` shows
  a tracked proxy, run the full `opr stop` semantics by spawning
...
- `tests/healthz-identity.test.ts` (new): startServer on an ephemeral port → `/healthz`
  body has `service === "openprovider"`, `pid === process.pid`, numeric `port` (follow
...
4. update.ts spawning `[process.argv[1], "stop"]`: verify process.argv[1] is cli.ts under
   the bun runtime for `opr update` (and what it is under the Node launcher path — the
