...
```
opr models add <provider> <modelId> [--display-name <name>] [--context-window <tokens>] [--modalities text,image,audio]
```
...
### 3.3 opr models remove

```
opr models remove <customId|provider/modelId> [--yes]
```
...
### 3.4 opr models list-custom

```
opr models list-custom [--json]
```
...
printUsage() 라인:
```
  opr v2 <status|on|off>      Toggle Codex multi_agent_v2 (multi-agent surface)
```

### 4.3 printUsage() models 라인 갱신

기존: `opr models [--json]         List available models from configured providers`
신규: `opr models <sub>            List models; manage custom models (add|remove|list-custom)`
...
- `opr help models` 출력 확인
- `opr --help` 출력에 v2 + models 갱신 확인
- `opr models add alibaba-token-plan-intl test-model --context-window 200000` → 성공
- `opr models list-custom` → 방금 추가한 항목 표시
- `opr models remove alibaba-token-plan-intl/test-model --yes` → 삭제
