...
+  try { return localStorage.getItem("opr-models-combos-open") === "1"; } catch { return false; }
+});
```

접힘 기본값: 접힘(false). localStorage 키는 프로바이더 접힘 배열(`opr-models-collapsed`)과
분리된 `opr-models-combos-open` 사용 (001 함정 #2).
