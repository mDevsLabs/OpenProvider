...
app-server`), $1 is "-s" and the bypass is missed — opr ensure runs and
respawns the proxy after every `opr stop`.
...
```diff
 if not "%opr_SHIM_BYPASS%"=="" goto run_codex
-${internalCommandChecks}
-if /I "%~1"=="--help" goto run_codex
-...
+:scan_codex_args
+if "%~1"=="" goto ensure_opr
+if "%~1"=="--" goto ensure_opr
+${internalCommandChecks}
+if /I "%~1"=="--help" goto run_codex
...
+:ensure_opr
+"%opr_BUN%" "%opr_CLI%" ensure >nul 2>nul
+:run_codex
```
...
-$skipEnsure = $env:opr_SHIM_BYPASS -or $internalCommands -contains $firstArg ...
+$skipEnsure = [bool]$env:opr_SHIM_BYPASS
+if (-not $skipEnsure) {
