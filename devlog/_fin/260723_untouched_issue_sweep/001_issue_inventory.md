...
- **Root cause:** Unix/Windows/PS shim checks only first CLI arg against
  ...
  $1="-s", so app-server bypass is missed and opr ensure runs.
- **Impact:** CodexBar polls defeat `opr stop` — proxy revives every ~20s.
...
- **Analysis:** Codex CLI checks native auth.json before proxy intercept. Pool
  ...
  CLI changes outside OpenProvider scope.
