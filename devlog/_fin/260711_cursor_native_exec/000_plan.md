...
Codex session runs with `danger-full-access`, routed through openprovider to the
...
- Live traffic confirms the path: `~/.openprovider/usage.jsonl` shows
...
## Work-phase map (goalplan slug: unblock-and-gate-file-read-write-for-openprovider-c)
...
- A/B evidence: direct `POST /v1/responses` with a cursor-routed model asked to
  read `/tmp/opr-native-exec-test.txt` — BEFORE: denial text; AFTER: exact
...
- IN: src/adapters/cursor/** (incl. transport.ts), src/types.ts (now CLEAN —
  ...
  ~/.openprovider/config.json (user-authorized).
...
| 2 | A/B round-trip alone cannot prove readArgs/writeArgs fired (upstream may answer without exec) | ACCEPTED — WP1 evidence now includes provider debug `frame` diagnostic events: `opr debug provider on` → every server frame flows through `debugProviderDiagnostic("cursor","frame",...)` (live-transport.ts:698) and exec frames carry the inner case (live-transport.ts:793) into the debug ring buffer (`/api/debug/logs`). Captured `exec: readArgs` / `exec: writeArgs` events + content/file round-trip = deterministic activation evidence. |
