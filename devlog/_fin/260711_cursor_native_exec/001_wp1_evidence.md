...
Timestamps are 2026-07-11 KST; raw payloads in /tmp/opr-ab-*.json (session-local).
...
- `POST /api/providers` with the full cursor provider object + flag ->
  ...
  /tmp/opr-cursor-provider-before.json). In-memory liveness per reviewer
...
`GET /api/debug/logs` frame events (`[opr:cursor:frame]`):
