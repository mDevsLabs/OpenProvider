...
`opr` CLI는 대부분의 명령에서 사람이 읽기 편한 텍스트만 출력한다. 이모지(✅, ❌, ⚠️, 🛑 등)와 자유 형식 문자열이 혼재되어 있어, 외부 도구나 에이전트가 결과를 파싱하기 어렵다.

**구조화 출력이 있는 명령:**
- `opr status --json` — `CliStatusJson` 타입의 JSON 출력 (schemaVersion 포함)

**구조화 출력이 없는 명령:**
- `opr doctor` — paths, proxy env, WHAM probe 결과를 `console.log`로 직접 출력
- `opr sync` — 모델 카탈로그 동기화 결과를 텍스트로 출력 (내부적으로 `CodexSyncResult` 구조체 존재)
- `opr service status` — 설치 상태를 텍스트 한 줄로 출력
- `opr start/stop/ensure` — 성공/실패를 이모지 + 텍스트로 출력
- `opr init` — 인터랙티브 프롬프트 (구조화 불필요하나 결과 요약은 필요)
- `opr codex-shim status` — 텍스트 한 줄
- `opr login/logout` — 텍스트 한 줄
...
- `opr doctor`는 문제를 발견해도 항상 exit 0으로 종료
...
Codex 에이전트, CI/CD 파이프라인, 모니터링 스크립트 등이 `opr`를 프로그래밍 방식으로 사용할 때:

1. **파싱 불가능한 출력**: 이모지와 자유 텍스트를 정규식으로 파싱하면 버전 업데이트 시 깨진다
2. **상태 판단 불가**: `opr doctor`가 경고를 출력해도 exit 0이므로 문제 감지를 놓침
3. **세부 에러 구분 불가**: exit 1만으로는 "포트 충돌", "권한 부족", "이미 실행 중" 등을 구분할 수 없음
4. **불필요한 출력 노이즈**: 스크립트에서 `opr ensure`를 실행할 때 이모지/텍스트 출력이 로그를 오염시킴
...
| 우선순위 | 명령 | 이유 | 기존 내부 타입 |
|----------|------|------|----------------|
| **P0** | `opr doctor --json` | 환경 진단을 자동화/모니터링에 활용, 가장 복잡한 출력 | `PathRow[]`, `ProxyEnvRow[]`, `WhamProbeResult` |
| **P0** | `opr sync --json` | 카탈로그 동기화 결과를 에이전트가 확인 | `CodexSyncResult` (이미 존재) |
| **P1** | `opr service status --json` | 서비스 관리 자동화 | `serviceStatusSummary()` 확장 필요 |
| **P1** | `opr ensure --json` | CI/에이전트 부트스트랩에서 가장 흔히 호출 | 신규 |
| **P2** | `opr start --json` | 데몬 시작 결과 확인 (포트, PID) | 신규 |
| **P2** | `opr stop --json` | 정리 결과 확인 | 신규 |
| **P3** | `opr codex-shim status --json` | 마이너 — 단순 상태 | 신규 |

`opr init`은 인터랙티브 명령이므로 `--json` 대상에서 제외한다.
...
#### 3.2.1 `opr doctor --json`

```typescript
interface DoctorJson {
  paths: Array<{
    label: string;       // "CODEX_HOME", "OpenProvider_HOME" 등
    path: string;
    exists: boolean;
...
```bash
# 에이전트가 환경 건강 확인
opr doctor --json | jq '.data.overallHealthy'

# WSL 환경 문제 감지
opr doctor --json | jq '.data.paths[] | select(.isDrvfs == true)'

# WHAM 연결 문제 확인
opr doctor --json | jq '.data.wham.classification'
```
...
```bash
# 동기화 후 추가된 모델 수 확인
opr sync --json | jq '.data.added'

# 경고 확인
opr sync --json | jq '.warnings // empty'
```

#### 3.2.3 `opr service status --json`
...
```bash
# 서비스 설치 여부 확인
opr service status --json | jq '.data.installed'

# 서비스 로그 경로 확인
opr service status --json | jq '.data.logPath'
```

#### 3.2.4 `opr ensure --json`
...
```bash
# 프록시 포트 확인
opr ensure --json | jq '.data.port'

# 새로 시작됐는지 확인
opr ensure --json | jq '.data.alreadyRunning'
```

#### 3.2.5 `opr start --json`
...
> 참고: `opr start`는 기본적으로 블로킹 모드이므로, `--json`이 함께 사용되면 시작 완료 후 JSON을 stdout에 출력하고 데몬 모드로 전환하거나, 별도의 `--json --detach` 조합을 요구하는 설계가 필요하다. 가장 실용적인 방식은 `opr ensure --json`을 권장하는 것이다.
...
| 명령 | 현재 | 개선 후 |
|------|------|---------|
| `opr start` (성공) | `0` (블로킹) | `0` |
| `opr start` (이미 실행 중) | `1` | `4` (STATE_CONFLICT) |
| `opr start` (포트 파싱 에러) | `1` | `2` (USAGE_ERROR) |
| `opr stop` (성공) | `0` | `0` |
| `opr stop` (프로세스 없음) | `0` ("No running proxy found") | `0` (멱등성 유지) |
| `opr stop` (stop 실패) | `1` | `1` (GENERAL_ERROR) |
| `opr ensure` (이미 실행 중) | `0` | `0` |
| `opr ensure` (시작 실패) | `1` | `5` (NETWORK_ERROR) |
| `opr ensure` (autostart 비활성) | `0` (텍스트만) | `0` |
| `opr doctor` (정상) | `0` | `0` |
| `opr doctor` (경고 발견) | `0` | `7` (HEALTH_WARNING) |
| `opr sync` (성공) | `0` | `0` |
| `opr sync` (카탈로그 없음) | `0` (경고 텍스트) | `7` (HEALTH_WARNING) |
| `opr uninstall` (부분 실패) | `1` | `6` (PARTIAL_FAILURE) |
| `opr service install` (환경 불일치) | `1` (throw) | `3` (CONFIG_ERROR) |
...
if opr ensure -q; then
  echo "Proxy ready"
fi

# 셸 스크립트: doctor 결과를 종료 코드로만 확인
opr doctor -q || echo "Environment issues detected (exit $?)"

# cron job: 서비스 상태 확인
opr service status -q
```
...
| 명령 | `--quiet` 지원 | 비고 |
|------|---------------|------|
| `opr status` | O | 종료 코드 0=running, 4=not running |
| `opr doctor` | O | 종료 코드 0=healthy, 7=warnings |
| `opr ensure` | O | 종료 코드 0=running |
| `opr sync` | O | 종료 코드 0=success |
| `opr start` | X | 블로킹 명령 — quiet 의미 없음 |
| `opr stop` | O | 종료 코드 0=stopped |
| `opr service *` | O | 종료 코드로 결과 전달 |
| `opr init` | X | 인터랙티브 — quiet 불가 |
...
5. `opr doctor`에 `--json` 추가 — 내부 타입을 그대로 노출
6. `opr sync`에 `--json` 추가 — `CodexSyncResult` 노출
7. `opr doctor`의 종료 코드를 `EXIT_HEALTH_WARNING`으로 변경

**Phase 2 (P1): service + ensure**
1. `opr service status --json` 추가
2. `opr ensure --json` 추가
3. 해당 명령들의 종료 코드 세분화

**Phase 3 (P2): start/stop + quiet**
1. `opr start/stop`에 `--json` 추가
2. `--quiet` 플래그 전체 적용
3. 모든 `process.exit(1)`을 의미별 상수로 교체

### 6.4 기존 `opr status --json`과의 호환

현재 `opr status --json`은 `CliStatusJson`을 직접 출력한다 (엔벨로프 없음):
...
- `opr status --json`은 현재 스키마 유지 (`schemaVersion`으로 이미 버전 관리)
...
- `opr status --json`의 기존 `data`가 엔벨로프의 `data` 필드로 이동
- 장점: 일관성
...
- **종료 코드 변경의 위험**: 현재 `exit(1)`에 의존하는 외부 스크립트가 있을 수 있음. 하지만 `exit(1)`은 여전히 "에러"를 의미하므로 `if opr ...; then` 패턴은 깨지지 않음. 세분화된 코드(2-7)에 의존하는 스크립트는 아직 없으므로 위험 낮음.
- **`opr status --json` 호환**: 선택지 A를 따르면 기존 파서 영향 없음.
...
opr doctor --json > /tmp/opr-diagnostics.json
...
# 모니터링 스크립트에서 건강 체크
if ! opr doctor -q; then
  alert "OpenProvider environment unhealthy (exit $?)"
fi
```
...
| 디버깅 | `opr doctor` 출력 복사-붙여넣기 | JSON 첨부로 정확한 진단 공유 |

---

## 9. 참고: 기존 `CliStatusJson` 구조

현재 `opr status --json`의 출력 타입 (참고용):
