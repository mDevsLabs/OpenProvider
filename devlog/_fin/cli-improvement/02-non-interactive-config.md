...
`opr init`은 `readline` 기반 대화형 프롬프트로만 동작한다(`src/init.ts`의 `createPrompt()`).
Provider를 추가/변경/삭제하거나 config 값을 수정하려면 다음 두 가지 방법뿐이다:

1. `opr init`을 대화형으로 실행 (CI/에이전트/스크립트에서 불가능)
2. `~/.OpenProvider/config.json`을 직접 편집 (스키마 검증 없음, 오타 리스크)

### 구체적 문제점

1. **에이전트 자동화 불가**: Codex, CI 파이프라인, 셋업 스크립트에서 provider를 프로그래밍적으로 관리할 수 없다.
2. **스키마 불투명성**: `configSchema`(`src/config.ts`)와 `oprProviderConfig`(`src/types.ts`)에 zod 기반 검증이 있지만, CLI 사용자에게 이 스키마가 노출되지 않는다. 어떤 필드가 필수인지, 어떤 adapter가 유효한지 알 수 없다.
3. **레지스트리 활용 불가**: `PROVIDER_REGISTRY`(`src/providers/registry.ts`)에 40개 이상의 사전 정의 provider가 있고, `providerConfigSeed()`(`src/providers/derive.ts`)가 완전한 config seed를 생성할 수 있지만, 이 기능을 `opr init`의 대화형 메뉴 이외의 경로로 활용할 방법이 없다.
4. **시크릿 확인 불가**: config를 확인하려면 `cat ~/.OpenProvider/config.json`으로 전체 파일을 열어야 하고, API 키가 평문으로 노출된다.
5. **단일 provider 교체시 전체 덮어쓰기**: `opr init`은 기존 providers를 유지하지 않고 선택한 하나의 provider로 전체 config를 새로 작성한다.

---

## 제안: 비대화형 서브커맨드

### 1. `opr provider` 서브커맨드 그룹
...
#### `opr provider show <name>`

특정 provider의 현재 config를 표시한다. 시크릿은 마스킹.

```
$ opr provider show anthropic
Provider: anthropic
  adapter:      anthropic
...
$ opr config get defaultProvider
openai

$ opr config get providers.openai.adapter
openai-responses

$ opr config get stallTimeoutSec
...
#### `opr config set <key> <value>`

dot-notation으로 특정 config 값을 설정한다.

```bash
$ opr config set port 8080
✅ port = 8080

$ opr config set websockets true
✅ websockets = true

$ opr config set stallTimeoutSec 120
✅ stallTimeoutSec = 120

# 지원하지 않는 키
$ opr config set unknownKey value
...
└── types.ts                # 기존 — oprProviderConfig, oprConfig
```
...
export async function handleProvider(args: string[]): Promise<void> {
  ...
  switch (sub) {
    ...
    default:
      console.error("Usage: opr provider <list|add|remove|show|set-default>");
      process.exit(1);
  }
}
...
export function maskConfigSecrets(config: oprConfig): oprConfig {
...
`opr init`은 대화형 온보딩 경험으로 그대로 유지한다. 새 서브커맨드들은 `init` 이후 개별
provider를 추가/제거/수정하거나, 스크립트/에이전트에서 자동화할 때 사용한다.
`init`이 하는 "Codex config.toml 주입"과 "autostart shim 설치"는 별도 커맨드
(`opr sync`, `opr codex-shim install`)로 이미 존재하므로, `provider add` 이후
사용자가 필요시 개별 호출하면 된다.
...
분리한다. `opr status --json`의 기존 패턴을 따른다.
