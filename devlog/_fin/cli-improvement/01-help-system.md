 ...
 현재 `opr help <cmd>`는 1-2줄의 요약만 출력한다. 예를 들어:
 ...
 **핵심 환경 변수:**
 ...
 | `OpenProvider_HOME` | `config.ts:33`, `service.ts:181,470` | OpenProvider 설정 디렉토리 경로 오버라이드 |
 | `OpenProvider_API_AUTH_TOKEN` | `server.ts:1539`, `service.ts:158,166`, `codex-inject.ts:60`, `login-cli.ts:11` | 비-루프백 바인딩 시 API 인증 토큰 |
 | `OpenProvider_BUN_PATH` | `bun-runtime.ts:23,64` | Bun 런타임 경로 오버라이드 |
 | `CODEX_CLI_PATH` | `codex-catalog.ts:329` | Codex CLI 바이너리 경로 오버라이드 |
 | `opr_SERVICE` | `cli.ts:149,215`, `star-prompt.ts:45`, `update-notify.ts:121` | 서비스 모드 플래그 (내부용) |
 | `opr_DEBUG_FRAMES` | `debug.ts:7` | 프레임 디버깅 활성화 |
 ...
 **프로바이더 관련 환경 변수:**
 ...
 | `OpenProvider_CURSOR_TEST_TOKEN` | `adapters/cursor/live-transport.ts:80` | Cursor 테스트 토큰 |
 | `KIRO_ACCESS_TOKEN` | `oauth/kiro.ts:67` | Kiro 액세스 토큰 |
 | `KIRO_REGION` | `oauth/kiro.ts:87,99` | Kiro API 리전 |
 | `OpenProvider_DEBUG_QUOTA` | `codex-auth-api.ts:314` | 쿼터 디버깅 활성화 |
 | `OpenProvider_USAGE_DEBUG` | `usage-debug.ts:7` | 사용량 디버깅 활성화 |
 ...
 **네트워크/프록시 환경 변수 (config.ts:332-343에서 자동 설정):**
 ...
 type HelpExample = {
   command: string;        // e.g. "opr start --port 8080"
 ...
 - **점진적 공개**: `opr --help`는 간결한 커맨드 목록, `opr help <cmd>`는
   옵션/환경변수/예시를 포함한 상세 도움말을 보여준다.
 - **전역 환경 변수**: 모든 커맨드에 공통인 환경 변수(`CODEX_HOME`, `OpenProvider_HOME` 등)는
   별도의 `globalEnvVars` 배열로 관리하고, `opr help` (인자 없음) 하단에 표시한다.
 ...
 Environment Variables:
   ...
   OpenProvider_HOME          OpenProvider config directory override (default: ~/.OpenProvider)
   OpenProvider_API_AUTH_TOKEN API auth token (required for non-loopback binds)
   OpenProvider_BUN_PATH      Override the Bun runtime binary path
 ...
 Environment Variables:
   OpenProvider_HOME    OpenProvider config directory override
   CODEX_HOME        Codex home directory override
 
 Examples:
   opr status          Human-readable status overview
   opr status --json   Machine-readable JSON output
 
 See also: doctor, start, service
 ```
 
 ### 3.3 `opr --help` (메인 도움말 하단 추가)
 ...
 Environment Variables:
   ...
   OpenProvider_HOME           OpenProvider config directory (default: ~/.OpenProvider)
   OpenProvider_API_AUTH_TOKEN  API auth token for non-loopback binds
   OpenProvider_BUN_PATH       Override the Bun runtime binary path
   CODEX_CLI_PATH           Override the Codex CLI binary path
   opr_DEBUG_FRAMES         Set to "1" to enable frame debugging
 
 Run 'opr help <command>' for detailed help on a specific command.
 ```
 ...
 const globalEnvVars: HelpEnvVar[] = [
   ...
   { name: "OpenProvider_HOME", description: "OpenProvider config directory (default: ~/.OpenProvider)" },
   { name: "OpenProvider_API_AUTH_TOKEN", description: "API auth token for non-loopback binds" },
   { name: "OpenProvider_BUN_PATH", description: "Override the Bun runtime binary path" },
   { name: "CODEX_CLI_PATH", description: "Override the Codex CLI binary path" },
   { name: "opr_DEBUG_FRAMES", description: 'Set to "1" to enable frame debugging' },
 ];
 ```
 ...
 const helpEntries: Record<string, HelpEntry> = {
   start: {
     ...
     envVars: [
       { name: "OpenProvider_API_AUTH_TOKEN", description: "Required for non-loopback hostname binds" },
     ],
     examples: [
       { command: "opr start", description: "Start on default port (10100)" },
       { command: "opr start --port 8080", description: "Start on custom port" },
     ],
   ...
   status: {
     ...
     examples: [
       { command: "opr status", description: "Human-readable status overview" },
       { command: "opr status --json", description: "Machine-readable JSON output" },
     ],
 ...
 export function printUsage(): void {
   const header = `OpenProvider (opr) -- Universal provider proxy for Codex\n\nUsage:`;
 ...
 3. **P2 (선택)**: `opr help --all`로 전체 환경 변수 / 옵션 일람 출력
