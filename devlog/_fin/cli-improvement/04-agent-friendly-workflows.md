...
> **작성일**: 2025-07-05
> **범위**: `opr` CLI의 비대화형(non-interactive) 워크플로우 확장
> **상태**: 제안(proposal)
...
`opr`는 **사람이 터미널에서 직접 사용하는 것**을 전제로 설계되어 있다. 이 설계는 인간 사용자에게는 자연스럽지만, AI 에이전트(Codex agent)나 CI/CD 파이프라인, 셸 스크립트 같은 **자동화 컨텍스트에서는 마찰이 된다**.

구체적인 문제점:

| 현재 명령 | 문제 | 에이전트 관점의 영향 |
|-----------|------|---------------------|
| `opr init` | `readline` 기반 대화형 프롬프트만 지원. 프로그래밍적으로 설정 불가 | 에이전트가 프록시를 처음 설정할 수 없음 |
| `opr start` | 포그라운드에서 `await new Promise(() => {})` 로 영구 블로킹 | 에이전트가 시작 후 다음 작업으로 넘어갈 수 없음 |
| `opr start` (이미 실행 중) | `process.exit(1)` -- 에러로 종료 | 멱등성 없음. 스크립트에서 "시작되어 있으면 OK" 패턴 불가 |
| `opr ensure` | 백그라운드 시작 + 대기 포함, 그러나 출력이 사람용 이모지 텍스트 | 종료 코드는 활용 가능하지만, 포트 번호 등을 파싱하기 어려움 |
| `opr status` | `--json` 플래그 지원 (좋음!), 하지만 단순 health-check 전용 명령 부재 | 상태 확인에 JSON 파싱이 필요함 |
...
```bash
# 에이전트가 스스로의 OpenProvider 프록시를 부트스트랩하는 이상적인 흐름
opr init --provider xai --non-interactive      # 없음
opr start --background                          # 없음
opr wait-ready --timeout 10000                  # 없음
opr health                                       # 없음 (opr status --json 으로 대체 가능하지만 과잉)
opr models --json                                # 없음
opr test-provider xai                            # 없음
```
...
### 2.1 `opr start --background` / `opr start -d`
...
`handleStart`에 `block` 매개변수가 이미 존재하지만, CLI에서는 사용되지 않는다. `opr ensure`가 내부적으로 `spawn` + `detach`를 하지만, `ensure`는 auto-start가 비활성화된 경우 아무것도 하지 않는다 (`codexAutoStartEnabled` 체크).

#### 제안 동작

```
opr start --background    # detach + 포트 반환 + exit 0
opr start -d              # 축약형
opr start -d --port 8080  # 포트 지정과 조합
```
...
**`opr ensure`와의 관계**: `ensure`는 "auto-start가 켜져 있을 때만" 동작하는 반면, `start -d`는 명시적 시작 명령이므로 auto-start 설정과 무관하게 동작한다. `ensure`는 codex-shim에서 호출되는 간접 실행 경로이고, `start -d`는 에이전트나 스크립트의 직접 실행 경로이다.

---

### 2.2 `opr wait-ready [--timeout <ms>]`
...
```bash
opr start -d && opr wait-ready --timeout 15000
# 또는
opr ensure && opr wait-ready
```
...
```
opr wait-ready                     # 기본 30초 타임아웃
opr wait-ready --timeout 5000      # 5초 타임아웃
opr wait-ready --json              # {"ready":true,"port":10100,"pid":12345,"elapsed_ms":1234}
```
...
**목적**: 프록시의 현재 health 상태를 단순하게 확인한다. `opr status`의 경량 버전.

#### `opr status`와의 차이

`opr status`는 서비스 상태, codex-shim, OAuth 로그인, 플러그인 등 전체 진단을 수행한다 (`collectStatus()`). 에이전트가 "프록시 살아있나?" 하나만 확인하고 싶을 때 불필요한 오버헤드다.
...
**목적**: `opr start`가 이미 실행 중인 프록시를 에러로 처리하지 않도록 한다.
...
// cli.ts:102-106
if (live) {
  console.error(`Proxy already running (...). Use 'opr stop' first.`);
  process.exit(1);  // <-- 에러!
}
```
...
```
opr start             # 이미 실행 중 -> 성공 (exit 0)
opr start --strict    # 이미 실행 중 -> 에러 (exit 1, 기존 동작)
```

이 변경은 `opr ensure`의 멱등 패턴과 일관성을 갖추게 된다. `ensure`는 이미 실행 중이면 `Proxy running on port N`을 출력하고 exit 0 한다.

---

### 2.5 `opr models` / `opr models --json`
...
$ opr test-provider badconfig
badconfig: failed
   Base URL: https://invalid.example.com/v1
   Error: ECONNREFUSED

$ opr test-provider xai --json
{
  "provider": "xai",
  "ok": true,
...
async function handleTestProvider() {
  const name = args[1];
  if (!name) {
    console.error("Usage: opr test-provider <provider-name>");
    process.exit(1);
  }
...
if opr health; then
  ...
  # 3. 준비될 때까지 대기
  opr wait-ready --timeout 15000
fi

# 4. 사용 가능한 모델 확인
MODELS=$(opr models --json)
echo "사용 가능 모델: $(echo $MODELS | jq length)개"

# 5. 프로바이더 동작 확인
opr test-provider xai --json | jq '.ok'
```
...
# GitHub Actions 예시
steps:
  - name: Start OpenProvider proxy
    run: |
      opr start -d --port 10100
      opr wait-ready --timeout 30000

  - name: Verify providers
    run: |
      opr test-provider xai
      opr models --json > available-models.json
...
// 멱등 시작
execSync("opr start -d", { stdio: "inherit" });

// 준비 대기
const readyOutput = execSync("opr wait-ready --json", { encoding: "utf8" });
const { port, pid } = JSON.parse(readyOutput);

// 모델 목록
const models = JSON.parse(
  execSync("opr models --json", { encoding: "utf8" }),
);
...
setInterval(() => {
  try {
    execSync("opr health", { stdio: "ignore" });
  } catch {
    console.error("프록시 다운 감지, 재시작...");
    execSync("opr start -d");
  }
}, 60_000);
```
...
case "$1" in
  ...
  *) opr ensure >/dev/null 2>&1 || true ;;
esac
exec codex.OpenProvider-real "$@"
```
...
| | `opr service install` | `opr start -d` |
|---|---|---|
| 부팅 시 자동 시작 | O | X |
| 크래시 자동 재시작 | O | X |
| OS 서비스 매니저 필요 | O | X |
...
1. **`opr init --non-interactive`**: config 파일을 직접 작성하는 것으로 충분한가, 아니면 `--provider`, `--api-key` 등의 CLI 플래그가 필요한가? 현재는 `config.json`을 직접 편집하는 것이 해결책이지만 문서화가 부족하다.

2. **`--json` 글로벌 플래그**: 모든 명령에 `--json` 을 개별 추가하는 것보다, `opr_OUTPUT=json` 환경 변수나 글로벌 `--output json` 플래그가 나은가?

3. **`opr start -d`의 구현**: 현재 프로세스에서 서버를 시작한 뒤 반환할 것인가 (현재 `handleStart`의 `block: false` 경로), 아니면 `ensure`처럼 자식 프로세스를 spawn할 것인가? 후자가 더 깔끔하지만, 업데이트 프롬프트 건너뛰기 등의 차이가 생긴다.
