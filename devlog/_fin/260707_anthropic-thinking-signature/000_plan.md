...
## Design (oprr1 envelope through Codex's encrypted_content slot)
...
- bridge.ts (SSE + buildResponseJSON): hold the latest signature/redacted payloads for
  the OPEN reasoning item; on close, attach `encrypted_content: "oprr1:" + base64(JSON
...
- parser.ts reasoning branch: decode oprr1 envelope -> thinking part carries REAL
  signature (and redacted blocks list); non-oprr1 encrypted_content (native OpenAI) is
  ignored as today.
- types.ts oprThinkingContent: + redacted?: string[] (raw redacted_thinking data).
...
- Non-goals: google thoughtSignature path (already handled separately);
  ...
  content for native — must ALSO not leak oprr1 envelopes to native backend: extend
  that scrub to drop oprr1 encrypted_content).
...
passthrough scrub of oprr1.

## Closed (260707)

Shipped in commit 4a59dbc: parseStream signature_delta/redacted_thinking capture
(block-scoped), bridge oprr1 envelope (SSE+JSON, hideThinkingSummary envelope-only
item with txt), schema encrypted_content, parser decode, anthropic replay
(redacted before thinking), passthrough oprr1 scrub. All three gpt-5.5 audit
