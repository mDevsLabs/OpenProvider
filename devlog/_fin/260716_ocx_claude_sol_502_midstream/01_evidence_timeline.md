# `opr claude` sol 서브에이전트 사망 — 증거 타임라인 (2026-07-16)

조사 대상: 최근 `opr claude`로 실행한 Claude Code 세션에서 sol 빌더 서브에이전트가
...
- 죽은 에이전트: `sol:build-registry` (agentType `opr-gpt-5-6-sol`, model `claude-opr-native--gpt-5.6-sol[1m]`)
...
### 프록시 레저 대조 (`~/.openprovider/usage.jsonl`, UTC)
...
## 사건 2 — openprovider 세션 mid-stream 종료 (부 사건)

- 세션: `~/.claude/projects/-Users-jun-Developer-new-700-projects-openprovider/69edf4a1-759c-4a02-a1e6-692d4e086a01.jsonl` line 475
...
1. opr가 502를 재시도하는가, 아니면 그대로 클라이언트에 치명 에러로 전달하는가? (sol 코드 분석)
2. `Connection closed mid-response`는 opr 생성인가 Claude Code 클라이언트 합성인가? (sol 코드 분석)
