...
Integration test that boots a real server with `startServer(0)`, hand-writes a small `usage.jsonl` fixture under a temp `OpenProvider_HOME`, and asserts:
...
1. Launch the proxy on a free port with a known `OpenProvider_HOME` and `OpenProvider_API_AUTH_TOKEN`.
...
- Auth token defaults differ between dev and tests. Mitigation: set `OpenProvider_API_AUTH_TOKEN` and `OpenProvider_HOME` explicitly per test.
