...
Parent plan: /Users/jun/Developer/new/700_projects/openprovider/devlog/370_cli-human-friendly/40_gptpro_hardening_followup_plan.md

GPT Pro finding: `opr start` can choose a transient fallback port when the configured/preferred port is busy. Existing persistence rules intentionally avoid writing that random fallback to config, but `opr status` and `opr gui` still read config port, so they can point at the wrong URL while the proxy is actually running.

## Objective

Record the actual runtime listen port separately from config and teach status/gui to use it only when the PID file still points at the current running openprovider start process. Do not make transient fallback ports permanent config defaults.
...
If preferred port `10100` is busy and openprovider starts on `58195`, status/gui still use `10100` unless config persistence rules changed the config. Those rules intentionally do not persist random fallback ports.
...
In `/Users/jun/Developer/new/700_projects/openprovider/src/config.ts` add:
...
In `/Users/jun/Developer/new/700_projects/openprovider/src/cli.ts`:
...
In `/Users/jun/Developer/new/700_projects/openprovider/src/cli-status.ts`:
...
In `/Users/jun/Developer/new/700_projects/openprovider/src/cli.ts` `gui` case:
...
In `/Users/jun/Developer/new/700_projects/openprovider/tests/config.test.ts`:
...
In `/Users/jun/Developer/new/700_projects/openprovider/tests/cli-status-json.test.ts`:

- add a pure helper seam in `cli-status.ts`, for example `selectListenTarget(config, pid, runtimePortState)`, so status port selection can be tested without faking an `opr start` process command line;
...
- keep CLI smoke for normal `status --json`, but do not require `bun test` to fake `readPid()` with the current test process PID because `readPid()` correctly rejects non-`opr start` processes.

## Acceptance Criteria

- `opr start` records the selected listen port separately from config.
- Cleanup paths remove only metadata for the expected PID.
- `opr status --json` prefers runtime metadata only when tied to the current PID.
- stale/mismatched runtime metadata is ignored.
- `opr gui` opens the runtime port when metadata is current.
