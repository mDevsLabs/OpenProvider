...
grok models
grok -p "Reply with exactly: opr_SMOKE_OK" -m <model> \
...
| `anthropic/claude-opus-4-8` | `opr-opus` | responses | **`opr_SMOKE_OK`** | 1 | Content success; Grok then errors on stream deserialize: missing `output_tokens_details` |
| `cursor/grok-4.5` | `opr-cursor` | responses | **`opr_SMOKE_OK`** | 1 | Content success; Grok errors missing `input_tokens_details` on completed event |
| `opencode-go/glm-5.2` | `opr-glm` | responses | **`opr_SMOKE_OK`** | 1 | Content success; same `output_tokens_details` deserialize error |
