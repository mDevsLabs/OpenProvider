# 000 — Grok Build에서 OpenProvider 모델 사용 가능 여부

## Loop-spec
- Loop archetype: spec-satisfaction research / documentation
- Trigger: xai-org/grok-build open-sourced; user asks whether **Grok Build can use OpenProvider-routed models** (already-installed Grok Build users)
- Goal: source-backed possible/partial/impossible verdict + upgraded `~/Developer/codex` analysis docs
- Non-goals: production OpenProvider inbound chat-completions implementation unless reopened; push
...
## Corrected question
**Grok Build TUI/CLI 사용자가 OpenProvider가 이미 라우팅 중인 모델들(gpt-5.6-*, anthropic/*, cursor/*, …)을 고를 수 있는가?**

NOT: OpenProvider가 Grok 계정 토큰을 쓰느냐 (그건 이미 구현된 반대 방향 Path).
...
### OpenProvider data plane (live :10100)
...
| wp1 | codex analysis upgrade (custom-model + opr inbound) | 010 |
| wp2 | feasibility verdict Grok→OpenProvider models | 020/030 |

## Working hypothesis
**PARTIAL → practically POSSIBLE via config** using Grok `api_backend = "responses"` pointed at `http://127.0.0.1:10100/v1` with OpenProvider model ids. Default Grok `chat_completions` backend is a mismatch unless OpenProvider adds that inbound route.
