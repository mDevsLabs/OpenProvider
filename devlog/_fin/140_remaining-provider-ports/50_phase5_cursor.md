...
Stream cursor agent models (text + thinking) through openprovider over cursor's HTTP/2 Connect+protobuf
transport. MVP = login + single/multi-turn **text**; exec bridge **stubbed**.

## Why it is last + isolated

Cursor shares **zero** code with Phases 10–40. It breaks all three openprovider transport assumptions
...
- `02_openprovider-fit.md` — the **optional `runTurn()` adapter hook** escape hatch (additive; isolates HTTP/2+protobuf to `src/adapters/cursor/`).
...
| exec bridge | **optional Phase 5** — separate approval (makes openprovider a partial Cursor CLI host) |
