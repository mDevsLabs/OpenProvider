...
| 287 | Confirmed openprovider capability/GUI-contract bug: darwin-only injection (system-env.ts:210) but Linux gets an actionable toggle + success response | Option (b): disable Auto-connect on non-Darwin with localized "macOS-only; use opr claude" explanation (reporter accepts this). Linux injection tracked separately | 4–8 h |
| 290 | Needs-repro, distinct from #92 (fails BEFORE child creation). OpenProvider normalizes zero-arg-byte calls to `{}` but no inspected path erases non-empty args | Keep open, `needs-info`: request four-boundary capture; recommend `opr v2 mode v1` meanwhile | capture 3–5 h |
...
- src/cli/v2.ts:98: `opr v2 mode v1|default|v2` exists (#288 workaround real).
...
| # | Action | Evidence |
|---|---|---|
| 280 | reply + close | https://github.com/lidge-jun/openprovider/issues/280#issuecomment-5053490275 |
| 291 | reply + close | https://github.com/lidge-jun/openprovider/issues/291#issuecomment-5053490282 |
| 288 | reply + close | https://github.com/lidge-jun/openprovider/issues/288#issuecomment-5053493021 |
| 297 | reply + close (not-repro; reopen on version evidence) | https://github.com/lidge-jun/openprovider/issues/297#issuecomment-5053493027 |

Extra evidence captured for #297 before closing: this machine's synced
`/Users/jun/.codex/openprovider-catalog.json` retains `max`/`ultra` on
