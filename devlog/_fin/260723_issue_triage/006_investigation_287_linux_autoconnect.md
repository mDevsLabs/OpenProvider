...
ANTHROPIC_BASE_URL, ~/.openprovider/claude-env.sh is absent, and only opr claude
...
>   lines.push(conditional("ANTHROPIC_AUTH_TOKEN", "openprovider-proxy"));
> ~~~
>
...
OpenProvider already has a clean Linux/systemd seam for its proxy service, but that
...
toggle. It would require elevated writes, broaden the effect beyond one OpenProvider
...
The current contract is user-wins and tracks only keys OpenProvider injected, which
...
state to SettingToggle, show a localized “macOS-only; use opr claude” explanation,
...
**OpenProvider bug (capability/GUI contract), not upstream Claude Code and not a
...
localized “macOS-only; use opr claude” explanation and a server-derived capability
