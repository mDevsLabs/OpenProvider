...
- openprovider must write config/profile/catalog/cache files under that resolved Codex home.
...
| Commit | Area | Summary |
| --- | --- | --- |
| `8ca68d0` | config injection | Removed legacy `[profiles.openprovider]` injection path. |
...
- openprovider runs as a service without the same shell environment;
- Windows Task Scheduler starts openprovider through a non-interactive path.
...
After Phase 80, `opr sync`, `opr start`, and service mode should agree on the same Codex home and catalog path across macOS, Linux, and Windows. The fallback profile is now the modern file-based profile:
