# WP5 — #389 model-switch visibility + opr sync relationship (040)
...
catalog (`provider-fetch.ts:424-444`, `sync.ts:455-496`); `opr sync` loads config
...
Conclusion: no semantic conflict; `opr sync` reproduces the same visibility; a
