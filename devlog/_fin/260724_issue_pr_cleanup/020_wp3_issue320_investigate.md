...
window when running `codex`, before the request reaches the OpenProvider proxy.
Secondary: after a Codex CLI npm update replaced the binary, the autostart shim
disappeared until `opr codex-shim install` was re-run.
...
- `codexAccountMode="pool"` is evaluated only AFTER a request reaches
  ...
  `src/codex/auth-context.ts:100`). At that point OpenProvider picks + refreshes
...
- OpenProvider's own expiry check (`src/codex/main-account.ts:40`) only excludes
...
- Install renames npm launcher to `*.OpenProvider-real`, writes wrapper at the
...
- Already self-repairs: every ordinary `opr` command auto-restores before
  dispatch (`src/cli/index.ts:64`; restore logic `shim.ts:1043,1092`;
  `opr status` eligible `src/cli/codex-shim-autorestore.ts:18`; test
  `tests/codex-shim-autorestore.test.ts:127`). A direct first `codex` call after
  replacement can't self-trigger (wrapper no longer runs); next `opr` repairs it.
  `opr service install` avoids launcher dependency for proxy startup.
...
> Thanks — this is two separate layers. In Pool mode, OpenProvider selects and
...
> ordinary `opr` command (manual fallback: `opr codex-shim install`). For
> restart-safe operation independent of the launcher, use `opr service install`.
