...
- Can openprovider gain speed by setting `supports_websockets = true` even though most upstreams are
  Chat Completions?
...
/tmp/openprovider-codex-src/codex-rs/core/src/tools/spec_plan.rs:291
/tmp/openprovider-codex-src/codex-rs/core/src/tools/spec_plan.rs:397
/tmp/openprovider-codex-src/codex-rs/core/src/tools/spec_plan.rs:618
```
...
```text
/tmp/openprovider-codex-src/codex-rs/model-provider-info/src/lib.rs:134
/tmp/openprovider-codex-src/codex-rs/model-provider-info/src/lib.rs:324
/tmp/openprovider-codex-src/codex-rs/codex-api/src/endpoint/responses_websocket.rs:334
```

## Current openprovider Behavior

openprovider currently injects a Responses provider, but it does not inject
`supports_websockets = true`.

That means Codex should stay on the HTTP/SSE Responses path for openprovider.

For routed non-OpenAI providers, openprovider usually translates Codex Responses requests into
...
could change request semantics in ways openprovider did not deliberately implement:
...
If openprovider sets `supports_websockets = true` without implementing the protocol, Codex may attempt
...
so a Codex-to-openprovider websocket would only change the first hop while openprovider still waits on the
...
2. providers that themselves expose a low-latency websocket protocol openprovider can bridge without
...
- openprovider's routed upstreams are mostly HTTP/SSE Chat Completions or HTTP/SSE-compatible streams;
- enabling websocket only between Codex and openprovider cannot make a provider/model websocket-capable
...
- setting `supports_websockets = true` would advertise a native capability openprovider does not
...
1. Do not set `supports_websockets = true` for openprovider routed providers.
2. Strip or explicitly set `use_responses_lite = false` for routed non-OpenAI models unless
   openprovider implements the related request-shape differences.
