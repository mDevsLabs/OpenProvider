...
Phase 100 should convert this research into explicit catalog/runtime metadata policy for openprovider.
...
/Users/jun/Developer/new/700_projects/openprovider/src/web-search/synthetic-tool.ts
```

Tasks:

1. Decide whether routed models should expose deferred `tool_search`.
2. Set `web_search_tool_type` according to openprovider sidecar capability, not template inheritance.
...
4. Synthesize Codex-relevant headers only where openprovider has truthful data.
...
openprovider routed models mostly end in upstream HTTP/SSE Chat Completions or HTTP/SSE-compatible
streams. Adding websocket only between Codex and openprovider does not make those upstream providers
...
2. Should routed models inherit Codex feature-default multi-agent behavior by deleting
   `multi_agent_version`, or should openprovider force a specific version?
...
1. generate a small openprovider-owned metadata projection from jawcode;
