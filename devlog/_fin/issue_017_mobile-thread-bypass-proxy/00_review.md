# Issue #17 — Mobile-created Codex threads may bypass local OpenProvider proxy
...
- **Status:** Likely a Codex mobile/remote-thread platform limitation, not an
  OpenProvider proxy defect. Pending reporter repro on latest version. Documentation
...
OpenProvider proxy. When a thread is created on mobile, it appears to be
...
- The auto-start shim (`opr codex-shim install`) ensures the proxy is running
...
The OpenProvider-side touchpoints that bound this behavior:
