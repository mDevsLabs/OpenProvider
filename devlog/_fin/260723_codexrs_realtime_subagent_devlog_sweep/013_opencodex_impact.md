# 013 — OpenProvider impact matrix (live src)

Measured against OpenProvider tree at worktime on 2026-07-23. No product patches in this goal.

| Area | Live OpenProvider anchor | Upstream contract | Status | Severity | Action |
| --- | --- | --- | --- | --- | --- |
...
| expose_spawn_agent_model_overrides / hide metadata | not managed by OpenProvider config helpers | defaults expose overrides=true, hide metadata=true | Gap/Observability | Med | Guidance text must not claim fields always hidden |
...
## Practical OpenProvider risks from this pull
...
3. **wait_agent disable:** rare user config can remove wait tool; OpenProvider guidance currently assumes wait exists.
