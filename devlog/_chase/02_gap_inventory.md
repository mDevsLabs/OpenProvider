...
> 상태: `⬜` 미착수 · `🟡` 부분/설계 · `✅` openprovider 선행/포팅 완료 · `—` 범위 밖
> 기록: `10_jawcode.md` · `20_cli-proxy-api.md` · `30_litellm.md`

## 요약 (축별 앞섬/뒤처짐)

| 축 | openprovider가 **앞서거나 유일** | openprovider가 **뒤처지거나 약함** |
|---|---|---|
...
| 갭 | 종류 | 상태 | upstream 근거 | openprovider 위치 |
|---|---|---|---|---|
...
| google-gemini-cli OAuth | G1 | — | jawcode `google-gemini-cli.ts` | 레거시. openprovider migration 대상에서 제외 |
...
| 멀티모달(embedding/tts/image/rerank) | G3 | — | litellm mode 분포 | openprovider 범위 밖 (chat/responses 프록시) |
...
- G1 deepinfra: `jawcode/packages/ai/src/providers/` (HEAD `27311f6` 신규 파일) → openprovider registry 한 줄 + 모델 시드
