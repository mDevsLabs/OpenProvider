...
**chase** = openprovider가 세 upstream 대비 **아직 안 따라온** 영역 + **무엇을 어떻게 따라갈지**의 참조 방안.
jawcode `struct_har/chase`와 동형 — 단, openprovider는 이미 jawcode 포팅을 끝냈고 별도
...
openprovider에는 jawcode의 `gjc_origin/`·`jwc_patched/` 밴드 트리에 해당하는 게 `src/`(코드 정본)와
`structure/`(아키텍처 SoT)다. 그래서 chase는 그 위에 **갭 추적**만 얹는다.

## 갭 4종 (openprovider 맞춤)

| 종류 | 설명 | 주 upstream |
|---|---|---|
| **G1 jawcode drift** | jawcode가 새 provider/모델을 추가해 openprovider가 뒤처짐 | jawcode |
...
| **G4 openprovider-only** | openprovider가 앞서거나 유일한 영역 (kiro·codex WS·구독 IDE 백엔드) | — |

## 상태 어휘

`⬜` 미착수 · `🟡` 부분/설계 · `✅` openprovider 선행 또는 포팅 완료 · `—` 범위 밖
...
카드가 "⬜ gap"이라 쓸 때, 먼저 openprovider에 이미 있는지 grep으로 확인한다.
...
# vertex OAuth (G2) — openprovider는 key/ADC만
