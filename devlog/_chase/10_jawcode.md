...
- 역할: openprovider 어댑터의 **직접 포팅 출처** (1차 SOT). provider 레이어가 여기 산다.

## chase MOC

> 상태: 🟡 운영 중 · **의미**: jawcode 대비 openprovider **뒤처짐(G1)**
> 상태 어휘: `⬜` 미착수 · `🟡` 부분 · `✅` openprovider 선행/완료 · `—` 범위 밖

### Reviewed through

| jawcode | openprovider |
|---|---|
| `27311f6` (code, 2026-07-01) | registry 48 provider / adapter 6종 (worktree) |

### Recent jawcode deltas

| 항목 | jawcode fact | openprovider 처리 |
|---|---|---|
...
| google-gemini-cli | `google-gemini-cli.ts` OAuth | — 레거시. openprovider migration 대상에서 제외 |
| kiro | `kiro.ts` + 4 모듈 | ✅ 9 모듈로 분화 (openprovider 선행) |
| models.json | 3758 모델 엔트리 | 🟡 카탈로그 주기 동기화 필요 |

## 규모 대조 (HEAD 기준)

| 지표 | jawcode | openprovider |
|---|---|---|
...
openai-responses / anthropic-messages / kiro 등 `api` 필드로 분기)이고, openprovider는
...
jawcode에만 있고 openprovider registry에 같은 id로 없는 것:
`alibaba-coding-plan`(openprovider는 `alibaba`로 보유), `amazon-bedrock`,
`cursor`, `deepinfra`, `google-gemini-cli`(legacy/out-of-scope), `minimax-code`, `minimax-code-cn`,
`openai-codex`, `opencode`.

openprovider에만 있는 것: `kimi`, `lm-studio`, `neuralwatt`, `openai-apikey`,
...
|---|---|---|
| `cursor` | **미포팅** | jawcode `cursor.ts`는 ~2.6k줄 agent 프로토콜. openprovider에 adapter 없음. 가장 큰 단일 gap |
| `deepinfra` | **미포팅** | HEAD에서 막 추가된 신규(10.062). tokenizer family 라우팅 포함 |
| `amazon-bedrock` | 부분 | openprovider는 `kiro` adapter로 Bedrock-on-Kiro만. 직접 Bedrock(sigv4) 경로는 없음 |
| `google-gemini-cli` | — | 레거시 경로. openprovider는 ai-studio/vertex/cloud-code-assist만 유지하고 gemini-CLI OAuth는 migration하지 않음 |

## API 변환 1:1 대조 포인트

| 영역 | jawcode | openprovider |
|---|---|---|
...
openprovider는 Kiro를 jawcode보다 더 잘게 쪼갰고(9 모듈 vs 5), Antigravity는 thought-signature
replay를 별도 모듈로 뺐다. 즉 **포팅 후 openprovider가 더 하드닝된 영역**(kiro/antigravity)과
...
3. `models.json` delta — 3758 엔트리 중 openprovider 카탈로그에 없는 신규 모델·컨텍스트 윈도우 주기 동기화.
