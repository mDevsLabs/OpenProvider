...
> 상태: 🟡 운영 중 · **의미**: cca 대비 openprovider wire/auth 깊이 **부분 뒤처짐(G2)**
> 상태 어휘: `⬜` 미착수 · `🟡` 부분 · `✅` openprovider 선행/완료 · `—` 범위 밖

### Reviewed through

| cli-proxy-api | openprovider |
|---|---|
| `00114be` (2026-06-29) | oauth 6 provider / `ws-bridge.ts` (worktree) |

### Recent cca deltas

| 항목 | cca fact | openprovider 처리 |
|---|---|---|
...
| kiro | (없음) | ✅ 풀세트 (openprovider 선행) |

## 규모 (HEAD 기준, non-test SLOC)

| 영역 | CCA SLOC | openprovider 대응 |
|---|---|---|
...
openprovider oauth: `anthropic, chatgpt(=codex), google-antigravity, kimi, kiro, xai`.

| provider | CCA | openprovider | 차이 |
|---|---|---|---|
...
| kiro | (없음) | `kiro` adapter 풀세트 | openprovider 우위 |

## 핵심 wire quirk 교차검증

| quirk | CCA 위치 | openprovider 위치 | 상태 |
|---|---|---|---|
| Antigravity thoughtSignature replay | `antigravity_reasoning_replay.go` (667줄) | `google-antigravity-replay.ts` (136줄) | openprovider가 훨씬 가벼움 — **replay 캐시 깊이 차이 점검 필요** |
...
| Antigravity translators | gemini/claude/openai 3종 | `google` adapter 단일 | openprovider는 단일 adapter로 흡수 |

## 따라잡을 우선순위

1. **Antigravity reasoning replay 깊이** — CCA 667줄 vs openprovider 136줄. clear-on-invalid,
   캐시 키, 멀티턴 시그니처 보존 로직을 라인 대조해 누락 확인.
2. **vertex OAuth** — openprovider는 key/ADC만. CCA `vertex_credentials.go` 흐름을 참조해
   서비스계정 OAuth 경로 보강 여부 결정.
3. **xai reasoning replay / WS** — CCA에 전용 모듈 존재. openprovider xai는 forward-only인지 확인.
