...
압도적 다수다. 정확한 수치는 계속 늘어나므로 재조회한다:
    ...
    gh pr list --repo lidge-jun/OpenProvider --state merged --limit 60 \
...
확인됐다(마지막 매치 우선). 그러나 실제 브랜치 보호는 없다:
    ...
    gh api repos/lidge-jun/OpenProvider/branches/dev/protection    # HTTP 404
    gh api repos/lidge-jun/OpenProvider/branches/main/protection   # HTTP 404
...
5. **절차 요건 2번 충족 증거**: 이 변경을 담은 PR에 @Ingwannu의 실제 승인이
   달린다. 검증:
       ...
       HEAD=$(gh pr view <N> --repo lidge-jun/OpenProvider --json headRefOid --jq .headRefOid)
       gh api repos/lidge-jun/OpenProvider/pulls/<N>/reviews --paginate \
         | jq --arg head "$HEAD" \
...
문서 변경만으로 GitHub 권한이 생기지 않는다. 현재 실측:
    ...
    gh api repos/lidge-jun/OpenProvider/collaborators --jq '.[] | "\(.login) triage=\(.permissions.triage) push=\(.permissions.push) maintain=\(.permissions.maintain) admin=\(.permissions.admin)"'
