# WP9 — `opr account`의 인증 코드를 셸 인자에서 걷어내기
...
- `src/cli/account-auth.ts` USAGE: `opr account code <provider> <redirect-or-code>`
...
1. `<redirect-or-code>` 위치 인자와 `--code`를 선택으로 만들고, 없으면
   **stdin에서 읽는다.** 파이프(`echo … | opr account code x`)와 TTY 프롬프트
   양쪽을 지원한다.
...
먹는다. `opr account code openai --flow flow-123`은 `--flow`를 코드로 삼고
...
- **`login`의 기본 동작은 건드리지 않는다.** 감사 지적대로 `--code` 없는
  `opr account login`은 지금처럼 브라우저 플로우를 열고 폴링한다. stdin을
...
- **`opr account` 총괄 USAGE도 같이 고친다** (`src/cli/account.ts:28`).
...
**F3 — 이미 끝난 스트림은 이벤트를 다시 내지 않는다.** `a | b | opr account
