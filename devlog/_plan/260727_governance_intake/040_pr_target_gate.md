...
1. **actor 검증.** 라벨을 승인 신호로 쓸 거면 누가 붙였는지 확인해야 한다.
   ...
   메인테이너 리뷰 승인(`pulls.listReviews`) 사용.
       ...
       gh api repos/lidge-jun/OpenProvider/collaborators --jq '.[] | "\(.login) triage=\(.permissions.triage) maintain=\(.permissions.maintain)"'
...
이 저장소에는 **브랜치 보호도 ruleset도 없다**:
    ...
    gh api repos/lidge-jun/OpenProvider/branches/dev/protection   # 404
    gh api repos/lidge-jun/OpenProvider/branches/main/protection  # 404
    gh api repos/lidge-jun/OpenProvider/rulesets                  # (없음)
