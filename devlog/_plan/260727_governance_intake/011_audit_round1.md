...
**수용.** 이게 계획 전체를 무효로 만드는 지적이었다.
    ...
    gh repo view lidge-jun/OpenProvider --json defaultBranchRef
    # main
...
**수용.** `+1192`로 적었으나 실측은 `+1193`이다.
    ...
    gh api repos/lidge-jun/OpenProvider/pulls/518 --jq '{additions,deletions,changed_files,commits}'
...
**부분 수용.** "마지막 매치가 우선"은 맞다고 확인됐다. 다만:
    ...
    gh api repos/lidge-jun/OpenProvider/branches/dev/protection
    # HTTP 404 (Branch not protected)
    gh api repos/lidge-jun/OpenProvider/branches/main/protection
    # HTTP 404
