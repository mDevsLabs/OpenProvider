# Grok retry loop: OpenProvider entries pinned to a dead port (4179)
...
`Retrying (attempt N/15)`, and the OpenProvider service log records nothing —
...
(`opr status` -> PID 5389, health ok, port 10100).

## Live config state

`~/.grok/config.toml` (355 lines) holds 31 `[model.opr-*]` tables in two
generations:

| generation | count | base_url | example alias |
| --- | --- | --- | --- |
| stale | 8 | `http://127.0.0.1:4179/v1` | `opr-gpt-5-6` |
| current | 23 | `http://127.0.0.1:10100/v1` | `opr-gpt-5-6-sol` |

`[models] default = "opr-gpt-5-6"` — the *stale* alias. Grok resolves the
...
1. **The fence is gone.** `grep "OpenProvider managed block"` finds neither
   marker. Grok rewrote the file itself: our writer emits
   `extra_headers = { "x-OpenProvider-grok" = "1" }` inline, but the live file
...
2. **Alias-suffix drift.** With the fence gone, `userModelAliases` reserved
   ...
   `opr-gpt-5-6-sol-3` sits alongside `opr-gpt-5-6-sol`. The orphan sweep
...
position: an `api_key = "OpenProvider-loopback"` entry whose loopback `base_url`
port differs from the live proxy port is unambiguously ours and unambiguously
dead.

## Plan

1. `src/grok/inject.ts` — extend the orphan sweep so an OpenProvider-owned entry
...
4. Live repair: restart via `opr service` and prove
...
- A Grok turn reaches the proxy (request visible in the OpenProvider log).
