...
`[models] default = "opr-gpt-5-6"` named a stale one, so every turn hit a
refused connection. Grok retried 15 times entirely on its own side, which is
why the OpenProvider log stayed empty and the TUI still showed the correct 372K —
...
1. `grokFenceEndpointDrift()` in `src/grok/status.ts` compares the fence
   ...
   emit, or agreement).
2. `opr status` reports the mismatch and names the repair command. This is the
...
OpenProvider and Grok are independent read-modify-rename writers over the same
file with no shared lock. Concurrent writes can race. The ownership sweep makes
the result recoverable on the next sync, and `opr status` now reports the
