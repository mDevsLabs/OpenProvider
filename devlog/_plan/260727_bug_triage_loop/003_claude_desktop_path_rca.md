...
제보자(NewLand-Ju, Claude Desktop 1.24012.9)는 `opr claude desktop apply`가
...
```bash
OpenProvider_CLAUDE_DESKTOP_CONFIG_DIR="$HOME/Library/Application Support/Claude/configLibrary" opr claude desktop apply
```
...
**2. 진짜 결함 (a): 환경 분기 미구현.** opr는 `GE()`의 세 분기 중 마지막 하나만
...
opr는 계속 `-3p` 경로에 썼다. 증상 설명과 정확히 일치한다.
...
```ts
const entry = Array.isArray(meta.entries) ? meta.entries.find(e => e?.name === "OpenProvider") : undefined;
```
...
```json
{
  ...
  "entries": [
    ...
    { "id": "1252f550-...", "name": "OpenProvider" }
  ]
}
```

사용자가 Desktop UI에서 "Default"로 전환하면 `appliedId`가 `d1444bab`로 바뀐다.
그 순간 opr의 상태 API는 여전히 `entries`에서 `OpenProvider`를 찾아내 "적용됨"이라고
보고하지만, Desktop은 OpenProvider 설정을 읽지 않는다. 조용한 거짓 보고다.
...
`configLibrary`를 포함한다. Windows에서 마이그레이션이 진행 중인 창에서는 opr가 Desktop이
