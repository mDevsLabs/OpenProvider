...
| 심볼 | 역할 |
|------|------|
| `findOpenProviderOrphans` (174) | 관리 블록 밖 OpenProvider 소유 엔트리 탐지 |
...
>
> `src/grok/inject.ts` now detects OpenProvider-owned entries outside the managed block
> (`findOpenProviderOrphans`), removes them (`removeOrphanTables`), and re-points
...
>
> Please re-run `opr sync` and confirm the duplicate `-2` aliases are gone and the
...
> profile (Desktop reads only that one, so an unapplied OpenProvider entry used to be
> reported as applied).
