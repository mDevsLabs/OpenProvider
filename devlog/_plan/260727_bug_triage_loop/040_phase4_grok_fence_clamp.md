...
- `src/grok/inject.ts` — `findOpenProviderOrphans`의 span 계산
...
`findOpenProviderOrphans`는 orphan의 본문 끝을 다음 **테이블 헤더**로 잡는다:
...
### `src/grok/inject.ts` — `findOpenProviderOrphans`
...
| 변형 | 결과 테이블 | 판정 |
|------|-------------|------|
| 현재 코드 | `[model.opr-gpt-5-6-sol]` | 정상 |
| plan040 초안(한정 없음) | `[model.opr-gpt-5-6-sol-2]` + `[model.opr-gpt-5-6-sol.extra_headers]` | **회귀** |
| 한정 추가 | `[model.opr-gpt-5-6-sol]` | 정상 |
...
test("a below-fence orphan whose alias collides does not pin default to -2", () => {
  // 단언: default === "opr-gpt-5-6-sol" (not "...-2")
});
```
