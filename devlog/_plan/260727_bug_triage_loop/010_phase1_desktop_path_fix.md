...
export function resolveConfigLibraryDir(inputs: DesktopPathInputs): string {
  const override = inputs.env.OpenProvider_CLAUDE_DESKTOP_CONFIG_DIR?.trim();
...
`OpenProvider_CLAUDE_DESKTOP_CONFIG_DIR` 오버라이드는 최우선을 유지한다. 기존 테스트
...
```ts
          const meta = JSON.parse(readFile(metaPath, "utf8"));
          const entry = Array.isArray(meta.entries) ? meta.entries.find((e: { name?: string }) => e?.name === "OpenProvider") : undefined;
          // Desktop reads ONLY the profile named by appliedId. An OpenProvider entry that
...
| `appliedId` 있음 + OpenProvider 엔트리 없음 | **`false`** | Desktop이 우리를 안 읽는 것이 확정 |
| `appliedId` ≠ OpenProvider 엔트리 id | `false` | 다른 프로필이 적용됨 |
| `appliedId` = OpenProvider 엔트리 id | `true` | 우리 프로필이 적용됨 |
...
describe("Claude Desktop configLibrary 경로 해석", () => {
  ...
  test("OpenProvider_CLAUDE_DESKTOP_CONFIG_DIR가 다른 모든 분기를 이긴다", () => {
    // 활성화: OpenProvider_... 와 CLAUDE_USER_DATA_DIR 동시 설정
    // 관측: 오버라이드 값이 그대로 반환
  });
...
test("appliedId가 다른 프로필을 가리키면 activeProfile이 false로 보고된다", async () => {
  // _meta.json에 OpenProvider 엔트리를 두되 appliedId는 Default의 id로 설정
  // GET /api/claude-desktop/status → activeProfile === false
});

test("appliedId가 OpenProvider 엔트리를 가리키면 activeProfile이 true다", async () => {
...
| c-appliedid | `appliedId` ≠ OpenProvider 엔트리 id | 상태 응답 `activeProfile === false` |
