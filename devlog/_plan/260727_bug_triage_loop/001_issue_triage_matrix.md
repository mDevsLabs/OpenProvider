...
- **UPSTREAM**: OpenProvider 코드로 해결 불가. 추적만.
...
| 417 | 한국어 음성 트랜스크립트 U+FFFD | bug, upstream-tracking | **UPSTREAM** | 제목이 이미 "not an opr relay bug"로 확정 |
...
| 386 | macOS 메뉴바 컴패니언 패키징 | enhancement | **ROADMAP** | 별도 워크트리 `OpenProvider-macos-app`에서 진행 중 |
...
`upstream-tracking` 4건도 배제한다. 정의상 OpenProvider 코드 변경으로 해결되지 않는다.
