...
let package = Package(
    name: "OpenProviderMenuBar",
    platforms: [.macOS(.v13)],
    products: [
        .executable(name: "OpenProviderMenuBar", targets: ["MenuBarApp"]),
    ],
    targets: [
...
<dict>
  ...
  <key>CFBundleExecutable</key>             <string>OpenProviderMenuBar</string>
  <key>CFBundleIdentifier</key>             <string>com.OpenProvider.menubar</string>
  <key>CFBundleInfoDictionaryVersion</key>  <string>6.0</string>
  <key>CFBundleName</key>                   <string>OpenProvider</string>
  <key>CFBundleDisplayName</key>            <string>OpenProvider</string>
  <key>CFBundlePackageType</key>            <string>APPL</string>
  <key>CFBundleIconFile</key>               <string>OpenProvider</string>
  <key>CFBundleShortVersionString</key>     <string>0.0.0</string>
  <key>CFBundleVersion</key>                <string>0.0.0</string>
  <key>LSUIElement</key>                    <true/>
  <key>LSMinimumSystemVersion</key>         <string>13.0</string>
  <key>NSHumanReadableCopyright</key>       <string>MIT — OpenProvider contributors</string>
</dict>
</plist>
```
...
- `CFBundleExecutable` must equal the binary name the builder copies into
  `Contents/MacOS/` — `OpenProviderMenuBar`.
- `CFBundlePackageType` must be `APPL` for the bundle to be treated as an application.
- `CFBundleIconFile` is `OpenProvider` (no extension), matching the `OpenProvider.icns` the
...
public enum ProxyDiscovery {
    ...
    public static func configDirectory(environment: [String: String] = ProcessInfo.processInfo.environment,
        ...
        if let override = environment["OpenProvider_HOME"], !override.isEmpty {
            return URL(fileURLWithPath: (override as NSString).expandingTildeInPath)
        }
        return home.appendingPathComponent(".OpenProvider")
    }
...
public actor ProxyClient {
    ...
    private func get<T: Decodable>(_ path: String) async throws -> T {
        ...
        if let key = apiKey { request.setValue(key, forHTTPHeaderField: "x-OpenProvider-api-key") }
        do {
...
`com.OpenProvider.menubar.apikey`. The key is never written to `UserDefaults`, never
...
file falls back · out-of-range port (`0`, `70000`) falls back · `OpenProvider_HOME` honoured
