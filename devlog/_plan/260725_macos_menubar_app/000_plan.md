...
**Branch:** `feat/macos-app` (dedicated worktree `<worktree>/OpenProvider-macos-app`, based on `origin/dev` @ `dbed8c15`)
...
Ship one maintainer-owned macOS menu bar companion for OpenProvider, replacing the two
...
React), and use different transports to reach the proxy (`opr` CLI subprocess vs HTTP
...
2. Proxy discovery honours `~/.OpenProvider/runtime-port.json` and falls back to 10100.
