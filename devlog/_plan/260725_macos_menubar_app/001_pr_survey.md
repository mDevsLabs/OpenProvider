...
Package.swift            swift-tools-version 5.9, .macOS(.v12)
  OpenProviderMenuBarCore   oprClient, oprLocator, StatusModels     (library, tested)
  OpenProviderMenuBar       main.swift, MenuText, StatusBarIcon      (executable)
  Tests                  OpenProviderMenuBarCoreTests
```

**Transport: `opr` CLI subprocess.** `oprClient.fetchStatus` locates the `opr`
executable via `oprLocator`, runs `opr status --json`, then brace-slices the stdout
(`output.firstIndex(of: "{")` … `lastIndex(of: "}")`) and decodes it. Write actions run
through `commandPlan(for:status:)`, which emits further `opr` argument vectors.
...
- `scripts/build-macos-app.sh` — assembles `OpenProvider.app` by hand: `Contents/MacOS`,
...
`~/.OpenProvider/runtime-port.json`; `api.rs` proxies WebView `invoke("api_request")` calls
...
`OpenProvider Menubar.app` and a `.dmg`. **But `.github/` is untouched** — no CI job, no
...
gh api repos/genglintong/OpenProvider/contents/menubar/src-tauri?ref=049ef2ac
...
| Transport | `opr` CLI subprocess | HTTP management API |
...
2. **HTTP beats CLI subprocess for a polling UI.** Spawning `opr` every refresh cycle
...
stop the proxy but can never start it. PR #387's CLI transport *could* run `opr start`.
