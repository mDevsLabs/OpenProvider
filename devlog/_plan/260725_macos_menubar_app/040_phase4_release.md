...
mkdir -p "$output_root"
app_bundle="$output_root/OpenProvider.app"

swift_args=(--package-path "$package_dir" -c "$configuration" --product OpenProviderMenuBar)
...
staging_root="$(mktemp -d "$output_root/.OpenProvider-build.XXXXXX")"
staged_app="$staging_root/OpenProvider.app"
iconset="$staging_root/OpenProvider.iconset"
trap 'rm -rf "$staging_root"' EXIT

mkdir -p "$staged_app/Contents/MacOS" "$staged_app/Contents/Resources"
cp "$bin_dir/OpenProviderMenuBar" "$staged_app/Contents/MacOS/OpenProviderMenuBar"
...
iconutil -c icns "$iconset" -o "$staged_app/Contents/Resources/OpenProvider.icns"
...
unzip -Z1 "$archive_path" | grep -Fqx 'OpenProvider.app/Contents/MacOS/OpenProviderMenuBar'
shasum -a 256 "$archive_name" > "$checksum_name"
```

Output: `OpenProvider-<version>-macos-universal.zip` + `.sha256`.
...
archive got rejected with "does not contain the OpenProvider executable". It is a race, not
...
bash scripts/build-macos-app.sh
  -> dist/macos/OpenProvider.app (version 2.7.35), arm64
  -> Info.plist: CFBundleExecutable=OpenProviderMenuBar, CFBundlePackageType=APPL,
     CFBundleIconFile=OpenProvider, LSUIElement=true, NSAllowsLocalNetworking=true
...
UNIVERSAL=0 bash scripts/package-macos-release.sh
  -> OpenProvider-2.7.35-macos-arm64.zip (813 KB) + .sha256
  -> shasum -a 256 -c: OK
...
1. `bun run build:macos` produces a launchable `dist/macos/OpenProvider.app`.
