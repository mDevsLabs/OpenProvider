...
start it again (`opr start`, or `opr service start` when a service is installed) as
...
public enum ActionOutcome: Equatable, Sendable {
    ...
    /// closing port, so the user must run `opr restore` too.
...
public func stop(startCommand: String) async -> ActionOutcome {
    ...
    return .failed(sawIndeterminate
        ? "The proxy accepted the stop, but its state could not be confirmed. Check with `opr status`."
...
and OpenProvider will not restart on its own." — not a generic "Are you sure?".

## Security rules

- Write requests carry the key in `x-OpenProvider-api-key`, read from the Keychain lazily
...
- **No shell execution at all.** The app never spawns `opr` or any other process; it only
...
- The app never writes to `~/.OpenProvider/config.json` directly; all mutation goes through
