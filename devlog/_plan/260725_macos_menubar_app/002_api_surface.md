...
where the config dir defaults to `~/.OpenProvider` (overridable by `OpenProvider_HOME`).
...
1. `OpenProvider_HOME` if set, else `~/.OpenProvider`.
...
- `x-OpenProvider-api-key: <key>`
- `authorization: Bearer <key>`
- `x-api-key: <key>`

validated against `OpenProvider_API_AUTH_TOKEN` or `config.apiKeys[].key` with
`timingSafeEqual`.

App behaviour: attempt unauthenticated first. On `401`, read the token from the macOS
Keychain and retry with `x-OpenProvider-api-key`. Never log the token, never write it to
...
return jsonResponse(restore.success
  ...
  : { success: false, message: "Proxy stopping, but native Codex restore failed: … Run `opr restore`." });
```
...
`opr restore`; the accompanying `message` is a server-formatted string and should not be
surfaced verbatim.
