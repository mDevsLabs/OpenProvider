...
the first launch shows *"OpenProvider.app cannot be opened because the developer cannot be
...
States plainly what changed and why: the transport moved from `opr status --json`
...
`opr start`, and HTTP cannot. The maintainer app ships **Stop proxy** rather than pretend
to restart.
