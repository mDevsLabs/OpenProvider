...
(`swift run --package-path app OpenProviderMenuBar`), which registers a menu bar item and
...
- The `unreachable` action is **not** a button that starts anything. It displays the
  command to run (`opr start`, or `opr service start` when a service is installed) as
...
9. `swift run --package-path app OpenProviderMenuBar` shows the menu bar item and popover.
