...
`opr_QUEUE_543` 캡처를 하겠다"고 되물었는데, 그 스위치는 이미 있다:
`src/lib/debug-settings.ts:7`의 `opr debug claude on|off|status|reset` 및
`opr_CLAUDE_DEBUG=1`. 한국어·중국어 문서에도 기재돼 있다. 답변만 하면 된다.
