...
| 461 | cli: `opr opencode` 런처 | enhancement | draft | **CONFLICTING** | — | +868/-0, 6f | **표면 확장 여부** |
...
`opr opencode`는 `opr claude`, native Codex injection에 이은 **새 런처 표면**이다.
+868/-0, 순수 추가.

정확히 말하면 "세 번째"는 **런처 명령** 기준이고(native injection, `opr claude`,
`opr opencode`), 우리가 지탱하는 **클라이언트 표면**은 그보다 많다: Codex
