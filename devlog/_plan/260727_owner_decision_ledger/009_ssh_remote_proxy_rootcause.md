...
**제출됨: [#570](https://github.com/lidge-jun/OpenProvider/issues/570)** — 이 문서의
...
- `tests/server-auth.test.ts:640-660` — 비루프백 바인드 + `x-OpenProvider-api-key` → 200
...
```
c29ee783e (06-27) "harden OpenProvider release and runtime paths"
...
- `opr status`/`doctor`는 원격을 모른다. PID가 안 맞으면 **로컬** `127.0.0.1:<port>`를
