...
`gui/src/pages/ApiKeys.tsx:474-497`은 `/v1/messages`를 lines 474-484에서 무조건 렌더링하고 lines 485-497에서 조건부로 다시 렌더링한다. disabled mode에서는 403 endpoint를 광고하고 enabled mode에서는 중복한다. lines 474-484를 지우면 된다. `gui/src/pages/ApiKeys.tsx:226-255`의 test button은 `x-OpenProvider-api-key`를 보내지 않아 non-loopback에서 401이 난다. 사용자 동작이 바뀌었지만 `docs-site/` 갱신도 없다.
