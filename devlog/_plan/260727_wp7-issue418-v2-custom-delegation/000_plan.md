...
https://github.com/lidge-jun/OpenProvider/issues/418

The reporter reproduced custom-parent → custom-child V2 delegation failure on
OpenProvider `2.7.39` / Codex CLI `0.145.0`. They later supplied a control trace
...
- Trigger: bug issue with partial reporter evidence but no same-run failing
  boundary trace.
- Goal: either identify a local OpenProvider fix or leave the issue with exact
...
- Existing maintainer request:
  https://github.com/lidge-jun/OpenProvider/issues/418#issuecomment-5069836945
...
- Reporter acknowledged the limitation and committed to repeating after the
  usage limit clears:
  https://github.com/lidge-jun/OpenProvider/issues/418#issuecomment-5070272410.
- Collaborator cross-link keeps #418 separate from #92 and confirms it remains
  pending the three-boundary `spawn_agent` capture:
  https://github.com/lidge-jun/OpenProvider/issues/418#issuecomment-5085535548.
...
OpenProvider has known handling for unreadable encrypted V2 child tasks (#92) and
...
- OpenProvider adapter damaging the outgoing `spawn_agent` schema;
- provider/model emitting empty `{}` arguments for `spawn_agent`;
- OpenProvider bridge losing non-empty provider arguments;
