...
const loopbackConfig = { hostname: "127.0.0.1" } as oprConfig;
...
OUT: `opr status`/`doctor`의 원격 미인식, GUI 스니펫 하드코딩 — 둘 다 009가
부수 확인으로 기록한 별개 항목.
...
> `import type { oprConfig } from "../src/types"` — `src/config.ts`는
> `oprConfig`를 재export하지 않는다(리뷰 지적, `tsc` 실측 TS2459). 저장소의
