---
title: How It Works
description: The full OpenProvider request lifecycle — parse, route, adapt, bridge, and stream.
---

import { Steps } from '@astrojs/starlight/components';

Codex speaks the OpenAI **Responses API**. OpenProvider accepts `POST /v1/responses` over HTTP with
...
When the selected provider is the ChatGPT/Codex passthrough, OpenProvider can choose a stored pool
...
- **New sessions can rebalance.** For a new thread, OpenProvider compares known quota usage across 5h,
...
3. **Authenticate** — for an `oauth` provider, OpenProvider swaps in a fresh, auto-refreshed access
...
4. **Vision sidecar (optional)** — if the routed model is listed in `provider.noVisionModels` and the
   request carries an image, OpenProvider describes each image with the configured ChatGPT vision
...
5. **Passthrough fast path** — if the adapter is a Responses passthrough (`openai-responses` or
   `azure-openai`), OpenProvider keeps the Responses body, applies targeted routing and compatibility
...
6. **Web-search sidecar (optional)** — if Codex enabled hosted `web_search` but the routed model is
   non-OpenAI, OpenProvider exposes a synthetic `web_search` function tool and runs the model in a small
...
8. **Adapt** — otherwise the chosen adapter's `buildRequest()` produces the upstream HTTP request
   (URL, headers, body) in the provider's native format, and OpenProvider `fetch`es it.
...
Codex hard-codes the Responses API. By translating at the protocol boundary, OpenProvider works with the
