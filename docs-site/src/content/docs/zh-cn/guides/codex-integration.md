---
title: Codex 集成
description: OpenProvider 如何将自身注入 Codex、同步模型目录、驱动 subagent 选择器，并干净地恢复。
---

OpenProvider 通过编辑 Codex 读取的两样东西，让 Codex 经由 proxy 路由：它的配置（`$CODEX_HOME/config.toml`，默认 `~/.codex/config.toml`）和它的模型目录。每一次编辑都是幂等且可逆的。
...
ChatGPT bearer 认证。由于注入的 `base_url` 指向 OpenProvider，proxy 会把这些调用中继到
OpenAI 上游：
...
当 OpenProvider 管理路由时，两种模式都会把 `$CODEX_HOME/OpenProvider.config.toml` 写成参考/回退配置。
...
Codex CLI、TUI、App 和 SDK 都读取同一个 Codex home。OpenProvider 会从 `CODEX_HOME` 解析该目录，
...
在 WSL 中，如果未设置 `CODEX_HOME`，且 Linux 侧 `~/.codex/config.toml` 不存在，OpenProvider 还会检查
...
Codex 保持一致。OpenProvider 也提供 `/v1/responses` WebSocket。专用提供商仅在
...
重映射。第一次同步时，它还会把旧版 OpenProvider 改过标识的线程迁回 `openai`。non-loopback 的专用
提供商模式会在运行期间把历史记录映射到 `OpenProvider`，退出时再恢复已备份的元数据。若希望完全不修改
...
的错误（或 Claude Code 出现类似的连接失败），说明 OpenProvider 代理没有在运行：
...
向 Codex 账号池添加 ChatGPT 账号时，OpenProvider 会在保存前向 Codex Responses 后端发送一个小型
