...
不同路由模型对托管 **Web Search** 和原生**图像输入**的支持并不相同。OpenProvider 通过两个
...
当 Codex 为非透传的路由模型请求托管 `web_search` 时，OpenProvider 会：

1. **移除**托管的 `web_search` 工具，改为向路由模型提供一个合成的
   `web_search(query)` function 工具。原托管工具的选项会保留并用于 sidecar 调用。
2. 让路由模型在一个小型 **agentic 循环**中运行。模型调用 `web_search` 时，OpenProvider 使用所选
...
路由模型的每次迭代都会向上游请求 `stream: true`，但 OpenProvider 会在决定搜索还是返回最终答案前，
...
当路由模型列在其 provider 的 `noVisionModels` 中，并且请求包含图像时，OpenProvider 会在主调用
