---
title: CLI 参考
description: 所有 opr 命令与参数。
---

OpenProvider 的命令行工具是 `opr`。运行 `opr help`（或 `--help` / `-h`）可查看顶层用法。
对帮助表中注册的命令，可运行 `opr help <command>` 查看命令专属帮助。帮助和版本命令均为只读，
...
`opr stop` 会先停止服务，以免它重新拉起代理。Web 仪表盘的 **Stop** 按钮
...
Codex pool 使用 `opr account refresh openai [--json]`。它会强制刷新账号 quota，并显示可用的
...
如果已完成的外部 Codex 更新覆盖了已安装的 shim，下一条普通 `opr` 命令会在执行前备份已稳定的
新启动器并恢复 shim。仍在变化的启动器不会被改动，而会稍后重试。修复失败只会警告，不会让请求的
命令失败；手动备用命令为 `opr codex-shim install`。若要关闭自动恢复，请将
...
:::tip[Service 与 Shim]
常驻代理请使用 `opr service`（推荐）。需要无 daemon 的轻量按需启动时，请使用
`opr codex-shim`；只有运行 `codex` 时才会启动代理。
:::
...
`opr help`、`opr --help`、`opr -h` —— 打印顶层用法和示例。

`opr help <command>`、`opr <command> --help`、`opr <command> -h` —— 打印
...
`opr --version`、`opr -v`、`opr version` —— 打印一行适合脚本读取的版本信息并退出。
