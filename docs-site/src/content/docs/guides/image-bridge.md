...
- An `xai` provider entry with an **API key**. The bridge pins fulfillment to the registry xAI
  ...
  calls. OAuth / `opr login xai` alone does **not** arm the bridge (the Grok CLI OAuth transport is
...
Image Bridge options live under `images` in `~/.OpenProvider/config.json`. Bridging is
...
Generated images are written to `~/.OpenProvider/artifacts/`. To prevent unbounded disk
...
1. When a Responses request lists `image_generation` in `tools`, OpenProvider detects it
...
3. When the model invokes that tool, OpenProvider intercepts the call and sends the prompt to xAI's
   image generation API.
4. Generated images are saved to `~/.OpenProvider/artifacts/` and the **local file path** is returned
