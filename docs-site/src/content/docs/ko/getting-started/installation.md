---
title: 설치
description: OpenProvider(opr) 프록시와 사전 요구 사항을 설치하고, 정상 실행되는지 확인합니다.
---

OpenProvider를 설치하면 같은 실행 파일을 가리키는 `opr`와 `OpenProvider` 명령이 함께 제공됩니다.
...
| 요구 사항 | 이유 |
| --- | --- |
| **[Node](https://nodejs.org) ≥ 18** | `opr`는 Bun 런타임에서 실행되지만, 런타임이 `npm install` 시 자동으로 번들되므로 Bun을 직접 설치할 필요가 **없습니다**. |
| **[OpenAI Codex](https://openai.com/codex)**(CLI, App, 또는 SDK) | OpenProvider가 앞단에 위치하는 클라이언트입니다. OpenProvider는 `$CODEX_HOME/config.toml`(기본값 `~/.codex/config.toml`)에 기록합니다. |
...
아닙니다. 아직 정식 배포되지 않은 OpenProvider 빌드를 시험할 때만 preview 채널을 사용하세요:
...
OpenProvider 자체를 직접 수정하며 작업하려면:
...
OpenProvider 상태 파일은 `$OpenProvider_HOME`(기본값 `~/.OpenProvider`) 아래에, Codex 연동 파일은
`$CODEX_HOME`(기본값 `~/.codex`) 아래에 저장됩니다.

| 경로 | 용도 |
| --- | --- |
| `$OpenProvider_HOME/config.json` | 프로바이더, 기본 프로바이더, 포트, 옵션. |
| `$OpenProvider_HOME/opr.pid` | 실행 중인 프록시의 PID(단일 인스턴스 가드). |
| `$OpenProvider_HOME/runtime-port.json` | 자동으로 고른 대체 포트를 포함한 현재 PID, 호스트명, 포트. |
| `$OpenProvider_HOME/auth.json` | 저장된 OAuth 자격 증명(`opr login` 시). |
| `$OpenProvider_HOME/catalog-backup*.json` | OpenProvider가 수정하기 전에 만든 Codex 모델 카탈로그 백업. |
| `$CODEX_HOME/config.toml` | 로컬 전용 구성에서는 OpenProvider가 관리하는 루트 `openai_base_url`을 추가합니다. 로컬이 아닌 주소에 바인딩할 때는 Codex가 API 인증 헤더를 보낼 수 있도록 `model_provider = "OpenProvider"`와 `[model_providers.OpenProvider]`를 사용합니다. |
| `$CODEX_HOME/OpenProvider.config.toml` | 기본 Codex 설정과 함께 생성되는 참고용 fallback 프로필. |
| `$CODEX_HOME/OpenProvider-catalog.json` | Codex가 사용하는 네이티브 및 라우팅 모델 카탈로그. |

:::note
OpenProvider는 절대 Codex 설정을 삭제하지 않습니다. 모든 주입은 되돌릴 수 있습니다 — `opr stop`, `opr restore`,
또는 `opr eject`는 OpenProvider가 추가한 줄만 정확히 제거하고 네이티브 Codex를 복원합니다.
:::
