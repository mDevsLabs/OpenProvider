---
title: CLI 레퍼런스
description: 모든 opr 명령어와 플래그.
---

OpenProvider CLI는 `opr`입니다. 최상위 사용법은 `opr help`(또는 `--help` / `-h`)로 확인합니다.
도움말 표에 등록된 명령의 상세 사용법은 `opr help <command>`로 볼 수 있습니다. 도움말과 버전
...
반환합니다. 실시간 카탈로그를 가져오는 명령은 아닙니다. 그 작업은 `opr sync`나 대시보드를
사용하세요.
...
Codex pool은 `opr account refresh openai [--json]`을 사용합니다. 계정 quota를 강제로 새로 고치고
...
완료된 외부 Codex 업데이트가 설치된 shim을 덮어쓰면 다음 일반 `opr` 명령이 안정화된 새 런처를
백업하고 명령 실행 전에 shim을 복구합니다. 아직 변경 중인 런처는 건드리지 않고 이후에 다시
시도합니다. 복구 실패는 요청한 명령을 실패시키지 않고 경고만 출력하며, 수동 대체 명령은
`opr codex-shim install`입니다. 자동 복구를 끄려면 `codexShimAutoRestore`를 `false`로 설정하거나
...
:::tip[Service vs Shim]
항상 프록시를 켜두려면 `opr service`를 사용하세요(권장). 데몬 없이 필요할 때만 가볍게 시작하려면
`opr codex-shim`을 사용하세요. 이 경우 프록시는 `codex`를 실행할 때만 시작됩니다.
:::
...
포그라운드 설치에는 다음 단계로 `opr start`를 안내합니다.
...
`opr help`, `opr --help`, `opr -h` — 최상위 사용법과 예제를 출력합니다.

`opr help <command>`, `opr <command> --help`, `opr <command> -h` — `src/cli/help.ts`에 등록된
...
`opr --version`, `opr -v`, `opr version` — 스크립트에서 읽기 쉬운 한 줄 버전을 출력하고
종료합니다.
