...
라우팅 모델마다 호스팅 **웹 검색**이나 네이티브 **이미지 입력** 지원 범위가 다릅니다. OpenProvider는
...
Codex가 패스스루가 아닌 라우팅 모델에 호스팅 `web_search`를 요청하면 OpenProvider는 다음 순서로
처리합니다.
...
라우팅 모델의 모든 반복은 업스트림에 `stream: true`를 요청하지만, OpenProvider는 검색 여부나 최종
...
라우팅 모델이 해당 프로바이더의 `noVisionModels`에 있고 요청에 이미지가 들어오면, OpenProvider는
