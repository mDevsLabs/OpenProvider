...
ルーティングモデルごとにホスト型**ウェブ検索**やネイティブ**画像入力**のサポート範囲が異なります。OpenProvider は
...
Codex がパススルーでないルーティングモデルにホスト型 `web_search` を要求すると OpenProvider は次の順序で
処理します。
...
ルーティングモデルのすべての反復は上流に `stream: true` を要求しますが、OpenProvider は検索可否や最終
...
ルーティングモデルが該当プロバイダーの `noVisionModels` にありリクエストに画像が来る場合、OpenProvider は
