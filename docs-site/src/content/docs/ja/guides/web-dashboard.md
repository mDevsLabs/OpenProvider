---
title: ウェブダッシュボード
description: プロキシ状態、プロバイダー、モデル、委任ガイド、認証プール、使用量、ログを管理する OpenProvider GUI。
---

OpenProvider はプロキシが提供するローカルウェブダッシュボード(`gui/` 配下の Vite/React アプリ)を含みます。
...
| **Codex 自動起動** | インストール済み Codex launcher shim に `opr ensure` の実行を許可します。このトグルは shim やバックグラウンドサービスをインストールしません。 |
...
保存します。v1 ターンでは OpenProvider が親エージェントに `spawn_agent` に渡す正確なモデルと推論
