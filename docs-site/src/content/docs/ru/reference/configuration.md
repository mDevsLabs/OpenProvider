...
поставлявшейся v1-конфигурации OpenProvider создаёт `config.json.pre-openai-tiers-v2.bak`, не
...
| `hostname?` | `string` | `"127.0.0.1"` | Адрес привязки. Установите `"0.0.0.0"`, чтобы открыть доступ по LAN (требуется `OpenProvider_API_AUTH_TOKEN`; см. [Удалённый доступ](#удалённый-доступ) ниже). |
...
По умолчанию OpenProvider привязывается к `127.0.0.1` (только loopback). Когда `hostname` установлен
в не-loopback-адрес, такой как `0.0.0.0`, OpenProvider принудительно включает токенную
аутентификацию **и** для management API (`/api/*`), **и** для плоскости данных
(`/v1/responses`).

Установите переменную окружения `OpenProvider_API_AUTH_TOKEN` перед запуском:
...
| `escapeBuiltinToolNames?` | `boolean` | Anthropic-совместимые шлюзы, такие как Umans, могут требовать экранирования имён инструментов на wire; OpenProvider убирает префикс перед возвратом вызовов инструментов в Codex. |
...
Когда `liveModels` равен `false`, а `models` пуст или опущен, OpenProvider не открывает для этого
...
со стороны Claude, OpenProvider внедряет основную аутентификацию ChatGPT во внутренний запрос
...
OpenProvider отпечаток (fingerprint) Claude Code OAuth. Это в рамках существующего OAuth-прецедента
