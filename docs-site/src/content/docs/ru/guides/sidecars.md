...
изображений**. OpenProvider восполняет эти возможности двумя сайдкарами. Каждый может работать через
...
Явно заданный `backend` имеет приоритет. Если он не задан, OpenProvider использует `anthropic`, когда
...
2. Запускает маршрутизируемую модель в небольшом **агентном цикле**. Когда она вызывает
   `web_search`, OpenProvider использует выбранный бэкенд сайдкара: OpenAI выполняет hosted
...
OpenProvider полностью буферизует семантические события внутри, прежде чем решить, искать дальше или
...
изображение, OpenProvider описывает каждое изображение **до** основного вызова и заменяет его текстом.
