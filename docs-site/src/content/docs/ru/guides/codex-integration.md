...
- **Резерв Google Antigravity (CCA):** когда ни forward-кандидат OpenAI, ни провайдер с
  ...
  `opr login google-antigravity`; OAuth-токен отправляется только на закреплённый хост реестра
...
провайдеров с API-ключом OpenProvider преобразует приватное пространство имён Codex `image_gen`
...
он устанавливает `opr_SERVICE=1`, поэтому перезапуск, инициированный сервисом, **не** дёргает
