---
title: Claude Code
description: Используйте любую маршрутизируемую модель из Claude Code — OpenProvider обслуживает Anthropic Messages API и обнаружение моделей шлюза на одном и том же порту.
---

OpenProvider обслуживает `POST /v1/messages` (а также `count_tokens`) наряду с `/v1/responses`, поэтому
...
`opr claude` убеждается, что прокси запущен, а затем запускает Claude Code с уже настроенным окружением:
...
Переменные, которые вы экспортируете сами, всегда имеют приоритет. Дополнительные аргументы передаются как есть: `opr claude -p "hello"`.

## Интеграция с системным окружением (macOS)

Когда `claudeCode.systemEnv` установлен в `true` (по умолчанию: **выключено**), `opr start`
...
обычные команды `claude` через прокси без обёртки `opr claude`. Уже открытые оболочки не
затрагиваются — их нужно открыть заново.

`opr stop` и остановка прокси **снимают внедрённые ключи** (прежние значения не восстанавливаются —
удаляются только ключи, внедрённые OpenProvider). Прокси также записывает `~/.OpenProvider/claude-env.sh`;
`opr start` устанавливает source-хук в `.zshrc`, который загружает этот файл автоматически.

Отключить можно параметром `claudeCode.systemEnv: false` в конфигурации или переключателем в GUI.
Функция доступна только на macOS; на других платформах используйте `opr claude`.
...
`accept-encoding`, `x-OpenProvider-api-key` и `origin` удаляются перед пересылкой. Все остальные
...
что предупреждение «claude.ai connectors are disabled» с `opr claude` больше не появляется.
...
начинающиеся с `claude` или `anthropic`, OpenProvider публикует маршрутизируемые модели как
...
используйте `/model <id>` внутри этого диалога. OpenProvider не видит состояние селектора и
...
`effectiveModelEnv` вычисляет шесть слотов, внедряемых `opr claude`, системным окружением и
...
Если отсутствуют и `tierModels.haiku`, и `smallFastModel`, OpenProvider оставляет обе переменные вспомогательной модели незаданными. Затем Claude Code выбирает нативную вспомогательную модель (сейчас Sonnet), что может привести к расходам у нативного провайдера.

## Агенты из ростера (injectAgents)

`opr claude` (и демон системного окружения) синхронизирует ваш ростер избранных подагентов
(вкладка Subagents, до 5 моделей) плюс `opr-self` в `~/.claude/agents/opr-*.md`.

- **`opr-self`** закрепляет модель по умолчанию из селектора `/model` (с откатом на
  `claudeCode.model`); если нет ни того, ни другого, файл не создаётся. Наследование модели
  НЕ используется.
- В теле каждого агента есть директива `<!-- opr-route: <model> -->` — по ней прокси закрепляет
...
- Перезаписываются или удаляются только проверенные по маркеру файлы `opr-*.md`, содержащие
  `generated-by: OpenProvider`; ваши собственные агенты никогда не затрагиваются.
...
Диспетчеризация: `subagent_type: "opr-gpt-5-6-sol"`. Цели с поддержкой 1M автоматически получают `[1m]`.
...
этом пакете не обучались, поэтому по умолчанию OpenProvider подменяет содержимое скилла короткой
...
изображений. OpenProvider закрывает эти пробелы до того, как ответит основная модель:
...
Явно указанный `backend` всегда имеет приоритет. Если он не задан, OpenProvider выбирает
...
OpenProvider не заимствует втихую учётные данные ChatGPT и не переключает бэкенды. Бэкенд OpenAI
...
Захват входящих запросов управляется командой `opr debug claude on|off|status|reset`, переменной
`opr_CLAUDE_DEBUG=1` или запросом `PUT /api/debug {"claude": true}`.
...
- Быстрый старт (`opr claude`) и блок ручной настройки окружения
...
сборке поиск выполнялся, но Claude Code всё равно насчитывал ноль, обновите OpenProvider.
...
`ANTHROPIC_AUTH_TOKEN`. `opr claude` намеренно НЕ устанавливает `ANTHROPIC_API_KEY`; если вы
экспортировали его сами, снимите переменную. `opr claude` внедряет `ANTHROPIC_BASE_URL`,
...
`CLAUDE_CODE_ENABLE_GATEWAY_MODEL_DISCOVERY=1` (с `opr claude` — автоматически). Запустите
`opr claude`, чтобы обновить кеш моделей шлюза в `~/.claude/cache/gateway-models.json`.
...
для маршрутизируемых моделей OpenProvider по умолчанию подставляет заглушку
(`blockedSkills: ["claude-api"]`).

**Подагент отправляется не в ту модель** — агенты из ростера (`opr-*`) используют директивы
`<!-- opr-route: ... -->`, а не аргумент `model` инструмента Agent. Убедитесь, что директива
