...
export const zh: Record<TKey, string> = {
  ...
  // startup health
  "startup.title": "启动安全",
  "startup.subtitle": "检查重启后 Codex 是否仍能连接 OpenProvider，避免本地代理路由陷入重复重连。",
  "startup.refresh": "刷新",
  ...
  "startup.shimHint": "支持的 Codex 脚本启动器运行时执行 opr ensure。",
  "startup.healthy": "正常",
  ...
  "startup.tray.error": "Windows 托盘操作失败。请运行 opr tray status 查看详情。",
  ...
  "startup.codexRuntime.clampHidden": "部分推理强度选项已隐藏，因为 OpenProvider 正在使用 Codex {version}。",
  "startup.codexRuntime.clampHiddenWithEfforts": "部分推理强度选项已隐藏，因为 OpenProvider 正在使用 Codex {version}（已移除：{efforts}）。",
  "startup.codexRuntime.olderBinary": "OpenProvider 正在使用较旧的 Codex 二进制文件（{version}）。检测到可用的较新安装。",

  // dashboard
  "dash.subtitle": "本地 OpenProvider 代理、其提供方以及路由到 Codex 的模型的实时状态。",
  ...
  "dash.stopping": "正在停止…",
  "dash.codexAutoStart": "随 Codex 启动 OpenProvider",
  "dash.codexAutoStartHint": "允许已安装的 launcher shim 运行 opr ensure。此设置不会安装重启保护；请在启动安全中检查实际状态。",
  ...
  "dash.effortCapNone": "无上限",
    "dash.maintenance": "维护",
  "dash.maintenanceHint": "刷新 Codex 模型目录，或安装新的 OpenProvider 版本。",
  ...
  "dash.projectConfigTitle": "项目 Codex 配置绕过了 OpenProvider",
  "dash.projectConfigHint": "这些仓库级设置会覆盖 OpenProvider 代理（例如直接走 OpenCode Go）。请移除它们，以便该项目使用 ~/.codex/config.toml 的代理路由。",
  "dash.checkUpdate": "检查更新",
  "dash.updateTitle": "更新 OpenProvider",
  ...
  "dash.updateStatus.running": "正在更新 OpenProvider。",
  "dash.updateStatus.restarting": "更新已安装。正在重启代理。",
  "dash.updateStatus.succeeded": "更新完成。",
  "dash.updateStatus.failed": "更新失败。",

  // providers
  "prov.subtitle": "配置 OpenProvider 路由到 Codex 的上游提供方。使用账户登录、添加提供方，或编辑原始配置。",
  "prov.add": "添加提供方",
  ...
  "oauthTos.anthropicBody": "通过 OpenProvider 等第三方代理直接复用 Claude 订阅 OAuth 令牌，并非 Anthropic 支持的集成方式，可能导致访问受限。可使用 Claude 订阅的受支持 Agent SDK 集成属于另一种方式。",
  "oauthTos.highBody": "OpenProvider 通过第三方 OAuth 路径连接 {provider}。如果该用法不受支持，访问可能会被限制或暂停。",
  "oauthTos.elevatedBody": "OpenProvider 通过非官方 OAuth 路径连接 {provider}。请尽量使用官方客户端；异常或自动化流量可能被视为滥用，访问可能会被限制或暂停。",
  "oauthTos.saferPath": "更安全的做法：改为在 OpenProvider 中配置 API 密钥。",
  ...
  // models
  "models.subtitle": "开关 Codex 可见的模型 — 原生 GPT passthrough 与已路由模型按提供方分组（点击标题可折叠）。已禁用的模型会从目录和模型选择器中隐藏。更改在下一个 Codex 回合生效 — OpenProvider 会使 Codex 的 5 分钟模型缓存失效，因此无需重启。",
  ...
  // subagents
  "sub.subtitle": "Codex 的 {cmd} 仅将优先级最高的前 5 个模型作为覆盖项公开。在此最多选择 5 个 — 原生 gpt 或已路由模型 — OpenProvider 会设置它们的目录优先级，使其正好排在前面。其他模型仍可按确切名称调用；此设置仅控制显示项。",
  "sub.featured": "精选",
  ...
  "logs.tabLogs": "日志",
  "logs.tabDebug": "调试",
  "logs.subtitle": "经过本地 OpenProvider 代理的最近请求，最新在前。",
  ...
  // api access page
  "api.title": "API 访问",
  "api.subtitle": "使用生成的 API 密钥从外部应用访问 OpenProvider 代理。通过 {authHeader} 或 {altHeader} 请求头进行认证。",
  ...
  "api.authTitle": "身份验证",
  "api.authResponses": "Responses 请求应使用 x-OpenProvider-api-key，因为 Authorization 可能保留给 Codex Direct 直通。",
  "api.authChatCompletions": "Chat Completions 和 /v1/models 接受 Authorization: Bearer ocx_... 或 x-OpenProvider-api-key。",
  "api.authMessages": "Messages 接受 Authorization: Bearer ocx_... 或 x-OpenProvider-api-key。",
  "api.authLoopback": "回环绑定（127.0.0.1 或 ::1）会跳过身份验证。远程绑定需要生成的 ocx_ 密钥或 OpenProvider_API_AUTH_TOKEN。",
  ...
  "claude.injectAgentsDesc": "将“子代理”页选中的模型（以及当前默认模型）注册为 Claude Code 可派遣的代理（opr-*）。从下一个会话开始生效。",
 ...
 "modal.baseUrlPlaceholder": "https://...",
  ...
  "cws.howTitle": "工作原理",
  "cws.howBody": "在 Codex 中请求组合的公开模型名称；未设置时默认使用 combo/<id>。OpenProvider 仅在可重试的上游错误时切换目标。若没有可用目标，请求会直接失败，不会回退到全局默认提供方。",
  ...
  "grok.subtitle": "OpenProvider 已注册到你的 Grok 配置中的模型。",
  "grok.loading": "正在加载 Grok 状态…",
  "grok.loadFail": "无法读取 Grok 配置。",
  "grok.notConfiguredTitle": "Grok Build 尚未接入",
  "grok.notConfiguredHint": "安装 Grok 后重启代理，OpenProvider 会把托管块写入：",
  "grok.endpoint": "端点",
  "grok.colModel": "模型",
