...
export const ko: Record<TKey, string> = {
  ...
  "startup.subtitle": "재부팅 후 로컬 프록시 라우팅이 재연결 반복으로 이어지기 전에 Codex가 OpenProvider에 연결될 수 있는지 확인합니다.",
  ...
  "startup.summary.protected": "재부팅 후에도 OpenProvider가 자동으로 준비됩니다",
  ...
  "startup.riskDetailCustomLocal": "Codex가 사용자 지정 로컬 게이트웨이를 바라봅니다. OpenProvider는 해당 게이트웨이의 재시작 수명주기를 관리하거나 검증할 수 없습니다.",
  "startup.riskDetailWindowsShim": "Launcher shim은 지원되는 CLI 스크립트만 보호하며 Windows의 Codex Desktop과 직접 codex.exe 실행은 이를 우회할 수 있습니다.",
  "startup.safeDetail": "현재 라우팅과 시작 방식이 일치합니다. 재부팅 후 opr start를 수동으로 실행할 필요가 없습니다.",
  ...
  "startup.shimHint": "지원되는 Codex 스크립트 런처가 시작될 때 opr ensure를 실행합니다.",
  "startup.healthy": "정상",
  ...
  "startup.tray.error": "Windows 트레이 작업에 실패했습니다. opr tray status에서 상세 내용을 확인하세요.",
  ...
  "startup.codexRuntime.clampHidden": "OpenProvider가 Codex {version}을(를) 사용해 일부 reasoning effort 옵션이 숨겨졌습니다.",
  "startup.codexRuntime.clampHiddenWithEfforts": "OpenProvider가 Codex {version}을(를) 사용해 일부 reasoning effort 옵션이 숨겨졌습니다(제거됨: {efforts}).",
  "startup.codexRuntime.olderBinary": "OpenProvider가 더 오래된 Codex 바이너리({version})를 사용 중입니다. 더 새 설치를 사용할 수 있습니다.",

  // dashboard
  "dash.subtitle": "로컬 OpenProvider 프록시와 프로바이더, 그리고 Codex로 라우팅되는 모델의 실시간 상태입니다.",
  ...
  "dash.stopping": "중지 중…",
  "dash.codexAutoStart": "Codex 실행 시 OpenProvider 시작",
  "dash.codexAutoStartHint": "설치된 launcher shim이 opr ensure를 실행하도록 허용합니다. 이 설정은 재부팅 보호를 설치하지 않으므로 시작 안전성에서 실제 상태를 확인하세요.",
  ...
  "dash.maintenanceHint": "Codex 모델 카탈로그를 새로고침하거나 최신 OpenProvider 릴리스를 설치합니다.",
  ...
  "dash.projectConfigTitle": "프로젝트 Codex 설정이 OpenProvider를 우회합니다",
  "dash.projectConfigHint": "저장소 로컬 설정이 OpenProvider 프록시를 덮어씁니다(예: OpenCode Go로 직접 라우팅). 해당 프로젝트에서 ~/.codex/config.toml 프록시를 쓰려면 제거하세요.",
  "dash.checkUpdate": "업데이트 확인",
  "dash.updateTitle": "OpenProvider 업데이트",
  ...
  "dash.updateStatus.running": "OpenProvider 업데이트 중입니다.",
  ...
  // providers
  "prov.subtitle": "OpenProvider가 Codex로 라우팅하는 업스트림 프로바이더를 설정합니다. 계정으로 로그인하거나, 프로바이더를 추가하거나, 원본 설정을 편집하세요.",
  "prov.add": "프로바이더 추가",
  ...
  "oauthTos.anthropicBody": "Claude 구독 OAuth 토큰을 OpenProvider 같은 타사 프록시에서 직접 재사용하는 방식은 Anthropic이 지원하는 통합이 아니며 접근이 제한될 수 있습니다. Claude 구독을 사용하는 공식 Agent SDK 통합은 별도입니다.",
  "oauthTos.highBody": "OpenProvider는 {provider}를 타사 OAuth 경로로 연결합니다. 지원되지 않는 사용 방식이면 접근이 제한되거나 정지될 수 있습니다.",
  "oauthTos.elevatedBody": "OpenProvider는 {provider}를 비공식 OAuth 경로로 연결합니다. 가능하면 공식 클라이언트를 사용하세요. 비정상적이거나 자동화된 트래픽은 남용으로 간주되어 접근이 제한되거나 정지될 수 있습니다.",
  "oauthTos.saferPath": "더 안전한 방법: OpenProvider에 API 키를 대신 설정하세요.",
  ...
  // models
  "models.subtitle": "Codex가 보는 모델을 켜고 끕니다 — 네이티브 GPT passthrough와 라우팅된 모델을 프로바이더별로 묶어 보여줍니다(헤더를 클릭하면 접힘). 비활성화된 모델은 카탈로그와 모델 선택기에서 숨겨집니다. 변경 사항은 다음 Codex 턴에 적용됩니다 — OpenProvider가 Codex의 5분 모델 캐시를 무효화하므로 재시작이 필요 없습니다.",
  ...
  // subagents
  "sub.subtitle": "Codex의 {cmd} 는 우선순위 상위 5개 모델만 오버라이드로 노출합니다. 여기서 최대 5개를 선택하면 — 네이티브 gpt 또는 라우팅된 모델 — OpenProvider가 카탈로그 우선순위를 설정해 정확히 이들이 앞에 옵니다. 다른 모델도 정확한 이름으로 호출할 수 있으며, 이 설정은 표시 항목만 제어합니다.",
  "sub.featured": "추천",
  ...
  "logs.tabLogs": "로그",
  "logs.tabDebug": "디버그",
  "logs.subtitle": "로컬 OpenProvider 프록시를 거친 최근 요청입니다. 최신순.",
  ...
  // api access page
  "api.title": "API 액세스",
  "api.subtitle": "생성한 API 키로 외부 앱에서 OpenProvider 프록시에 접근합니다. {authHeader} 또는 {altHeader} 헤더로 인증합니다.",
  ...
  "api.authTitle": "인증",
  "api.authResponses": "Responses 요청은 Authorization이 Codex Direct 전달용으로 예약될 수 있으므로 x-OpenProvider-api-key를 사용해야 합니다.",
  "api.authChatCompletions": "Chat Completions와 /v1/models는 Authorization: Bearer ocx_... 또는 x-OpenProvider-api-key를 허용합니다.",
  "api.authMessages": "Messages는 Authorization: Bearer ocx_... 또는 x-OpenProvider-api-key를 허용합니다.",
  "api.authLoopback": "루프백 바인드(127.0.0.1 또는 ::1)는 인증을 건너뜁니다. 원격 바인드는 생성된 ocx_ 키 또는 OpenProvider_API_AUTH_TOKEN이 필요합니다.",
  ...
  "claude.authModeHint": "subscription은 Claude 계정 필요, proxy는 OpenProvider 프록시만으로 사용 가능",
  ...
  "claude.injectAgentsDesc": "위 '서브에이전트' 탭에서 고른 모델들(+현재 기본 모델)을 Claude Code의 파견 가능한 에이전트(opr-*)로 자동 등록합니다. 새 세션부터 적용돼요.",
 ...
 "modal.baseUrlPlaceholder": "https://...",
  ...
  "cws.howTitle": "동작 방식",
  "cws.howBody": "Codex에서 콤보의 공개 모델 이름을 요청하세요. 설정하지 않으면 combo/<id>가 기본값입니다. OpenProvider는 재시도 가능한 업스트림 오류에서만 다음 대상으로 넘깁니다. 사용 가능한 대상이 없으면 전역 기본 프로바이더로 우회하지 않고 요청을 실패 처리합니다.",
  ...
  "grok.subtitle": "OpenProvider가 Grok 설정에 등록한 모델입니다.",
  "grok.loading": "Grok 상태를 불러오는 중…",
  "grok.loadFail": "Grok 설정을 읽지 못했습니다.",
  "grok.notConfiguredTitle": "Grok Build가 연결되지 않았습니다",
  "grok.notConfiguredHint": "Grok을 설치한 뒤 프록시를 다시 시작하면 OpenProvider가 관리 블록을 다음 위치에 씁니다:",
  "grok.endpoint": "엔드포인트",
  "grok.colModel": "모델",
