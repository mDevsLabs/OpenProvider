...
export const ja: Record<TKey, string> = {
  ...
  // startup health
  "startup.title": "起動安全性",
  "startup.subtitle": "再起動後にローカルプロキシへの接続が再接続ループになる前に、Codex が OpenProvider へ到達できるか確認します。",
  "startup.refresh": "更新",
  ...
  "startup.summary.protected": "再起動後も OpenProvider を利用できます",
  ...
  "startup.riskDetailCustomLocal": "Codex はカスタムローカルゲートウェイを参照しています。OpenProvider はその再起動ライフサイクルを管理・検証できません。",
  "startup.riskDetailWindowsShim": "Launcher shim は対応する CLI スクリプトのみを保護し、Windows の Codex Desktop と codex.exe の直接起動はこれを迂回できます。",
  "startup.safeDetail": "現在のルーティングと起動方式は整合しています。再起動後に opr start を手動実行する必要はありません。",
  ...
  "startup.shimHint": "対応する Codex スクリプトランチャーの起動時に opr ensure を実行します。",
  "startup.healthy": "正常",
  ...
  "startup.tray.error": "Windows トレイ操作に失敗しました。opr tray status で詳細を確認してください。",
  ...
  "startup.codexRuntime.clampHidden": "OpenProvider が Codex {version} を使用したため、一部の reasoning effort オプションが非表示になりました。",
  "startup.codexRuntime.clampHiddenWithEfforts": "OpenProvider が Codex {version} を使用したため、一部の reasoning effort オプションが非表示になりました（削除: {efforts}）。",
  "startup.codexRuntime.olderBinary": "OpenProvider は古い Codex バイナリ（{version}）を使用しています。より新しいインストールが利用可能です。",

  // dashboard
  "dash.subtitle": "ローカル OpenProvider プロキシ、そのプロバイダー、Codex にルーティングされるモデルのライブ状態です。",
  ...
  "dash.stopping": "停止中…",
  "dash.codexAutoStart": "Codex と一緒に OpenProvider を起動",
  "dash.codexAutoStartHint": "インストール済み launcher shim に opr ensure の実行を許可します。この設定だけでは再起動保護はインストールされません。起動安全性で実際の状態を確認してください。",
  ...
  "dash.maintenanceHint": "Codex のモデルカタログを更新するか、より新しい OpenProvider リリースをインストールします。",
  ...
  "dash.projectConfigTitle": "プロジェクトの Codex 設定が OpenProvider をバイパスします",
  "dash.projectConfigHint": "これらのリポジローカル設定は OpenProvider プロキシを上書きします(例: OpenCode Go に直接ルーティング)。~/.codex/config.toml のルーティングがそのプロジェクトで適用されるように削除してください。",
  "dash.checkUpdate": "更新を確認",
  "dash.updateTitle": "OpenProvider を更新",
  ...
  "dash.updateStatus.running": "OpenProvider を更新しています。",
  ...
  // providers
  "prov.subtitle": "OpenProvider が Codex にルーティングする上流プロバイダーを設定します。アカウントでログインするか、プロバイダーを追加、または生の設定を編集します。",
  "prov.add": "プロバイダーを追加",
  ...
  "oauthTos.anthropicBody": "OpenProvider のような第三者プロキシ経由で Claude サブスクリプションの OAuth トークンを直接再利用することは、Anthropic がサポートする統合ではなく、アクセス制限につながる可能性があります。Claude サブスクリプションを使用するサポートされた Agent SDK 統合は別物です。",
  "oauthTos.highBody": "OpenProvider は第三者 OAuth パス経由で {provider} に接続します。サポート外の利用はアクセス制限や停止につながる可能性があります。",
  "oauthTos.elevatedBody": "OpenProvider は非公式 OAuth パス経由で {provider} に接続します。可能な場合は公式クライアントを使用してください。異常または自動化されたトラフィックは悪用とみなされ、アクセスが制限または停止される可能性があります。",
  "oauthTos.saferPath": "より安全な選択肢: 代わりに OpenProvider で API キーを設定してください。",
  ...
  // models
  "models.subtitle": "Codex に表示するモデルを切り替えます — ネイティブ GPT パススルーとルーティングプロバイダー、プロバイダー別(ヘッダーをクリックで折りたたみ)。無効化されたモデルはカタログ + モデルピッカーから隠されます。変更は次回の Codex ターンで適用 — OpenProvider は Codex の 5 分間モデルキャッシュを無効化するので再起動は不要です。",
  ...
  // subagents
  "sub.subtitle": "Codex の {cmd} は最初の 5 モデル(優先度順)のみをオーバーライドとして通知します。ここで最大 5 つを選んでください — ネイティブ gpt またはルーティング — OpenProvider がカタログ優先度を設定し、これらが先頭に来るようにします。他のモデルも正確な名前で呼び出し可能です; これは表示のみを制御します。",
  "sub.featured": "おすすめ",
  ...
  "logs.tabLogs": "ログ",
  "logs.tabDebug": "デバッグ",
  "logs.subtitle": "ローカル OpenProvider プロキシを経由した最近のリクエスト(新しい順)。",
  ...
  "pws.copyDoctor": "opr doctor をコピー",
  ...
  // api access page
  "api.title": "API アクセス",
  "api.subtitle": "生成した API キーで外部アプリから OpenProvider プロキシにアクセスします。キーは {authHeader} または {altHeader} ヘッダーで認証します。",
  ...
  "api.authResponses": "Responses リクエストは x-OpenProvider-api-key を使ってください。Authorization は Codex Direct 用に予約されている場合があります。",
  "api.authChatCompletions": "Chat Completions と /v1/models は Authorization: Bearer ocx_... または x-OpenProvider-api-key を受け付けます。",
  "api.authMessages": "Messages は Authorization: Bearer ocx_... または x-OpenProvider-api-key を受け付けます。",
  "api.authLoopback": "ループバック (127.0.0.1 または ::1) は認証を省略します。リモートでは生成した ocx_ キーまたは OpenProvider_API_AUTH_TOKEN が必要です。",
  ...
  "claude.injectAgentsDesc": "サブエージェントタブで選んだモデル(と現在のデフォルトモデル)をディスパッチ可能な Claude Code エージェント(opr-*)として登録します。次回セッションから適用されます。",
  ...
  "grok.subtitle": "OpenProvider が Grok 設定に登録したモデルです。",
  "grok.loading": "Grok の状態を読み込み中…",
  "grok.loadFail": "Grok 設定を読み取れませんでした。",
  "grok.notConfiguredTitle": "Grok Build が未設定です",
  "grok.notConfiguredHint": "Grok をインストールしてプロキシを再起動すると、OpenProvider が管理ブロックを次の場所に書き込みます:",
  ...
  "cws.howTitle": "仕組み",
  "cws.howBody": "Codex に combo/<id> を要求します。OpenProvider はターゲットを選び、再試行可能な上流の失敗時のみホップします。利用可能なターゲットが残っていない場合、グローバルなデフォルトプロバイダーを使わずにフェイルクローズします。",
