...
export const en = {
  ...
  "startup.subtitle": "Verify that Codex can reach OpenProvider after a restart, before local proxy routing becomes a reconnect loop.",
  ...
  "startup.summary.protected": "OpenProvider will be available after restart",
  ...
  "startup.riskDetailCustomLocal": "Codex points to a custom local gateway. OpenProvider cannot manage or verify that gateway's restart lifecycle.",
  ...
  "startup.safeDetail": "The current routing and startup mechanism are consistent. No manual opr start should be required after restart.",
  ...
  "startup.shimHint": "Runs opr ensure when a supported Codex script launcher starts.",
  ...
  "startup.tray.error": "The Windows tray action failed. Check opr tray status for details.",
  ...
  "startup.codexRuntime.clampHidden": "Some reasoning effort options were hidden because OpenProvider used Codex {version}.",
  "startup.codexRuntime.clampHiddenWithEfforts": "Some reasoning effort options were hidden because OpenProvider used Codex {version} (removed: {efforts}).",
  "startup.codexRuntime.olderBinary": "OpenProvider is using an older Codex binary ({version}). A newer installation is available.",

  // dashboard
  "dash.subtitle": "Live status of the local OpenProvider proxy, its providers, and the models routed into Codex.",
  ...
  "dash.codexAutoStart": "Start OpenProvider with Codex",
  "dash.codexAutoStartHint": "Allows an installed launcher shim to run opr ensure. This setting does not install restart protection; check Startup safety for the effective state.",
  ...
  "dash.maintenanceHint": "Refresh Codex's model catalog or install a newer OpenProvider release.",
  ...
  "dash.projectConfigTitle": "Project Codex config bypasses OpenProvider",
  "dash.projectConfigHint": "These repo-local settings override the OpenProvider proxy (e.g. route to OpenCode Go directly). Remove them so ~/.codex/config.toml routing applies in that project.",
  "dash.checkUpdate": "Check update",
  "dash.updateTitle": "Update OpenProvider",
  ...
  "dash.updateStatus.running": "Updating OpenProvider.",
  ...
  // providers
  "prov.subtitle": "Configure the upstream providers OpenProvider routes into Codex. Log in with an account, add a provider, or edit the raw config.",
  ...
  "oauthTos.anthropicBody": "Directly reusing Claude subscription OAuth tokens through a third-party proxy such as OpenProvider is not a supported Anthropic integration and may lead to access restrictions. Supported Agent SDK integrations that use Claude subscriptions are separate.",
  "oauthTos.highBody": "OpenProvider connects {provider} through a third-party OAuth path. Unsupported use may lead to access limits or suspension.",
  "oauthTos.elevatedBody": "OpenProvider connects {provider} through an unofficial OAuth path. Use the official client when possible; unusual or automated traffic may be treated as abuse and access may be limited or suspended.",
  "oauthTos.saferPath": "Safer option: configure an API key in OpenProvider instead.",
  ...
  // subagents
  "sub.subtitle": "Codex's {cmd} advertises only the first 5 models (by priority) as overrides. Pick up to 5 here — native gpt or routed — and OpenProvider sets their catalog priority so exactly these lead. Any other model is still callable by its exact name; this only controls what's shown.",
  ...
  "logs.tabLogs": "Logs",
  "logs.tabDebug": "Debug",
  "logs.subtitle": "Recent requests routed through the local OpenProvider proxy, newest first.",
 ...
 "pws.noModelMatch": "No models match the filter.",
  ...
  "pws.copyDoctor": "Copy opr doctor",
 ...
 "modal.baseUrlPlaceholder": "https://...",
  ...
  "api.subtitle": "Use generated API keys to access the OpenProvider proxy from external apps. Keys authenticate via the {authHeader} or {altHeader} header.",
  ...
  "api.authResponses": "Responses requests require x-OpenProvider-api-key because Authorization may be reserved for Codex Direct passthrough.",
  "api.authChatCompletions": "Chat Completions and /v1/models accept Authorization: Bearer ocx_... or x-OpenProvider-api-key.",
  "api.authLoopback": "Loopback binds (127.0.0.1 or ::1) bypass authentication. Remote binds require a generated ocx_ key or OpenProvider_API_AUTH_TOKEN.",
  "api.authMessages": "Messages accepts Authorization: Bearer ocx_... or x-OpenProvider-api-key.",
  ...
  "claude.injectAgentsDesc": "Registers the models picked on the Subagents tab (plus the current default model) as dispatchable Claude Code agents (opr-*). Applies from the next session.",
  ...
  "cws.howBody": "Ask Codex for the combo's public model name. Without one, the default is combo/<id>. OpenProvider selects a target and hops only on retryable upstream failures. If no target remains available, the request fails closed instead of using the global default provider.",
  ...
  "grok.subtitle": "Models OpenProvider has registered in your Grok config.",
  ...
  "grok.notConfiguredHint": "Start or restart the proxy with Grok installed and OpenProvider writes a managed block into:",
