 ...
 * already covered) into ~/.claude/agents/opr-*.md — one dispatchable
 ...
 * `opr-*.md` inside the agents dir. User-authored agents are never touched.
 */
 ...
 * no-model opr-self dispatch fell back to claude-fable-5 — devlog 072), so the
...
export function buildClaudeAgentDefs(config: oprConfig, windows: Record<string, number>, configDir = claudeConfigDir()): ClaudeAgentDef[] {
  ...
  for (const entry of roster.slice(0, 5)) {
    ...
    push(sanitizeName(id), alias, `Delegate work to ${id} (${provider}) via openprovider routing. General-purpose worker/explorer on that model. ${NO_MODEL_ARG}`);
  }
...
function renderAgentDef(def: ClaudeAgentDef): string {
  ...
  return [
    ...
    `<!-- opr-route: ${def.model} -->`,
    "",
    `You are a delegated worker running on \`${def.model}\` through the local openprovider proxy.`,
    `IDENTITY: your ACTUAL underlying model is \`${def.model}\` — the openprovider proxy routes this`,
    ...
    "auto-generated by openprovider (`opr claude`) from the featured subagent roster —",
...
/**
 * Sync owned agent files: write/overwrite current defs, prune stale opr-*.md,
...
export function syncClaudeAgentDefs(defs: readonly ClaudeAgentDef[], configDir = claudeConfigDir()): string[] | null {
  try {
    ...
    for (const def of defs) {
      ...
      // A pre-existing opr-* file WITHOUT our marker is user property: skip the def.
      try {
        lstatSync(target);
...
/**
 * Dispatcher directive appended to every opr-* description. The opr-route body
