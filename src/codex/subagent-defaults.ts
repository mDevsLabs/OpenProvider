...
export const MANAGED_SUBAGENT_DEFAULT_MARKER = "# Managed by OpenProvider: native subagent default";
export const MANAGED_AGENTS_TABLE_MARKER = "# Managed by OpenProvider: native subagent defaults table";
...
/**
 * Add/update OpenProvider-owned native subagent defaults, or remove them with
...
export function transformManagedSubagentDefaults(
  ...
  for (const key of TARGET_KEYS) {
    ...
    if (replaceManagedString(lines[definition.index]!.text, key, value ?? "__OpenProvider_validation__") === null) {
