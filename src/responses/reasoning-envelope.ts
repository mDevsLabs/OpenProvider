 ...
 * signature (and any redacted blocks) inside a transparent `oprr1:` + base64(JSON) envelope.
 *
 * Native OpenAI-encrypted blobs (no oprr1 prefix) are left untouched by the decoder, and the
 * passthrough scrub strips oprr1 envelopes before native forwarding.
 */

export const opr_REASONING_PREFIX = "oprr1:";
...
export function encodeReasoningEnvelope(envelope: ReasoningEnvelope): string {
  return opr_REASONING_PREFIX + Buffer.from(JSON.stringify(envelope), "utf-8").toString("base64");
}

/** Decode an oprr1 envelope; returns null for native (OpenAI-encrypted) blobs or garbage. */
export function decodeReasoningEnvelope(encryptedContent: string): ReasoningEnvelope | null {
  if (!encryptedContent.startsWith(opr_REASONING_PREFIX)) return null;
  try {
    const parsed: unknown = JSON.parse(Buffer.from(encryptedContent.slice(opr_REASONING_PREFIX.length), "base64").toString("utf-8"));
