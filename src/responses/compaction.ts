 ...
 * summarizer and wraps the summary text in a transparent envelope: `opr1:` + base64(utf8 summary).
 * Codex stores the item and replays it in later input; the parser decodes our envelope back into
 * plain text for routed models. Real OpenAI-encrypted blobs (no `opr1:` prefix) are opaque —
 * routed models get a short "history was compacted" note instead.
 */

export const opr_COMPACTION_PREFIX = "opr1:";
...
export function encodeCompactionSummary(summary: string): string {
  return opr_COMPACTION_PREFIX + Buffer.from(summary, "utf-8").toString("base64");
}

/** Decode an `opr1:` envelope; returns null for real (OpenAI-encrypted) blobs or garbage. */
export function decodeCompactionSummary(encryptedContent: string): string | null {
  if (!encryptedContent.startsWith(opr_COMPACTION_PREFIX)) return null;
  try {
    return Buffer.from(encryptedContent.slice(opr_COMPACTION_PREFIX.length), "base64").toString("utf-8");
  } catch {
    return null;
  }
}
