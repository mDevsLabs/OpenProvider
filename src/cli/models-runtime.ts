...
const USAGE = `Usage:
  opr models live [--provider <name>] [--json]
  opr models edit <custom-id> [--model-id <id>] [--display-name <name|->]
      [--context-window <tokens|0>] [--modalities <text,image,audio|->] [--json]
  opr models <enable|disable> <provider/model|native-model> [--native] [--json]
  opr models provider <name> <on|off> [--json]
  opr models selected <provider> [--set <id,id...>|--clear] [--json]
  opr models context <status|value <tokens>|provider <name> <on|off>|all <on|off>> [--json]
  opr models shadow <status|set> [model|-] [--enabled <on|off>] [--json]`;
