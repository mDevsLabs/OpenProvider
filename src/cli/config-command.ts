...
const USAGE = `Usage:
  opr config [show] [--json] [--source]
  opr config get <dot.path> [--json]
  opr config set <dot.path> <json-or-string> [--json]
  opr config unset <dot.path> [--json]
  opr config validate [path|-] [--json]
  opr config export <path|->
  opr config import <path|-> --yes [--json]`;
...
function validate(value: unknown): oprConfig {
...
export async function handleConfigCommand(argv: string[]): Promise<number> {
  return runCliAction(async () => {
    ...
    if (action === "import") {
      ...
      printData({ ok: true, source: path }, wantsJson, [`Imported config from ${path}. Restart or run opr sync if needed.`]);
      return;
    }
