...
export async function runtimeBaseUrl(deps: RuntimeApiDeps = {}): Promise<string> {
  ...
  if (!live) throw new RuntimeApiError("Proxy is not running. Start it with: opr start", 503, null);
 ...
 * tokens: mistyping `opr account cancel <p> --code <secret>` on a command that
...
export interface RejectArgsOptions {
   ...
   * positional is plausibly the credential itself — `opr account code <p>`
...
export async function readSecretLine(deps: RuntimeApiDeps, label: string): Promise<string> {
  ...
  // something-else | opr account code <p>` reaches here that way.
