...
/** Fires exactly once from the first interactive `opr start`. */
...
/**
 * First interactive `opr start`: a one-time `[Y/n]` "star on GitHub?" prompt.
...
export async function maybeShowStarPrompt(): Promise<void> {
  try {
    ...
    let yes = false;
    try {
      const ans = (await rl.question("\n  \x1b[38;5;141m⭐ Enjoying openprovider? Star it on GitHub?\x1b[0m [Y/n] ")).trim().toLowerCase();
