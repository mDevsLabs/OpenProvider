...
const USAGE = `Usage:
  opr account login <provider> [--id <account-id>] [--reauth] [--code -] [--no-wait] [--json]
  opr account code <provider> [--flow <flow-id>] [--json]   (reads the code from stdin)
  opr account cancel <provider> [--flow <flow-id>] [--json]
  opr account reset-credits <account-id|main> [--consume --yes] [--json]
...
visible to anyone who can run ps:
  pbpaste | opr account code <provider> --flow <flow-id>
  opr account login <provider> --code -   (same, for the login flow)`;
...
async function login(argv: string[], deps: RuntimeApiDeps): Promise<void> {
  ...
  // Only resolve when --code was actually given: a plain `opr account login`
...
async function code(argv: string[], deps: RuntimeApiDeps): Promise<void> {
  ...
  // `opr account code openai --flow f1` read `--flow` as the code and then
