...
/**
 * Does the openprovider dummy marker belong in the system environment?
 *
 * Keyed on the SAME resolver `opr claude` uses, so an auto config with no Claude auth
 ...
 *
 * NOTE this is a SNAPSHOT: the file only changes when this runs (proxy start, `opr
 * ensure`, or a settings save). `opr claude` re-resolves live on every launch.
 */
function systemEnvMarkerMode(config: oprConfig): "proxy" | "subscription" {
...
function writeShellEnvFile(port: number, config: oprConfig, modelEnv: Record<string, string> = {}, auto?: AutoContextMode): void {
  const lines = [
    `# Generated by openprovider — do not edit manually`,
  ...
  // Auto-context (devlog 260712 020): same contract as `opr claude` / launchctl.
...
const SHELL_HOOK_MARKER = "# openprovider claude-env hook";
const SHELL_HOOK_LINE = `${SHELL_HOOK_MARKER}\n[ -f ~/.openprovider/claude-env.sh ] && source ~/.openprovider/claude-env.sh`;
 ...
 */
async function computeEffectiveModelEnv(config: oprConfig, auto?: AutoContextMode): Promise<{ modelEnv: Record<string, string>; windows: Record<string, number> }> {
...
export async function injectSystemEnv(port: number, config: oprConfig): Promise<SystemEnvResult> {
  ...
  try {
    ...
    if (config.apiKeys?.length) {
      ...
      // the openprovider-owned dummy token so a launchd-started Claude regains its own
...
export async function applySystemEnvToggle(config: oprConfig, port: number): Promise<SystemEnvResult | RevertResult> {
