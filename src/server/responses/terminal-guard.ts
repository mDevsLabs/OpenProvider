...
function latestUserText(parsed: oprParsedRequest): string {
...
function recentTurnHasToolActivity(parsed: oprParsedRequest): boolean {
  ...
  let lastAssistant: oprAssistantMessage | undefined;
...
export function analyzeTerminalTurn(parsed: oprParsedRequest, events: readonly AdapterEvent[]): TerminalTurnAnalysis {
...
function assistantMessageFromEvents(events: readonly AdapterEvent[]): oprAssistantMessage | undefined {
  let text = "";
  let thinking = "";
  ...
  const content: oprAssistantContentPart[] = [];
...
export function buildContinuationRequest(parsed: oprParsedRequest, events: readonly AdapterEvent[]): oprParsedRequest {
...
export interface GuardedEventStreamOptions {
  ...
  continuation: (parsed: oprParsedRequest) => AsyncIterable<AdapterEvent> | Promise<AsyncIterable<AdapterEvent>>;
  adapterName?: string;
  maxAutoContinuations?: number;
}

function mergeUsage(first: oprUsage | undefined, second: oprUsage | undefined): oprUsage | undefined {
...
export async function* guardTerminalEventStream(options: GuardedEventStreamOptions): AsyncGenerator<AdapterEvent> {
  ...
  let continuations = 0;
  let accumulatedUsage: oprUsage | undefined;
