...
export async function mcpExec(execMsg: ExecServerMessage, deps: CursorNativeToolDeps): Promise<Uint8Array> {
  ...
  try {
    ...
    const result = deps.mcp
      ...
      : create(McpResultSchema, { result: { case: "error", value: create(McpErrorSchema, { error: "No local MCP executor is configured inside openprovider." }) } });
  ...
  try {
    const result = deps.readMcpResource
      ...
      : create(ReadMcpResourceExecResultSchema, {
        result: { case: "error", value: create(ReadMcpResourceErrorSchema, { uri: args.uri, error: "No local MCP resource executor is configured inside openprovider." }) },
      });
