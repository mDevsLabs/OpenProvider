...
import type { oprContentPart, oprParsedRequest } from "../types";
...
/**
 * Whether a stored `oprToolCall.thoughtSignature` is a REAL upstream Gemini signature versus a
...
function firstUserText(parsed: oprParsedRequest): string | undefined {
  for (const msg of parsed.context.messages) {
    if (msg.role !== "user") continue;
    if (typeof msg.content === "string") return msg.content;
    const first = (msg.content as oprContentPart[]).find(p => p.type === "text" && typeof p.text === "string");
 ...
 */
export function antigravitySessionId(parsed: oprParsedRequest): string {
