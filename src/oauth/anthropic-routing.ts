...
import type { oprAccountPoolRotationStrategy, oprConfig } from "../types";
...
export function anthropicAccountPoolConfig(config: oprConfig): AnthropicAccountPoolConfig {
  const raw = config.anthropicAccountPool;
  if (!raw || typeof raw !== "object") return {};
  return raw;
}

export function isAnthropicAccountPoolEnabled(config: oprConfig): boolean {
  return anthropicAccountPoolConfig(config).enabled === true;
}

export function anthropicAutoSwitchThreshold(config: oprConfig): number {
...
function stickyLimitForPool(config: oprConfig): number {
  return normalizeAccountPoolStickyLimit(anthropicAccountPoolConfig(config).stickyLimit);
}

function anthropicPoolStrategy(config: oprConfig): oprAccountPoolRotationStrategy {
 ...
 */
function pickFillFirstAnthropicAccount(config: oprConfig, now: number): string | null {
