...
const KIRO_CLI_RECOVERY_SUFFIX = ".OpenProvider-recovery";
const KIRO_CLI_RECOVERY_HEADER_V1 = Buffer.from("OpenProvider-kiro-session-v1\n", "utf8");
const KIRO_CLI_RECOVERY_HEADER = Buffer.from("OpenProvider-kiro-session-v2\n", "utf8");
...
export function restoreKiroCliSession(snapshot: KiroCliSessionSnapshot): void {
  const nonce = `${process.pid}.${Date.now()}.${Math.random().toString(16).slice(2)}`;
  const staged = `${snapshot.path}.opr-restore.${nonce}.tmp`;
  const displacedBase = `${snapshot.path}.opr-restore.${nonce}.new`;
