    ...
    if (local) {
      ...
      throw new Error(
        process.platform === "darwin"
          ? "No Claude Code token found (Keychain or ~/.claude/.credentials.json). Run 'opr login anthropic' for browser OAuth."
          : "No Claude Code token found (~/.claude/.credentials.json). Run 'opr login anthropic' for browser OAuth.",
      );
    }
  }
